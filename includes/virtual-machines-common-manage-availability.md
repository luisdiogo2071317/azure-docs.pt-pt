...
Os [discos geridos](../articles/virtual-machines/windows/managed-disks-overview.md) proporcionam uma melhor fiabilidade para os Conjuntos de Disponibilidade ao garantir que os discos das VMs num Conjunto de Disponibilidade estão suficientemente isolados uns dos outros a fim de evitar pontos de falha únicos. Este é automaticamente colocar os discos em domínios de falhas de armazenamento diferente (clusters de armazenamento) e alinhá-los com o domínio de falhas VM. Se a um domínio de falhas de armazenamento falhar devido uma falha de hardware ou software, apenas a instância VM com discos o domínio de falhas de armazenamento falha.
![Discos FDs geridos pelo](./media/virtual-machines-common-manage-availability/md-fd-updated.png)
...
3. **Utilize uma conta de armazenamento separada para cada VM num Conjunto de Disponibilidade.** Não partilhe Contas de armazenamento com várias VMs no mesmo Conjunto de Disponibilidade. É aceitável para as VMs em diferentes conjuntos de disponibilidade para partilhar contas de armazenamento se forem seguidos acima melhores práticas ![não gerido discos FDs](./media/virtual-machines-common-manage-availability/umd-updated.png)
