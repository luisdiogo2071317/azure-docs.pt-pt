...
| Zonas de Disponibilidade | SKU básico, com redundância de zona e zonais front-ends para entrada e mapeamentos de fluxos de saída, saída resistir a falhas de zona, balanceamento de carga entre zonas. | Não Disponível |
...
| Seguro por padrão | Predefinido entrada fechado para pontos finais públicos do Balanceador de carga e de IP e um grupo de segurança de rede deve ser usado para explicitamente lista de permissões para o tráfego para o fluxo. | Padrão aberto, grupo de segurança de rede opcional. |
...
| [Regras de saída](../articles/load-balancer/load-balancer-outbound-rules-overview.md) | Alocação de porta de declarativa configuração de NAT saída, incluindo o endereço de IP público (s) ou s IP público, configurável saída tempo limite de inatividade, SNAT personalizado | Não disponível |
