...
- Para utilizar uma VNets clássica, o principal de serviço `MicrosoftAzureBatch` tem de ter a função `Classic Virtual Machine Contributor` do Controlo de Acesso Baseado em Funções (RBAC) na VNet especificada. Para utilizar uma VNet com base no Azure Resource Manager, tem de ter permissões para aceder a VNet e implementar VMs da sub-rede.
