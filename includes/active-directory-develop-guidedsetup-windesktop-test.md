...
Quando estiver pronto para executar o teste, utilize uma conta do Microsoft Azure Active Directory (conta escolar ou profissional) ou uma conta Microsoft (live.com, outlook.com) para iniciar sessão. Na primeira vez que executar a aplicação, é-lhe pedido que inicie sessão no:
...
Depois de iniciar sessão, deverá ver as informações de perfil de utilizador devolvidas pela chamada para a Microsoft Graph API. Os resultados são apresentados no **resultados de chamada de API** caixa. Informações básicas sobre o token que foi adquirido através de uma chamada para **AcquireTokenAsync** ou **AcquireTokenSilentAsync** devem ser visíveis na **informações do Token** caixa. Os resultados contêm as seguintes propriedades:
...
|**Token de acesso** |Cadeia |A cadeia do token que é enviada para HTTP pedidos que necessitam de um **autorização** cabeçalho.|
