---
title: Manter o fornecedor de recursos do SQL no Azure Stack | Documentos da Microsoft
description: Saiba como pode manter o serviço de fornecedor de recursos do SQL no Azure Stack.
...
O fornecedor de recursos do SQL não é atendido como parte do Azure Stack, porque é um componente do suplemento. A Microsoft fornece atualizações para o fornecedor de recursos do SQL conforme necessário. Quando for lançada uma placa SQL atualizada, é fornecido um script para aplicar a atualização. Este script cria um novo fornecedor de recursos VM, migrando o estado do fornecedor de antigo VM para a nova VM. Para obter mais informações, consulte [atualizar o fornecedor de recursos do SQL](azure-stack-sql-resource-provider-update.md).
...
É responsável pela criação e manutenção de contas de administrador do sistema nos seus servidores SQL. O fornecedor de recursos precisa de uma conta com esses privilégios para gerir bases de dados para os utilizadores, mas não tem acesso aos dados dos utilizadores. Se precisar de atualizar as palavras-passe de administrador do sistema nos seus servidores SQL, pode utilizar a interface de administrador do fornecedor de recursos para alterar uma palavra-passe armazenada. Essas senhas são armazenadas no Cofre de chaves na sua instância do Azure Stack.

Para modificar as definições, selecione **navegue** &gt; **recursos administrativos** &gt; **servidores de alojamento de SQL** &gt; **Inícios de sessão SQL** e selecione um nome de utilizador. A alteração deve ser feita na instância do SQL primeiro (e as réplicas, se necessário.) Sob **configurações**, selecione **palavra-passe**.
...
|VMLocalCredential|Conta de administrador local na MySQLAdapter VM.|
...
**Solução**:<br>
Utilize o cmdlet Get-AzsDBAdapterLogs para recolher todos os registos do fornecedor de recursos, incluindo AzureStack.DatabaseAdapter.SecretRotation.ps1_*.log, salvo na C:\Logs.

## <a name="update-the-virtual-machine-operating-system"></a>Atualize o sistema de operativo da máquina virtual

Utilize um dos seguintes métodos para atualizar o sistema de operativo da máquina virtual.
...
1. As definições do Windows Defender update a partir de transferência [definição do Windows Defender](https://www.microsoft.com/en-us/wdsi/definitions).
   ...
   Nas definições de atualização de página, desloque para baixo até "Manualmente, transfira e instale as definições". Transfira o ficheiro de 64 bits "Antivírus do Windows Defender para Windows 10 e Windows 8.1".
...
2. Crie uma sessão do PowerShell para o ponto final de manutenção do SQL recursos fornecedor adaptador da máquina virtual.

3. Copie o ficheiro de atualização de definições para a máquina virtual com a sessão de ponto final de manutenção.

4. Na sessão do PowerShell de manutenção, execute o *DBAdapterWindowsDefenderDefinitions atualização* comando.

5. Depois de instalar as definições, recomendamos que elimine o ficheiro de atualização de definições utilizando a *Remove-ItemOnUserDrive* comando.
...
Quando um fornecedor de recursos é instalado ou atualizado, o **dbadapterdiag** é criada a conta de utilizador. Irá utilizar esta conta para recolher registos de diagnóstico.
...
Para utilizar o *DBAdapterDiagnostics* comandos, criar uma sessão remota do PowerShell para a máquina de virtual de fornecedor de recursos e executar o **Get-AzsDBAdapterLog** comando.
