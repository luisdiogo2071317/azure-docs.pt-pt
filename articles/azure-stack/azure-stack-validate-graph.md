---
title: Validar a integração de gráfico do Azure para o Azure Stack
...
ms.locfileid: "49947338"
---
# <a name="validate-graph-integration-for-azure-stack"></a>Validar a integração de gráfico para o Azure Stack

Utilize a ferramenta do Verificador de preparação do Azure Stack (AzsReadinessChecker) para validar se o seu ambiente está pronto para a integração de gráfico com o Azure Stack. Deve validar a integração do gráfico antes de começar a integração do Centro de dados ou antes de uma implementação do Azure Stack.
...
* Identificar o nome de utilizador e palavra-passe para uma conta para o serviço de gráficos no Active Directory existente
* Identificar a raiz de floresta do Active Directory FQDN

## <a name="validate-graph"></a>Validar o gráfico
...
1. A partir da linha do PowerShell, execute o seguinte para definir *$graphCredential* variável para a conta de gráfico. Substitua `contoso\graphservice` com a sua conta com o `domain\username` formato.

    `$graphCredential = Get-Credential contoso\graphservice -Message "Enter Credentials for the Graph Service Account"`

1. Prompt do PowerShell, execute o seguinte para iniciar a validação de gráfico. Especifique o valor para **- ForestFQDN** como o FQDN para a raiz de floresta:

     `Invoke-AzsGraphValidation -ForestFQDN contoso.com -Credential $graphCredential`

1. Depois da ferramenta é executada, reveja o resultado. Confirme que o estado está OK para requisitos de integração do gráfico. Uma validação com êxito parece ser semelhante ao seguinte:
...
Em ambientes de produção, testar a conectividade de rede de uma estação de trabalho de operadores não pode ser considerado totalmente indicativa de conectividade disponível para o Azure Stack. Tem a conectividade para o tráfego LDAP efetuar a integração de identidade de rede VIP pública do carimbo de Azure Stack.
...
Se uma verificação de validação falhar, exibir detalhes sobre a falha na janela do PowerShell. A ferramenta também regista informações para o *AzsGraphIntegration.log*.
