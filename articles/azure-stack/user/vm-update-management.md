...
description: Saiba como utilizar as soluções de gestão de atualizações, controlo de alterações e inventário na automatização do Azure para gerir VMs do Windows que são implementadas no Azure Stack.
...
- **[Gestão de atualizações](https://docs.microsoft.com/azure/automation/automation-update-management)**. Com a solução de gestão de atualizações, pode rapidamente avaliar o estado das atualizações disponíveis em todos os computadores agente e gerenciar o processo de instalação de atualizações necessárias para estas VMs do Windows.

- **[Controlo de alterações](https://docs.microsoft.com/azure/automation/automation-change-tracking)**. Alterações de software instalado, de serviços do Windows, de registo do Windows e de ficheiros nos servidores monitorizados são enviadas para o serviço do Log Analytics na cloud para processamento. Lógica é aplicada para os dados recebidos e o serviço em nuvem regista os dados. Ao utilizar as informações no dashboard do controlo de alterações, pode ver facilmente as alterações efetuadas na sua infraestrutura de servidor.

- **[Inventário](https://docs.microsoft.com/azure/automation/automation-vm-inventory)**. O inventário de controlo para uma máquina virtual do Azure Stack Windows fornece uma interface de utilizador baseada no browser para definir e configurar a recolha de inventário. 

> [!IMPORTANT]
> Estas soluções são as mesmas que as utilizadas para gerir VMs do Azure. O Azure e VMs do Windows do Azure Stack são geridas da mesma forma, a partir da mesma interface, com as mesmas ferramentas. As VMs de pilha do Azure também são cobradas as mesmas que as VMs do Azure ao utilizar a gestão de atualizações, controlo de alterações e inventário soluções com o Azure Stack.

## <a name="prerequisites"></a>Pré-requisitos
Vários pré-requisitos têm de ser cumpridos antes de utilizar estas funcionalidades para atualizar e gerir VMs do Windows do Azure Stack. Estes incluem os passos que devem ser levados no portal do Azure, bem como o portal de administração do Azure Stack.
...
Depois de ativar as soluções de automatização do Azure no portal do Azure, em seguida tem de iniciar sessão no portal de administração do Azure Stack como um administrador da nuvem e transfira o **atualização do Azure e o gerenciamento de configuração** extensão do Azure Item do mercado de pilha. 
...
Siga estes passos para ativar a gestão de atualizações para as VMs do Windows de pilha do Azure.

1. Inicie sessão no portal de utilizador do Azure Stack.

2. No portal do Azure Stack utilizador-, aceda ao painel de extensões do Windows, as máquinas virtuais para o qual pretende ativar estas soluções, clique em **+ adicionar**, selecione o **atualização do Azure e o gerenciamento de configuração**extensão e clique em **criar**:

   [![](media/vm-update-management/3-sm.PNG "Painel de extensão de VM do Windows")](media/vm-update-management/3-lg.PNG#lightbox)
...
As VMs do Azure Stack Windows agora podem ser incluídas em implementações de atualização agendada, juntamente com as VMs do Azure.
...
Se tiver um grande número de VMs do Windows do Azure Stack, pode utilizar [este modelo do Azure Resource Manager](https://github.com/Azure/AzureStack-QuickStart-Templates/tree/master/MicrosoftMonitoringAgent-ext-win) implantar mais facilmente a solução em VMs do Windows. O modelo implementa a extensão do Microsoft Monitoring Agent para uma VM existente do Windows e adiciona-o para uma área de trabalho do Azure LogAnalytics existente.
