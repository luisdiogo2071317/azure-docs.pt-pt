...
[OpenFaaS] [ open-faas] é uma estrutura para a criação de funções sem servidor por cima de contentores. Como um projeto de código aberto, este tem adquiridos adoção em grande escala dentro da Comunidade. Este documento fornece detalhes sobre a instalação e utilização OpenFaas num cluster do serviço de contentor do Azure (AKS).
...
* Um cluster do serviço de contentor do Azure (AKS) e as credenciais AKS configuradas no sistema de desenvolvimento.
...
Por fim, instale a CLI de OpenFaaS. Este exemplo utilizado brew, consulte o [documentação OpenFaaS CLI] [ open-faas-cli] para mais opções.
