...
description: Saiba como pode criar e gerir a sua experiência de início de sessão em aplicações com o Azure Active Directory B2C.
...
ms.locfileid: "37440886"
---
# <a name="what-is-azure-active-directory-b2c"></a>O que é o Azure Active Directory B2C?

O Azure Active Directory (Azure AD) B2C é um serviço de gestão de identidades que lhe permite personalizar e controlar a forma como os clientes se inscrevem, iniciam sessão e gerem os respetivos perfis quando utilizam as suas aplicações. Isto inclui aplicações desenvolvidas para iOS, Android e .NET, entre outros. O Azure AD B2C permite estas ações ao mesmo tempo que protege as identidades dos seus clientes.

Pode configurar uma aplicação registada no Azure AD B2C para realizar diversas ações de gestão de identidades. Alguns exemplos incluem:
...
O Azure AD B2C suporta o [OpenID Connect](https://openid.net/connect/) para todas as experiências de cliente. Na implementação do OpenID Connect do Azure AD B2C, a sua aplicação inicia este percurso do utilizador através da emissão de pedidos de autenticação para o Azure AD B2C. O resultado do pedido é um `id_token`. Este token de segurança representa a identidade do cliente.

Cada aplicação que utiliza o Azure AD B2C tem de ser registada num inquilino do Azure AD B2C com o portal do Azure. O processo de registo recolhe e atribui valores à sua aplicação. Estes valores incluem um ID de aplicação que identifica exclusivamente a aplicação e um URI de redirecionamento que pode ser utilizado para direcionar as respostas de volta ao mesmo.
...
Estes passos podem diferir ligeiramente dependendo do tipo de aplicação que está a criar.
...
O Azure AD B2C protege de ataques denial of service e de palavra-passe lançados contra as suas aplicações de várias formas. O Azure AD B2C utiliza técnicas de deteção e mitigação, como cookies SYN e limites de velocidade e de ligação para proteger os recursos contra ataques denial of service. A mitigação também é incluída para ataques de força bruta de palavra-passe e ataques de palavra-passe de dicionário.
...
- Contas de redes sociais que o cliente pode utilizar para se inscrever na aplicação
...
[Políticas personalizadas](active-directory-b2c-overview-custom.md) são ficheiros de configuração que definem o comportamento do Framework de Experiência de Identidade no seu inquilino do Azure AD B2C. As políticas personalizadas podem ser totalmente editadas para realizar muitas tarefas. Uma política personalizada é representada como um ou vários ficheiros com formatação XML que fazem referência entre si numa cadeia hierárquica. 

Várias políticas personalizadas de diferentes tipos podem ser utilizadas no seu inquilino do Azure AD B2C, conforme necessário, e podem ser reutilizadas em todas as aplicações. Esta flexibilidade permite-lhe definir e modificar experiências de identidade de cliente com alterações mínimas ou sem alterações ao seu código. As políticas podem ser utilizadas ao adicionar um parâmetro de consulta especial a pedidos de autenticação HTTP.

As políticas personalizadas podem ser utilizadas para controlar os percursos do utilizador das seguintes formas:
...
Um fornecedor de identidade é um serviço que efetua a autenticação de identidades de clientes e emite tokens de segurança. No Azure AD B2C, pode configurar vários fornecedores de identidade no seu inquilino, como uma conta Microsoft, Facebook ou Amazon, entre outros. 
...
A maior parte do conteúdo HTML e CSS que é apresentado aos clientes pode ser controlado. Ao utilizar a funcionalidade de personalização da IU de página, personaliza o aspeto e a funcionalidade de qualquer política. Também pode manter a consistência visual e de marca entre a sua aplicação e o Azure AD B2C.
...
É preciso associar o seu inquilino do Azure AD B2C à sua subscrição do Azure para ativar todas as funcionalidades e pagar os custos de utilização. Para permitir que os clientes do Azure AD B2C iniciem sessão na sua aplicação, tem de registá-la num inquilino do Azure AD B2C.
...
> [Tutorial: Registar uma aplicação para ativar a inscrição e o início de sessão com o Azure AD B2C](tutorial-register-applications.md)
