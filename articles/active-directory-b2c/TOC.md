# [Documentação do Azure AD B2C](index.md)

# [Descrição geral](active-directory-b2c-overview.md)

# Introdução
## [Testar uma aplicação web do Azure AD B2C](active-directory-b2c-quickstarts-web-app.md)
## [Testar uma aplicação de ambiente de trabalho do Azure AD B2C](active-directory-b2c-quickstarts-desktop-app.md)
## [Testar uma aplicação de página única do Azure AD B2C](active-directory-b2c-quickstarts-spa.md)
## [Criar um Diretório do Azure AD B2C](active-directory-b2c-get-started.md)
## [Registar a sua aplicação](active-directory-b2c-app-registration.md)
## [Políticas Incorporadas](active-directory-b2c-reference-policies.md)
## Aplicações móveis e de ambiente de trabalho
### [Descrição geral](active-directory-b2c-reference-oauth-code.md)
### iOS
#### [iOS Swift através de MSAL](https://github.com/Azure-Samples/active-directory-b2c-ios-swift-native-msal)
#### [iOS ObjC através de AppAuth](active-directory-b2c-devquickstarts-ios.md)
### Android
#### [Android através de MSAL](https://github.com/Azure-Samples/active-directory-b2c-android-native-msal)
#### [Android através de AppAuth](active-directory-b2c-devquickstarts-android.md)
### [.NET](https://github.com/Azure-Samples/active-directory-b2c-dotnet-desktop)
### [Xamarin](https://github.com/Azure-Samples/active-directory-b2c-xamarin-native)
## Web Apps
### [Descrição geral](active-directory-b2c-reference-oidc.md)
### [Autenticar utilizadores no ASP.NET](active-directory-b2c-tutorials-web-app.md)
### [ASP.NET](active-directory-b2c-devquickstarts-web-dotnet-susi.md)
### [Núcleo do ASP.NET](https://github.com/Azure-Samples/active-directory-b2c-dotnetcore-webapp)
### [Node.js](active-directory-b2c-devquickstarts-web-node.md)
## Aplicações de página única
### [Descrição geral](active-directory-b2c-reference-spa.md)
### [JavaScript com MSAL](https://github.com/Azure-Samples/active-directory-b2c-javascript-msal-singlepageapp)
### [JavaScript com Hello](https://github.com/Azure-Samples/active-directory-b2c-javascript-singlepageapp-dotnet-webapi)
## APIs da Web
### [Proteger uma API do ASP.NET](active-directory-b2c-tutorials-web-api.md)
### [ASP.NET](active-directory-b2c-devquickstarts-api-dotnet.md)
### [Núcleo do ASP.NET](https://github.com/Azure-Samples/active-directory-b2c-dotnetcore-webapi)
### [Node.js](active-directory-b2c-devquickstarts-api-node.md)
### [Chamar uma API Web .NET](active-directory-b2c-devquickstarts-web-api-dotnet.md)

# Guias
## Configurar fornecedores de identidades
### [Conta Microsoft](active-directory-b2c-setup-msa-app.md)
### [Facebook](active-directory-b2c-setup-fb-app.md)
### [Google](active-directory-b2c-setup-goog-app.md)
### [Amazon](active-directory-b2c-setup-amzn-app.md)
### [LinkedIn](active-directory-b2c-setup-li-app.md)
### [Twitter](active-directory-b2c-setup-twitter-app.md)
### [GitHub](active-directory-b2c-setup-github-app.md)
### [Weibo](active-directory-b2c-setup-weibo-app.md)
### [QQ](active-directory-b2c-setup-qq-app.md)
### [WeChat](active-directory-b2c-setup-wechat-app.md)
## [Configurar a reposição personalizada de palavra-passe](active-directory-b2c-reference-sspr.md)
## [Personalizar a IU](active-directory-b2c-reference-ui-customization.md)
### [Ferramenta auxiliar de personalização](active-directory-b2c-reference-ui-customization-helper-tool.md)
## [Utilizar a Graph API do Azure AD](active-directory-b2c-devquickstarts-graph-dotnet.md)
## [Obter tokens de acesso](active-directory-b2c-access-tokens.md)
## [Desativar a verificação do e-mail](active-directory-b2c-reference-disable-ev.md)
## [Configurar tokens](active-directory-b2c-token-session-sso.md)
## [Ativar MFA](active-directory-b2c-reference-mfa.md)
## [Utilizar atributos personalizados](active-directory-b2c-reference-custom-attr.md)
## [Personalizar a palavra-passe](active-directory-b2c-reference-password-complexity.md)
## [Personalização de idioma](active-directory-b2c-reference-language-customization.md)
## [Utilizar a API de relatórios](active-directory-b2c-reference-usage-reporting-api.md)
## [Migrar utilizadores](active-directory-b2c-user-migration.md)
## [Migrar as identidades sociais](active-directory-b2c-social-migration.md)

# Estrutura de Experiência de Identidade
## [Políticas personalizadas](active-directory-b2c-overview-custom.md)
## Guias
### [Introdução](active-directory-b2c-get-started-custom.md)
### [Pacote de iniciante](active-directory-b2c-reference-custom-policies-understanding-contents.md)
### Configurar fornecedores de OIDC/OAUTH
#### [Azure AD](active-directory-b2c-setup-aad-custom.md)
#### [Conta Microsoft](active-directory-b2c-custom-setup-msa-idp.md)
#### [Google+](active-directory-b2c-custom-setup-goog-idp.md)
#### [LinkedIn](active-directory-b2c-custom-setup-li-idp.md)
#### [Twitter](active-directory-b2c-custom-setup-twitter-idp.md)
### Configurar fornecedores de SAML
#### [AD FS](active-directory-b2c-custom-setup-adfs2016-idp.md)
#### [Salesforce](active-directory-b2c-setup-sf-app-custom.md)
### Integrar APIs RESTful
#### [Validar a intervenção do utilizador](active-directory-b2c-rest-api-validation-custom.md)
#### [Obter afirmações adicionais](active-directory-b2c-rest-api-step-custom.md)
#### [Adicionar a sua própria API RESTful](active-directory-b2c-custom-rest-api-netfw.md)
#### [Proteger APIs RESTful com autenticação básica](active-directory-b2c-custom-rest-api-netfw-secure-basic.md)
#### [Proteger APIs RESTful com autenticação de certificados](active-directory-b2c-custom-rest-api-netfw-secure-cert.md)
### Fluxos do utilizador
#### [Configurar intervenção do utilizador](active-directory-b2c-configure-signup-self-asserted-custom.md)
#### [Atributos personalizados](active-directory-b2c-create-custom-attributes-profile-edit-custom.md)
#### [Personalizar a IU](active-directory-b2c-ui-customization-custom.md)
#### [Personalizar a IU dinamicamente](active-directory-b2c-ui-customization-custom-dynamic.md)
#### [Personalizar tokens](active-directory-b2c-reference-manage-sso-and-token-configuration.md)
#### [Complexidade de palavras-passe](active-directory-b2c-reference-password-complexity-custom.md)
#### [Alteração de palavras-passe](active-directory-b2c-reference-password-change-custom.md)
#### [Manter a sessão iniciada](active-directory-b2c-reference-kmsi-custom.md)
### Resolução de problemas e Registos de Eventos
#### [Criar eventos personalizados com o Application Insights](active-directory-b2c-custom-guide-eventlogger-appins.md)
#### [Recolher registos com o Application Insights](active-directory-b2c-troubleshoot-custom.md)
## Referência
### [Notas de versão](active-directory-b2c-developer-notes-custom.md)
### [Fornecedores de sessões SSO](active-directory-b2c-reference-sso-custom.md)
### [Personalização de idioma](active-directory-b2c-language-customization-custom.md)
### [Definição da Arquitetura Fidedigna](active-directory-b2c-reference-trustframeworks-defined-ief-custom.md) 

# Referência
## [Exemplos de código](https://azure.microsoft.com/en-us/resources/samples/?service=active-directory-b2c)
## Glossário
### [Tipos de aplicações](active-directory-b2c-apps.md)
### [Protocolos de autenticação](active-directory-b2c-reference-protocols.md)
### [Tokens](active-directory-b2c-reference-tokens.md)
## [Disponibilidade regional e residência dos dados](active-directory-b2c-reference-tenant-type.md)
## [Ativar faturação](active-directory-b2c-how-to-enable-billing.md)
## [Gestão de ameaças](active-directory-b2c-reference-threat-management.md)
## [Problemas ao criar um diretório](active-directory-b2c-support-create-directory.md)
## [Aplicação de extensões](active-directory-b2c-reference-extensions-app.md)

# Relacionado
## [Azure Active Directory](../active-directory/active-directory-whatis.md)
## [Multi-Factor Authentication](../multi-factor-authentication/multi-factor-authentication.md)

# Recursos
## [Fórum de comentários do Azure AD B2C](https://feedback.azure.com/forums/169401-azure-active-directory/category/160596-b2c)
## [Mapa do Azure](https://azure.microsoft.com/roadmap/?category=security-identity)
## [Perguntas mais frequentes](active-directory-b2c-faqs.md)
## [Preços](https://azure.microsoft.com/pricing/details/active-directory-b2c/)
## [Calculadora de preços](https://azure.microsoft.com/pricing/calculator/)
## [Atualizações de serviço](https://azure.microsoft.com/updates/?product=active-directory-b2c)
## [Stack Overflow](https://stackoverflow.com/questions/tagged/azure-ad-b2c)
## [Suporte](active-directory-b2c-support.md)
## [Vídeos](https://azure.microsoft.com/documentation/videos/index/?services=active-directory-b2c)
