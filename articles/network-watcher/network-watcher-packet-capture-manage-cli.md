...
Executar o `az network watcher packet-capture show-status` cmdlet, obtém o estado de uma captura de pacotes em execução, ou foi concluída.

```azurecli
az network watcher packet-capture show-status --name packetCaptureName --location {networkWatcherLocation}
```

O exemplo seguinte é o resultado do `az network watcher packet-capture show-status` cmdlet. O exemplo seguinte é quando a captura está parada, com um StopReason TimeExceeded. 
