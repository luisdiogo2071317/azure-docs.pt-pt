...
| Armazenamento de Blobs do Azure | Sim | Utilize {date} e {time} tokens. o do seu campos de eventos no padrão de caminho. Escolha o formato de data, tais como aaaa/MM/DD, DD/MM/AAAA, MM-DD-AAAA. HH é utilizada para o formato de hora. Como parte do [pré-visualização](https://aka.ms/ASAPreview), saída de blob pode ser particionada por um atributo de evento personalizado único {fieldname}. | Segue-se de que a entrada partições para [consultas totalmente paralelizáveis](stream-analytics-scale-jobs.md). | 
