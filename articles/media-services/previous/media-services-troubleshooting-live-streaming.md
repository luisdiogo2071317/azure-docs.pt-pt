...
* **Problema em potencial**: não é possível encontrar o codificador de registos que podem ajudar a depurar problemas.
  
  * **Telestream Wirecast**: normalmente pode encontrar registos em C:\Users\{username} \AppData\Roaming\Wirecast\ 
  * **Em direto elementar**: pode encontrar tem ligações para os registos no portal de gestão. Clique em **estatísticas**, em seguida, **registos**. Sobre o **ficheiros de registo** página, verá uma lista de registos para todos os itens de LiveEvent; selecione a sua sessão atual de correspondência. 
  * **Flash Media Live Encoder**: É possível encontrar o **diretório de registo...**  ao navegar para o **registo de codificação** separador.
...
* **Problema em potencial**: O codificador que está a ser utilizado não deinterlace automaticamente. 
...
* **Problema em potencial**: canal de codificação do Azure não foi reposta corretamente. 
    ...
    **Passos de resolução de problemas**: Certifique-se de que o codificador já não é enviar para o AMS, parar e reiniciar o canal. Depois de executar novamente, tente ligar seu codificador com as novas definições. Se isso ainda não corrigir o problema, tente criar um novo canal inteiramente, às vezes, canais podem tornar-se danificado após várias tentativas falhadas.  
* **Problema em potencial**: tamanho de GOP a ou as definições de quadro-chave não estejam otimizadas. 
  
    **Passos de resolução de problemas**: intervalo de tamanho ou quadro chave GOP recomendado é de dois segundos. Alguns codificadores calculam esta definição no número de quadros, enquanto outros utilizam segundos. Por exemplo: na saída de 30 fps, o tamanho de GOP seria 60 quadros, que é equivalente a 2 segundos.  
* **Problema em potencial**: fechadas portas estão a bloquear o fluxo. 
