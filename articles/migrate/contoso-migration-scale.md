...
**Servidor de configuração** | Com base na estimativa da Contoso da replicação desses 100 = 200 VMs em conjunto e o [requisitos de dimensionamento do servidor de configuração](https://docs.microsoft.com/azure/site-recovery/site-recovery-plan-capacity-vmware.md#size-recommendations-for-the-configuration-server), Contoso prever necessidades de uma máquina do servidor de configuração da seguinte forma:<br/><br/> CPU: 16 vCPUs (2 sockets * 8 núcleos @ 2,5 GHz)<br/><br/> Memória: 32 GB<br/><br/> Disco de cache: 1 TB<br/><br/> Taxa de alteração de dados: 1 TB para 2 TB.<br/><br/> Além dos requisitos de dimensionamento Contoso tem de certificar-se de que o servidor de configuração é ideal localizado na mesma rede e segmento de LAN que as VMs que serão migrados.
