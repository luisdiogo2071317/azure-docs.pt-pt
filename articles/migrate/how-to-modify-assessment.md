...
2. Modifique as propriedades de acordo com a tabela abaixo:
    ...
    **Tipo de armazenamento** | Pode especificar o tipo de discos que quer alocar no Azure. Esta propriedade é aplicável quando o critério de dimensionamento é como o dimensionamento no local. Pode especificar o tipo de disco de destino como discos geridos Premium ou discos geridos Standard. Para o dimensionamento com base no desempenho, a recomendação de disco é feita automaticamente com base nos dados de desempenho das VMs. Note que o Azure Migrate só suporta discos geridos para avaliação de migrações. | O valor predefinido é discos Premium gerido (critério de dimensionamento como *tal como no local dimensionamento*).
    **Instâncias reservadas** |  Também pode especificar se tiver [reservado instâncias](https://azure.microsoft.com/pricing/reserved-vm-instances/) no Azure e Azure migrar o serão estimar os custos em conformidade. Reservado instâncias não são aplicáveis a regiões sovereign (Azure Government, Datacenters e China) e são aplicáveis apenas a oferta pay as you go no Azure migrar. | O valor predefinido para esta propriedade é instâncias de 3 anos reservados.
    ...
    **Tempo de atividade VM** | Se as suas VMs não vão ser executados 24x7 no Azure, pode especificar a duração (número de dias, por mês) e o número de horas por dia para a qual seria possível execução e as estimativas de custo serão efetuadas em conformidade. | O valor predefinido é de 31 dias, por mês e 24 horas por dia.
