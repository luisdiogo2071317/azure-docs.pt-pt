...
Neste artigo, Contoso migra a sua aplicação SmartHotel VM de front-end para uma VM do Azure com o serviço Azure Site Recovery. Contoso também migra a base de dados de aplicação para a instância gerida da base de dados SQL do Azure.
...
Pode transferir a aplicação de SmartHotel de exemplo utilizada neste artigo [GitHub](https://github.com/Microsoft/SmartHotel360).
...
- A aplicação está em camadas em duas VMs (**WEBVM** e **SQLVM**) e está localizada num anfitrião ESXi do VMware versão 6.5 (**contosohost1.contoso.com**). 
- O ambiente do VMware é gerido pelo vCenter Server 6.5 (**vcenter.contoso.com**) em execução numa VM.
- Contoso migra a base de dados de aplicação (**SmartHotelDB**) para uma instância de gerida de base de dados do Azure SQL.
- Contoso migra as VMs do VMware no local para uma VM do Azure.
- A Contoso tiver um datacenter no local (**contoso-datacenter**) e um controlador de domínio no local (**contosodc1**).
...
Contoso irá migrar os escalões web e os dados da sua aplicação SmartHotel para o Azure, concluindo estes passos:
...
> * **Passo 2: Preparar o serviço de gestão de base de dados**: Contoso tem de registar o fornecedor de migração de base de dados, criar uma instância e, em seguida, crie um projeto de serviço de gestão de base de dados. Contoso também tem de configurar uma assinatura de acesso partilhado (SAS) Uniform Resource Identifier (URI) para o serviço de gestão de base de dados. Um URI de SAS fornece acesso delegado a recursos na conta de armazenamento da Contoso, para que a Contoso pode conceder permissões limitadas a objetos de armazenamento. Contoso configura um URI de SAS, para que o serviço de gestão de base de dados pode aceder ao contentor de conta de armazenamento para o qual o serviço carrega os ficheiros de cópia de segurança do SQL Server.
...
> * **Passo 6: Migrar a base de dados com o serviço de gestão de base de dados**: Contoso migra a base de dados.
...
Contoso configura a rede virtual da seguinte forma: 

1. Contoso cria uma nova rede virtual (**VNET-SQLMI-EU2**) na região E.U.A. Leste 2 primária. Ele adiciona a rede virtual para o **ContosoNetworkingRG** grupo de recursos.
2. Contoso atribui um espaço de endereços de 10.235.0.0/24. Contoso garante que o intervalo não se sobrepõe quaisquer outras redes na sua empresa.
2. Contoso adiciona duas sub-redes para a rede:
...
6. Depois da rede virtual e sub-redes são implementadas, o Contoso elementos redes da seguinte forma:
...
7. Contoso define as definições de DNS personalizadas. Pontos DNS para controladores de domínio do Azure da Contoso. O DNS do Azure é secundário. Os controladores de domínio do Azure de Contoso estão localizados da seguinte forma:
...
- Não existem não existem custos adicionais para a criação de tabelas de rotas no Microsoft Azure.
 ...
 Para configurar o encaminhamento:

1. Contoso cria uma tabela UDR. Contoso cria a tabela de rotas na **ContosoNetworkingRG** grupo de recursos.

    ![Tabela de rotas](media/contoso-migration-rehost-vm-sql-managed-instance/mi-route-table.png)

2. Para atender aos requisitos de instância gerida, depois da tabela de rotas (**MIRouteTable**) é implementado, a Contoso adiciona uma rota que tem um prefixo de endereço de 0.0.0.0/0. O **tipo de próximo salto** opção estiver definida como **Internet**.

    ![Prefixo da tabela de rota](media/contoso-migration-rehost-vm-sql-managed-instance/mi-route-table-prefix.png)
    
3. Contoso associa a tabela de rotas com o **SQLMI-DB-EUS2** sub-rede (na **VNET-SQLMI-EUS2** rede). 
...
Agora, a Contoso pode aprovisionar uma instância gerida do SQL da base de dados:

1. Uma vez que a instância gerida atende uma aplicação de negócio, Contoso implementa a instância gerida na região E.U.A. Leste 2 primária da empresa. Contoso adiciona a instância gerida para o **ContosoRG** grupo de recursos.
2. Contoso seleciona um escalão de preço, a computação de tamanho e o armazenamento para a instância. Saiba mais sobre [preços de instância gerida](https://azure.microsoft.com/pricing/details/sql-database/managed/).
...
## <a name="step-2-prepare-the-database-management-service"></a>Passo 2: Preparar o serviço de gestão de base de dados

Para preparar o serviço de gestão de base de dados, a Contoso precisa de fazer algumas coisas:

- Registe o fornecedor de serviço de gestão de base de dados no Azure.
- Fornece o serviço de gestão de base de dados com acesso ao armazenamento do Azure para carregar os ficheiros de cópia de segurança que são utilizados para migrar uma base de dados. Para fornecer acesso ao armazenamento do Azure, a Contoso cria um contentor de armazenamento de Blobs do Azure. Contoso gera um URI de SAS para o contentor de armazenamento de Blobs. 
- Crie um projeto de serviço de gestão de base de dados.

Em seguida, a Contoso é concluído os passos seguintes:

1. Contoso regista o fornecedor de migração de base de dados na sua subscrição.
    ![Serviço de gestão de base de dados - registar](media/contoso-migration-rehost-vm-sql-managed-instance/dms-subscription.png)

2. Contoso cria um contentor de armazenamento de Blobs. Contoso gera um URI de SAS para que o serviço de gestão de base de dados pode acessá-lo.

    ![Serviço de gestão de base de dados: gerar um URI de SAS](media/contoso-migration-rehost-vm-sql-managed-instance/dms-sas.png)

3. Contoso cria uma instância de serviço de gestão de base de dados. 

    ![Serviço de gestão de base de dados - criar instância](media/contoso-migration-rehost-vm-sql-managed-instance/dms-instance.png)

4. Contoso coloca a instância do serviço de gestão de base de dados no **PROD-DC-EUS2** sub-rede da **VNET-PROD-DC-EUS2** rede virtual.
    - Contoso coloca o serviço de gestão de base de dados existe porque o serviço tem de estar numa rede virtual que pode aceder a VM do servidor de SQL no local através de um gateway VPN.
    - O **VNET-PROD-EUS2** é configurar o peering **VNET-HUB-EUS2** e está autorizado a utilizar gateways remotos. O **utilizar gateways remotos** opção garante que o serviço de gestão de base de dados pode comunicar conforme necessário.

        ![Serviço de gestão de base de dados - configurar rede](media/contoso-migration-rehost-vm-sql-managed-instance/dms-network.png)

*Precisa de mais ajuda?*

- Saiba como [configurar o serviço de gestão de base de dados](https://docs.microsoft.com/azure/dms/quickstart-create-data-migration-service-portal).
...
Contoso define a recuperação de Site da seguinte forma:

1. Uma vez que a VM é um front-end de publicação na web ' para a aplicação de SmartHotel, Contoso efetua a ativação pós-falha da VM para a sua rede de produção existente (**VNET-PROD-EUS2**) e a sub-rede (**PROD-FE-EUS2**). A rede e sub-rede estão localizados na região E.U.A. Leste 2 primária. Configure a rede de Contoso quando ele [implementado a infraestrutura do Azure](contoso-migration-infrastructure.md).
2. Contoso cria uma conta de armazenamento (**contosovmsacc20180528**). A Contoso utiliza uma conta para fins gerais. Contoso seleciona o armazenamento standard e replicação de armazenamento localmente redundante.

    ![Recuperação de sites - criar conta de armazenamento](media/contoso-migration-rehost-vm-sql-managed-instance/asr-storage.png)

3. Com a conta de armazenamento e de rede no local, a Contoso cria um cofre (**ContosoMigrationVault**). Contoso coloca o Cofre na **ContosoFailoverRG** grupo de recursos na região E.U.A. Leste 2 primária.
...
Para preparar o VMware para o Site Recovery, Contoso têm de concluir estas tarefas:
...
Contoso configura a conta ao concluir estas tarefas:
...
- Contoso especifica esta conta quando ele configura a replicação na consola do Azure.
...
Após a ativação pós-falha para o Azure, a Contoso quer ser capaz de ligar as VMs replicadas no Azure. Para ligar as VMs replicadas no Azure, o Contoso têm de concluir algumas tarefas na VM no local antes da migração: 

1. Para ter acesso através da internet, a Contoso permite RDP na VM no local antes da ativação pós-falha. Contoso garante que as regras TCP e UDP estão adicionadas para o **pública** perfil e que o RDP é permitido na **Firewall do Windows** > **aplicações permitidas** para todos os perfis .
2. Para obter acesso através de VPN de site a site da Contoso, a Contoso permite RDP na máquina no local. Contoso permite que o RDP na **Firewall do Windows** > **aplicações e funcionalidades permitidas** para **domínio e privadas** redes.
3. Contoso define a política de SAN do sistema operacional na VM no local para **OnlineAll**.

A Contoso também precisa verificar esses itens quando é executada uma ativação pós-falha:

- Não deve haver nenhuma atualização do Windows pendentes na VM quando é acionada uma ativação pós-falha. Se as atualizações do Windows estão pendentes, Contoso não consigo iniciar sessão para a máquina virtual até que a atualização esteja concluída.
- Após a ativação pós-falha, deverá verificar Contoso **diagnósticos de arranque** para ver uma captura de ecrã da VM. Se a Contoso não é possível ver o diagnóstico de arranque, deve verificar que a VM está em execução e, em seguida, reveja [sugestões de resolução de problemas](http://social.technet.microsoft.com/wiki/contents/articles/31666.troubleshooting-remote-desktop-connection-after-failover-using-asr.aspx).

## <a name="step-5-replicate-the-on-premises-vms-to-azure-by-using-site-recovery"></a>Passo 5: Replicar as VMs no local para o Azure utilizando a recuperação de Site

Antes de executar uma migração para o Azure, a Contoso precisa de configurar a replicação e ative a replicação para a sua VM no local.

### <a name="set-a-replication-goal"></a>Definir um objetivo de replicação

1. No cofre, sob o nome do cofre (**ContosoVMVault**), Contoso define um objetivo de replicação (**introdução** > **Site Recovery**  >   **Preparar infraestrutura**).
2. Contoso Especifica que as suas máquinas estão localizados no local, que são VMs do VMware, e que a Contoso pretende replicar para o Azure.
...
Para continuar, a Contoso tem de confirmar que concluiu o planejamento da implantação. Para confirmar, Contoso seleciona **Sim, já o fiz**. Nesta implementação, a Contoso está migrando apenas uma única VM, para que ele não precisa de planeamento da implementação.

### <a name="set-up-the-source-environment"></a>Configurar o ambiente de origem

Agora, a Contoso precisa de configurar o seu ambiente de origem. Para configurar o seu ambiente de origem, a Contoso transfere um modelo OVF. Contoso utiliza o modelo para implementar o servidor de configuração e seus componentes associados como de elevada disponibilidade, a VM de VMware no local. Os componentes no servidor incluem:
...
- O servidor de processos também instala o serviço de mobilidade nas VMs que a Contoso pretende replicar. O servidor de processo efetua a deteção automática de VMs de VMware no local.
...
1. Contoso transfere o modelo OVF a partir do portal do Azure (**preparar infraestrutura** > **origem** > **servidor de configuração**).
    
    ![Adicionar um servidor de configuração](./media/contoso-migration-rehost-vm-sql-managed-instance/add-cs.png)

2. Contoso importa o modelo para o VMware, para criar e implementar a VM.

    ![Implementar modelo OVF](./media/contoso-migration-rehost-vm-sql-managed-instance/vcenter-wizard.png)

3.  Quando Contoso fica na VM pela primeira vez, a VM entrar numa experiência de instalação do Windows Server 2016. Contoso aceita o contrato de licença e introduz uma palavra-passe de administrador.
4. Quando a instalação estiver concluída, Contoso inicia sessão na VM como administrador. A primeira vez que a Contoso inicia sessão na VM, a ferramenta de configuração do Azure Site Recovery é executado automaticamente.
5. Na ferramenta de configuração de recuperação de Site, o Contoso insere um nome a utilizar para registar o servidor de configuração no cofre.
6. A ferramenta verifica a ligação da VM para o Azure. Depois da ligação é estabelecida, a Contoso seleciona **iniciar sessão** para iniciar sessão para a subscrição do Azure. As credenciais têm de ter acesso ao Cofre no qual o servidor de configuração está registado. 

    ![Registar o servidor de configuração](./media/contoso-migration-rehost-vm-sql-managed-instance/config-server-register2.png)

7. A ferramenta executa algumas tarefas de configuração e, em seguida, é reiniciada. Contoso inicia sessão na máquina novamente. O Assistente de gestão de servidor de configuração é iniciado automaticamente.
8. No assistente, a Contoso seleciona o NIC para receber o tráfego de replicação. Não é possível alterar esta definição depois de estar configurada.
9. Contoso seleciona a subscrição, grupo de recursos e cofre dos serviços de recuperação no qual pretende registar o servidor de configuração.

    ![Selecione o Cofre dos serviços de recuperação](./media/contoso-migration-rehost-vm-sql-managed-instance/cswiz1.png)

10. Contoso transfere e instala o servidor MySQL e o VMWare PowerCLI. Em seguida, a Contoso valida as definições do servidor.
11. Após a validação, a Contoso aciona o FQDN ou endereço IP do vCenter Server instância ou vSphere no anfitrião. Contoso deixa a porta predefinida e insere um nome a apresentar para a instância de servidor vCenter no Azure.
12. Contoso tem de especificar a conta que criou anteriormente para que o Site Recovery pode detetar automaticamente as VMs de VMware que estão disponíveis para replicação. 
13. Contoso introduz credenciais, para que o serviço de mobilidade é instalado automaticamente quando a replicação está ativada. Para máquinas Windows, a conta tem permissões de administrador local nas VMs. 

    ![Configurar servidor vCenter](./media/contoso-migration-rehost-vm-sql-managed-instance/cswiz2.png)

7. Quando o registo estiver concluído, no portal do Azure, Contoso verifica novamente que o servidor de configuração e o servidor VMware estão listados no **origem** página no cofre. Deteção pode demorar 15 minutos ou mais. 
8. Recuperação de sites liga-se aos servidores VMware com as definições especificadas. Site Recovery Deteta as VMs.

### <a name="set-up-the-target"></a>Configurar o destino

Agora, a Contoso precisa de configurar o ambiente de replicação de destino:

1. Na **preparar infraestrutura** > **destino**, Contoso seleciona as definições de destino.
2. Recuperação de sites verifica que existe uma conta de armazenamento e rede de destino especificado.

### <a name="create-a-replication-policy"></a>Criar uma política de replicação

Quando a origem e destino estão configurados, a Contoso cria uma política de replicação e associa a política com o servidor de configuração:

1. Na **preparar infraestrutura** > **definições de replicação** > **política de replicação** >  **criar e Associar**, Contoso cria o **ContosoMigrationPolicy** política.
2. A Contoso utiliza as predefinições:
...
Agora, a Contoso pode começar WebVM a replicar.

1. Na **replicar aplicação** > **origem** > **replicar**, Contoso seleciona as definições de origem.
2. Contoso indica que quer ativar máquinas virtuais, seleciona a instância do servidor vCenter e define o servidor de configuração.

    ![Ativar a replicação - origem](./media/contoso-migration-rehost-vm-sql-managed-instance/enable-replication1.png)
 
3. Contoso Especifica as definições de destino, incluindo o grupo de recursos e a rede em que a VM do Azure estarão localizada após a ativação pós-falha. Contoso Especifica a conta de armazenamento no qual serão armazenados os dados replicados.

    ![Ativar a replicação - destino](./media/contoso-migration-rehost-vm-sql-managed-instance/enable-replication2.png)

4. Contoso seleciona **WebVM** para replicação. Site Recovery instala o serviço de mobilidade em cada VM quando a replicação está ativada. 

    ![Ativar a replicação – selecione a VM](./media/contoso-migration-rehost-vm-sql-managed-instance/enable-replication3.png)

5. Contoso verifica que a política de replicação correta está selecionada. Em seguida, ele permite a replicação para **WEBVM**. Contoso controla o progresso de replicação em **tarefas**. Depois de a tarefa **Finalizar Proteção** ser executada, a máquina está preparada para ativação pós-falha.
6. Na **Essentials** no portal do Azure, Contoso pode ver a estrutura para as VMs que estão a replicar para o Azure:
...
## <a name="step-6-migrate-the-database-by-using-the-database-management-service"></a>Passo 6: Migrar a base de dados com o serviço de gestão de base de dados

A Contoso precisa de criar um projeto de serviço de gestão de base de dados e, em seguida, migre a base de dados.

### <a name="create-a-database-management-service-project"></a>Criar um projeto de serviço de gestão de base de dados

1. Contoso cria um projeto de serviço de gestão de base de dados. Contoso seleciona os **SQL Server** tipo de servidor de origem. Contoso seleciona **instância gerida da base de dados SQL do Azure** como destino.

     ![Serviço de gestão de base de dados - novo projeto de migração](./media/contoso-migration-rehost-vm-sql-managed-instance/dms-project.png)

2. É aberto o Assistente de migração.

### <a name="migrate-the-database"></a>Migrar a base de dados 

1. No Assistente de migração, a Contoso Especifica a VM de origem onde está localizada a base de dados no local. Contoso insere as credenciais para aceder a base de dados.

    ![Serviço de gestão de base de dados - detalhes da origem](./media/contoso-migration-rehost-vm-sql-managed-instance/dms-wizard-source.png)

2. Contoso seleciona a base de dados para migrar (**SmartHotel.Registration**):

    ![Serviço de gestão de base de dados - bases de dados de selecionar origem](./media/contoso-migration-rehost-vm-sql-managed-instance/dms-wizard-sourcedb.png)

3. Para o destino, a Contoso introduz o nome da instância gerida no Azure. Contoso introduz as credenciais de acesso para a instância gerida.

    ![Serviço de gestão de base de dados - detalhes do destino](./media/contoso-migration-rehost-vm-sql-managed-instance/dms-target-details.png)

4. Na **nova atividade** > **executar migração**, Contoso Especifica as definições para executar a migração:
    - Credenciais de origem e de destino.
    - A base de dados para migrar.
    - A rede partilhar esse Contoso criada na VM no local. O serviço de gestão de base de dados leva as cópias de segurança de origem para esta partilha. 
    ...
    - O URI de SAS que fornece o serviço de gestão de base de dados com o acesso ao contentor de conta de armazenamento para o qual o serviço carrega os ficheiros de cópia de segurança para a migração.

        ![Serviço de gestão de base de dados - configurar definições de migração](./media/contoso-migration-rehost-vm-sql-managed-instance/dms-migration-settings.png)

5. Contoso guarda a migração e, em seguida, executa-lo.
6. Na **descrição geral**, Contoso monitoriza o estado de migração.

    ![Serviço de gestão de base de dados - Monitor](./media/contoso-migration-rehost-vm-sql-managed-instance/dms-monitor1.png)

7. Quando a migração estiver concluída, a Contoso verifica que os bancos de dados de destino existem na instância gerida.

    ![Serviço de gestão de base de dados – verificar a migração de base de dados](./media/contoso-migration-rehost-vm-sql-managed-instance/dms-monitor2.png)

## <a name="step-7-migrate-the-vm-by-using-site-recovery"></a>Passo 7: Migrar a VM com o Site Recovery

Contoso é executada uma ativação pós-falha de teste rápida e, em seguida, efetua a migração de VM.

### <a name="run-a-test-failover"></a>Executar uma ativação pós-falha de teste

Antes de migrar uma ativação pós-falha de teste WEBVM, ajuda a garantir que tudo funciona conforme esperado. Contoso concluir os passos seguintes:

1. Contoso executa uma ativação pós-falha de teste para o ponto mais recente no tempo (**processado mais recentemente**).
2. Contoso seleciona **encerrar a máquina antes de iniciar a ativação pós-falha**. Com esta opção selecionada, o Site Recovery tenta encerrar a VM de origem antes de ele aciona a ativação pós-falha. Ativação pós-falha continua, mesmo que o encerramento falhe. 
...
3. Quando a ativação pós-falha estiver concluída, a réplica é apresentada VM do Azure no portal do Azure. Contoso verifica se tudo o que está a funcionar corretamente: a VM tem um tamanho adequado, está ligado à rede correta e está em execução. 
4. Depois de verificar a ativação pós-falha de teste, a Contoso limpa a ativação pós-falha. Em seguida, regista e guarda todas as observações. 

### <a name="migrate-the-vm"></a>Migrar a VM

1. Depois de verificar que a ativação pós-falha de teste funcionou como esperado, a Contoso cria um plano de recuperação para a migração. Contoso adiciona WEBVM ao plano de:

     ![Criar plano de recuperação - selecionar itens](./media/contoso-migration-rehost-vm-sql-managed-instance/recovery-plan.png)

2. Contoso é executada uma ativação pós-falha no plano. Contoso seleciona o ponto de recuperação mais recente. Contoso Especifica que o Site Recovery deve tentar encerrar a VM no local antes de ele aciona a ativação pós-falha.

    ![Ativação pós-falha](./media/contoso-migration-rehost-vm-sql-managed-instance/failover1.png)

3. Após a ativação pós-falha, a Contoso verifica que a VM do Azure é apresentado conforme esperado no portal do Azure.

   ![Detalhes do plano de recuperação](./media/contoso-migration-rehost-vm-sql-managed-instance/failover2.png)

4. Depois de verificar a VM no Azure, a Contoso conclui a migração para concluir o processo de migração, parar a replicação para a VM e parar a faturação do Site Recovery para a VM.
...
Como a etapa final do processo de migração, a Contoso atualiza a cadeia de ligação da aplicação para apontar para a base de dados migrada está em execução na instância gerida da Contoso.

1. No portal do Azure, o Contoso localiza a cadeia de ligação selecionando **configurações** > **cadeias de ligação**.

    ![Cadeias de ligação](./media/contoso-migration-rehost-vm-sql-managed-instance/failover4.png)  

2. Contoso atualiza a cadeia de caracteres com o nome de utilizador e palavra-passe da instância gerida do SQL da base de dados.
3. Depois da cadeia de caracteres é configurada, o Contoso substitui a cadeia de ligação atual no arquivo Web. config do seu aplicativo.
4. Depois de atualizar o arquivo e salvá-lo, Contoso reinicia o IIS no WEBVM. Para reiniciar o IIS, a Contoso é executado `IISRESET /RESTART` numa janela de linha de comandos.
...
6. Neste momento, Contoso pode encerrar no local a máquina SQLVM. A migração foi concluída.
...
Com a migração completa, a aplicação de SmartHotel está em execução numa VM do Azure e a base de dados SmartHotel está disponível na instância gerida do Azure SQL da base de dados.  
...
- A equipe de segurança analisa os grupos de segurança de rede que são utilizados para controlar o acesso para a VM. Os grupos ajudam a garantir que apenas o tráfego que tem permissão para a aplicação de segurança de rede pode passar.
...
- A equipe de segurança permite que a deteção de ameaças na instância gerida. Deteção de ameaças envia um alerta para o sistema de suporte técnico da Contoso segurança equipe/serviço para abrir um pedido, desde que seja detetada uma ameaça. Saiba mais sobre [deteção para a instância gerida de ameaças](https://docs.microsoft.com/azure/sql-database/sql-database-managed-instance-threat-detection).
...
Contoso efetua uma cópia de segurança dos dados no WEBVM com o serviço de cópia de segurança do Azure. Saiba mais sobre [Azure Backup](https://docs.microsoft.com/azure/backup/backup-introduction-to-azure-backup?toc=%2fazure%2fvirtual-machines%2flinux%2ftoc.json).
...
Neste artigo, Contoso realojar a aplicação de SmartHotel no Azure ao migrar a aplicação de VM de front-end para o Azure com o serviço de recuperação de sites. Contoso migra a base de dados no local para uma instância de gerida de base de dados do Azure SQL com o serviço de gestão de base de dados do Azure.

## <a name="next-steps"></a>Passos Seguintes

No próximo artigo da série, a Contoso [realojar a aplicação de SmartHotel em VMs do Azure](contoso-migration-rehost-vm.md) com o serviço Azure Site Recovery.
