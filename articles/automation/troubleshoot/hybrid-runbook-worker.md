...
O runbook está suspenso pouco depois tentar executá-lo três vezes. Existem condições, que podem interromper o runbook a conclusão com êxito e a mensagem de erro relacionados não inclui qualquer informação adicional que indica por que.
...
A lista seguinte mostra os processos que estão a utilizar uma função de trabalho do Runbook de híbrida do Linux. Todos estão localizados no `/var/opt/microsoft/omsagent/state/automationworker/` diretório.

* **OMS.Conf** -este é o processo do Gestor de trabalho, isso é iniciado diretamente a partir de DSC.

* **Worker.Conf** -este processo é o processo de trabalho híbrida de registado automaticamente, é iniciada pelo Gestor de trabalho. Este processo é utilizado pela gestão de atualizações e é transparente ao usuário. Este processo não está presente, se a solução de gestão de atualizações não estiver ativada na máquina.
...
Se o serviço Windows do Microsoft Monitoring Agent não está em execução, isso impede que a função de trabalho de Runbook híbrida comunicar com a automatização do Azure.
...
Na **aplicativo e o Gestor de registos de serviços** log de eventos, verá eventos 4502 e EventMessage contendo **hybridagent**com a descrição seguinte: *o certificado apresentado pelo serviço \<wsid\>. oms.opinsights.azure.com não foi emitido por uma autoridade de certificação utilizada para os serviços da Microsoft. Contacte o administrador de rede para ver se estão a executar um proxy que intercepte a comunicação TLS/SSL. O artigo KB3126513 tem informações adicionais de resolução de problemas de conectividade.*

#### <a name="cause"></a>Causa

Isto pode dever-se a firewall, proxy ou de rede, bloquear a comunicação com o Microsoft Azure. Certifique-se de que o computador tem acesso de *. Azure-Automation.NET saída nas portas 443.

#### <a name="resolution"></a>Resolução

Os registos são armazenados localmente em cada função de trabalho híbrida em C:\ProgramData\Microsoft\System Center\Orchestrator\7.2\SMA\Sandboxes. Pode verificar se há qualquer aviso ou eventos de erro escritos para o **aplicativos e serviços Logs\Microsoft-SMA\Operations** e **aplicativo e o Gestor de registos de serviços** do registo de eventos que indica uma conectividade ou outro problema que afeta a inclusão da função para a automatização do Azure ou o problema ao efetuar as operações normais.

[Runbook de saída e mensagens](../automation-runbook-output-and-messages.md) são enviados para a automatização do Azure da híbrida executar as funções de trabalho tal como tarefas de runbook na cloud. Também pode ativar os fluxos de verboso e progresso da mesma forma que faria para outros runbooks.
