...
O Projeto de DevOps do Azure apresenta uma experiência simplificada que cria os recursos do Azure e configura um pipeline de integração contínua (CI) e entrega contínua (CD) para a sua aplicação Python no Visual Studio Team Services (VSTS), que é a solução de DevOps da Microsoft para o Azure.  
...
1. Crie uma **nova** conta VSTS ou escolha uma conta **existente**.  Escolha um **nome** para o seu projeto VSTS.  Selecione a sua **subscrição do Azure**, a **localização** e escolha um **nome** para a sua aplicação.  Quando tiver terminado, escolha **Concluído**.
    ...
    ![Introduzir informações do VSTS](_img/azure-devops-project-python/vstsazureinfo.png)
...
O projeto de DevOps do Azure configura automaticamente uma compilação de CI e um acionador de versão.  Agora, está pronto para colaborar com uma equipa na sua aplicação Python, com um processo de CI/CD que implementa automaticamente o seu trabalho mais recente no seu site.
