...
O Projeto de DevOps do Azure apresenta uma experiência simplificada que cria os recursos do Azure e configura um pipeline de integração contínua (CI) e de entrega contínua (CD) para a sua aplicação PHP nos Serviços DevOps do Azure.  
...
O Projeto de DevOps do Azure configura automaticamente um acionador de lançamento e de compilação de CI.  Agora está pronto para colaborar com uma equipa na sua aplicação PHP, com um processo de CI/CD que implementa automaticamente o seu trabalho mais recente no seu site.
