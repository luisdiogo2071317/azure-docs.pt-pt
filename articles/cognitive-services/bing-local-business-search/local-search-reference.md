...
|----------|-----------|----------|--------------|
|<a name="count" />Contagem|O número de resuts para retornar, começando com o índice especificado pelo `offset` parâmetro.|Cadeia|Não|   
|<a name="localCategories" />localCategories|Lista de opções que definem a pesquisa por categoria de negócios.  Consulte [Local de negócios de categorias de pesquisa](local-categories.md)|Cadeia|Não|  
|<a name="mkt" />mkt|O mercado de onde os resultados provêm. <br /><br />Para obter uma lista de possíveis valores de mercado, consulte [códigos de mercado](#market-codes).<br /><br /> **Nota:** a API de pesquisa de negócios locais atualmente suporta apenas en--nos mercado e a linguagem.<br /><br />|Cadeia|Sim|
|<a name="offset"/>deslocamento|O índice inicial de resultados especificados pelo `count` parâmetro.|Número inteiro|Não|  
|<a name="query" />p|Termo de pesquisa do usuário.|Cadeia|Não|  
...
|Nome|Valor|Tipo|  
|----------|-----------|----------|  
|tipo|Dica de tipo, que pode ser definida como um dos seguintes:<br /><br /><ul><li>Hotel</li><li>LocalBusiness<br /></li><li>Restaurante</ul><li>|Cadeia|  
|Endereço|O endereço postal de onde está localizada a entidade.|[PostalAddress](#postaladdress)|  
|entityPresentationInfo|Obter informações adicionais sobre a entidade como sugestões que pode utilizar para determinar o tipo da entidade. Por exemplo, se é um restaurante ou hotel. O `entityScenario` campo é definido como ListItem.|[entityPresentationInfo](#entitypresentationinfo)|  
|nome|Nome da entidade.|Cadeia|  
|Telefone|Número de telefone da entidade.|Cadeia|  
|url|O URL para o site da entidade.<br /><br /> Utilizar este URL, juntamente com o nome da entidade para criar uma hiperligação que quando clicado leva o usuário ao site da entidade.|Cadeia|  
|webSearchUrl|O URL para o resultado da pesquisa do Bing para este local.|Cadeia| 
