...
O `Billing` definição de configuração especifica o ponto final do URI do recurso de imagem digitalizada no Azure utilizada para controlar informações de faturas para o contentor. Tem de especificar um valor para esta definição de configuração e o valor tem de ser um ponto final válido URI para um recurso de imagem digitalizada no Azure.
