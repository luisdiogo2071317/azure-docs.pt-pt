...
Crie um novo projeto do Python através do seu editor ou IDE favorito. Em seguida, copie este fragmento de código para o seu projeto num ficheiro com o nome `translate-text.py`.
...
Atualmente, está disponível um ponto final para Tradução de Texto e está definido como `base_url`. `path` define a rota `translate` e identifica que queremos utilizar a versão 3 da API.
...
Além da tradução de texto, saiba como utilizar a API de Texto do Microsoft Translator para:
