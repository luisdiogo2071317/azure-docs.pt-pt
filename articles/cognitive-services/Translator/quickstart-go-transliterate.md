---
title: 'Início Rápido: converter script de texto, Go – API de Texto do Microsoft Translator'
...
ms.locfileid: "52993295"
---
# <a name="quickstart-use-the-translator-text-api-to-transliterate-text-using-go"></a>Início rápido: Utilizar a API de texto do Translator a transcrição de texto com Go
...
Crie um novo projeto de Go usando o seu IDE ou editor favorito. Em seguida, copie este fragmento de código para o seu projeto num ficheiro com o nome `transliterate-text.go`.
...
Vamos criar uma função a transcrição de texto. Esta função irá usar um único argumento, a chave de subscrição de texto do Translator.
...
Em seguida, vamos construir o URL. O URL é criado com o `Parse()` e `Query()` métodos. Observará que os parâmetros são adicionados com o `Add()` método. Neste exemplo, estamos a transliterar de japonês para o alfabeto latino.

Copie este código para o `transliterate` função.
...
Em seguida, criar uma estrutura anônima para o corpo do pedido e codificá-lo como JSON com `json.Marshal()`. Adicione este código para o `transliterate` função.
...
Adicione este código para o `transliterate` função para descodificar a resposta JSON e, em seguida, formate e imprima o resultado.
