...
Vamos ver se conseguimos obter mais detalhes na recolha de desempenho que gerou este alerta e verificar a nossa suspeita de que os problemas são causados por esse processo de cópia de segurança.  Altere o intervalo de tempo para **6 horas**.  Em seguida, clique em **Favoritos** e desloque para baixo para as pesquisas guardadas do **Mapa de Serviço**.  Criámos estas consultas especificamente para esta análise.  Clique em **Cinco processos principais pela CPU para acmetomcat**.
...
Neste caso, podemos ver que o processo de cópia de segurança está a consumir consistentemente cerca de 60% da CPU do servidor da aplicação.  É óbvio que este novo processo é responsável pelo nosso problema de desempenho.  A nossa solução seria obviamente remover este novo software de cópia de segurança do servidor de aplicação.  Podíamos utilizar a Configuração de Estado Pretendido (DSC) gerida através da Automatização do Azure, para definir políticas que se certificam que este processo nunca é executado nestes sistemas críticos.
