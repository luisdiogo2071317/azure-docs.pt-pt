...
- Filtragem da solução só se aplica a soluções fornecidas pela Microsoft.  Não é aplicável às soluções [criado por si próprio ou parceiros](monitoring-solutions-creating.md).
...
- Saiba mais sobre as soluções de gestão, incluindo as soluções que estão disponíveis para instalação no seu ambiente no [soluções de gestão de adicionar o Azure Log Analytics para a área de trabalho](monitoring-solutions.md).
