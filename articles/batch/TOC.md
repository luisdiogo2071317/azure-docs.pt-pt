
# [Documentação do Batch](index.yml)

# Descrição geral
## [O que é o Azure Batch?](batch-technical-overview.md)

# Inícios rápidos
## [Executar uma tarefa do Batch - CLI](quick-create-cli.md)
## [Executar uma tarefa do Batch - Portal](quick-create-portal.md)
## [Executar uma tarefa do Batch - .NET](quick-run-dotnet.md)
## [Executar uma tarefa do Batch - Python](quick-run-python.md)


# Tutoriais

## [Processamento de ficheiros em paralelo - .NET](tutorial-parallel-dotnet.md)
## [Processamento de ficheiros em paralelo - Python](tutorial-parallel-python.md)
## [Composição de cenas com o Arnold](tutorial-rendering-cli.md)
## [Simulação de R em paralelo](tutorial-r-doazureparallel.md)

# Amostras
## Exemplos de código
### [Exemplos de código do Azure](https://azure.microsoft.com/resources/samples/?service=batch)
### [Repositório de exemplos do Batch](https://github.com/Azure/azure-batch-samples)
## [CLI do Azure](cli-samples.md)

# Conceitos
## [Funcionalidades de programador](batch-api-basics.md)
## [APIs e ferramentas](batch-apis-tools.md)
## [Quotas e limites](batch-quota-limit.md)

# Guias de procedimentos
## Gerir contas do Batch
### [Gerir contas do Batch com o portal](batch-account-create-portal.md)
### [Gerir contas do Batch com a Gestão de Lotes .NET](batch-management-dotnet.md)

## Autenticar com o Azure AD
### [Azure AD com o serviço Batch](batch-aad-auth.md)
### [Azure AD com a Gestão do Batch](batch-aad-auth-management.md)

## [Criar pacotes de aplicações](batch-application-packages.md)

## Criar e gerir agrupamentos
### [Nós de computação com dimensionamento automático](batch-automatic-scaling.md)
### [VMs de computação intensiva](batch-pool-compute-intensive-sizes.md)
### [Configurar o acesso aos nós de computação](pool-endpoint-configuration.md)
### [Criar um conjunto com uma imagem personalizada](batch-custom-images.md)
### [Criar um conjunto numa rede virtual](batch-virtual-network.md)
### [Nós de computação do Linux](batch-linux-nodes.md)
### [VMs de baixa prioridade](batch-low-pri-vms.md)

## Gerir trabalhos e tarefas
### [Tarefas de preparação e conclusão de trabalhos](batch-job-prep-release.md)
### [Tarefas de nós simultâneas](batch-parallel-node-tasks.md)
### [Contagem de tarefas por estado](batch-get-task-counts.md)
### [Dependências de tarefas](batch-task-dependencies.md)
### [Contas de utilizador para executar tarefas](batch-user-accounts.md)
### [Executar tarefas de Lote sem código](batch-cli-templates.md)


## [Manter saída de trabalhos e tarefas](batch-task-output.md)
### [Manter a saída com a API de Batch](batch-task-output-files.md)
### [Manter a saída com a biblioteca Convenções de Ficheiros](batch-task-output-file-conventions.md)

## Executar cargas de trabalho
### [Tarefas MPI](batch-mpi.md)
### [Cargas de trabalho do contentor](batch-docker-container-workloads.md)
### [Composição](batch-rendering-service.md)
### [Processamento de dados com o Batch e o Data Factory](../data-factory/v1/data-factory-data-processing-using-batch.md?toc=%2fazure%2fbatch%2ftoc.json)

## Monitorizar recursos
### [Registo de diagnósticos](batch-diagnostics.md)
### [Consultas de lista eficientes](batch-efficient-list-queries.md)

## Utilizar ferramentas para programadores
### [Utilizar o Batch .NET](batch-dotnet-get-started.md)
### [Utilizar o Batch Python](batch-python-tutorial.md)
### [Utilizar o Batch Node.js](batch-nodejs-get-started.md)
### [Utilizar cmdlets do PowerShell](batch-powershell-cmdlets-get-started.md)
### [Utilizar a CLI do Azure](batch-cli-get-started.md)

# Referência

## [CLI do Azure](/cli/azure/batch)
## [Azure PowerShell](/powershell/module/azurerm.batch)
## [.NET](/dotnet/api/overview/azure/batch)
## [Java](/java/api/overview/azure/batch)
## [Node.js](/javascript/api/overview/azure/batch)
## [Python](/python/api/overview/azure/batch)
## REST
### [Serviço Batch](/rest/api/batchservice)
### [Gestão de Batch](/rest/api/batchmanagement/)
## [Batch Analytics](batch-analytics.md)
### [Evento de criação de conjunto](batch-pool-create-event.md)
### [Evento de início de eliminação de conjunto](batch-pool-delete-start-event.md)
### [Evento de conclusão de eliminação de conjunto](batch-pool-delete-complete-event.md)
### [Evento de início de redimensionamento de conjunto](batch-pool-resize-start-event.md)
### [Evento de conclusão de redimensionamento de conjunto](batch-pool-resize-complete-event.md)
### [Evento de início de tarefa](batch-task-start-event.md)
### [Evento de conclusão de tarefa](batch-task-complete-event.md)
### [Evento de falha de tarefa](batch-task-fail-event.md)
## [Variáveis de ambiente do nó de computação do Batch](batch-compute-node-environment-variables.md)

# Recursos
## [Mapa do Azure](https://azure.microsoft.com/roadmap/)
## [Fórum do Batch](https://social.msdn.microsoft.com/Forums/en-us/home?forum=azurebatch)
## [Batch Shipyard](https://github.com/Azure/batch-shipyard)
## [Blogue](https://blogs.technet.microsoft.com/windowshpc/)
## [Soluções HPC no Azure](../virtual-machines/linux/high-performance-computing.md?toc=%2fazure%2fbatch%2ftoc.json)
## [Preços](https://azure.microsoft.com/pricing/details/batch/)
## [Calculadora de preços](https://azure.microsoft.com/pricing/calculator/)
## [Modelo do Resource Manager](/azure/templates/microsoft.batch/batchaccounts)
## [Atualizações de serviço](https://azure.microsoft.com/updates/?product=batch&updatetype=&platform=)
## [Stack Overflow](http://stackoverflow.com/questions/tagged/azure-batch)
## [Vídeos](https://azure.microsoft.com/documentation/videos/index/?services=batch)


