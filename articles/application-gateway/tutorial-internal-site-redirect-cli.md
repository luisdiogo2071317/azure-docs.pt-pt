...
As regras são processadas pela ordem em que são criados e o tráfego é direcionado através da primeira regra que corresponde ao URL enviados para o gateway de aplicação. A regra básica predefinida que foi criada não é necessária neste tutorial. Neste exemplo, criar duas novas regras com o nome *contosoComRule* e *contosoOrgRule* e eliminar a regra predefinida que foi criada.  Pode adicionar regras utilizando [criar regra de gateway de aplicação de rede az](/cli/azure/application-gateway#az_network_application_gateway_rule_create).
...
Neste exemplo, crie um conjunto de dimensionamento de máquina virtual que suporta o conjunto predefinido de back-end que foi criado. O conjunto de dimensionamento que criar com o nome *myvmss* e contém duas instâncias de máquina virtual no qual instala NGINX.
