...
* Não parâmetros de saída: Utiliza `ConditionalValue<T>` para retornar um booleano e um valor em vez de parâmetros de saída. `ConditionalValue<T>` é como `Nullable<T>` mas não exige que T é um struct.
