...
Enviar dados de monitorização e diagnóstico para o Application Insights (IA) pode ser feito como parte da configuração WAD. Se optar por utilizar a ia para análise de eventos e visualização, leia [como configurar um coletor de IA](service-fabric-diagnostics-event-analysis-appinsights.md#add-the-ai-sink-to-the-resource-manager-template) como parte da sua "WadCfg".
