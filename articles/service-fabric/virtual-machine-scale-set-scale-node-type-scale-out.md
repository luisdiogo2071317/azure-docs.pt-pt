...
> [!WARNING]
> Antes de tentar este procedimento num cluster de produção, recomendamos que Estude os modelos de exemplo e verifique se o processo em relação a um cluster de teste. O cluster também está disponível para um período de tempo. NÃO pode efetuar alterações para vários VMSS declarado como o NodeType mesmo no parrallel; terá de efetuar as operações de implementação separada para aplicar as alterações para cada VMSS NodeType individualmente.
