...
3. **[Criar um utilizador de teste Bynder](#create-a-bynder-test-user)**  - para ter um equivalente da Eduarda Almeida na Bynder que está ligado à representação de utilizador do Azure AD.
...
3. Sobre o **Bynder domínio e URLs** secção, execute os seguintes passos, se desejar configurar a aplicação no **IDP** iniciada pelo modo:
...
7. Para configurar o início de sessão único num **Bynder** lado, terá de enviar o transferido **XML de metadados** para [equipa de suporte de Bynder](https://www.bynder.com/en/support/). Se definir esta definição para que a ligação de SAML SSO definidas corretamente em ambos os lados.
