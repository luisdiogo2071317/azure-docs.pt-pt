...
3. **[Criar um utilizador de teste OnTrack](#create-an-ontrack-test-user)**  - para ter um equivalente da Eduarda Almeida na OnTrack que está ligado à representação de utilizador do Azure AD.
...
3. Sobre o **OnTrack domínio e URLs** secção, execute os seguintes passos:
...
8. Para configurar o início de sessão único num **OnTrack** lado, terá de enviar o transferido **XML de metadados** para [equipa de suporte de OnTrack](mailto:CustomerService@insigniagroup.com). Se definir esta definição para que a ligação de SAML SSO definidas corretamente em ambos os lados.
