...
3. **[Criar um utilizador de teste do conector de redes de Meta](#create-a-meta-networks-connector-test-user)**  - para ter um equivalente da Eduarda Almeida no conector de redes de metadados que está ligado à representação de utilizador do Azure AD.
...
3. Sobre o **Meta redes conector do domínio e URLs** secção, execute os seguintes passos, se desejar configurar a aplicação no **IDP** iniciada pelo modo:
...
5. Conector de redes de meta espera que as asserções SAML num formato específico. Configure as seguintes declarações para esta aplicação. Pode gerir os valores destes atributos do **atributos de utilizador** secção na página de integração de aplicações. Captura de ecrã seguinte mostra um exemplo disso.
...
8. Sobre o **configuração do conector de redes de Meta** secção, clique em **configurar conector de redes de Meta** para abrir **configurar início de sessão** janela. Cópia a **ID de entidade de SAML e SAML único início de sessão no URL do serviço** partir o **secção de referência rápida.**
...
10. Abra um novo separador no seu browser e inicie sessão sua conta de administrador do conector de redes de Meta.
...
15. Sobre o **gerais** separador. Execute os seguintes passos:

    ![Configurar o início de sessão único](./media/metanetworksconnector-tutorial/configure5.png)

    a. Na **URL fornecedor de identidade única Sign-On**, cole a **SAML único início de sessão no URL do serviço** valor que copiou do portal do Azure.

    b. Na **emissor do fornecedor de identidade**, cole a **ID de entidade de SAML** valor que copiou do portal do Azure.
