...
4. Na caixa de pesquisa, escreva **SafetyNet**, selecione **SafetyNet** no painel de resultados, em seguida, clique em **Add** botão para adicionar a aplicação.
...
3. **[Criar um utilizador de teste do SafetyNet](#create-a-safetynet-test-user)**  - para ter um equivalente da Eduarda Almeida na SafetyNet está ligada à representação de utilizador do Azure AD.
...
3. Sobre o **SafetyNet domínio e URLs** secção, execute os seguintes passos, se desejar configurar a aplicação no **IDP** iniciada pelo modo:
...
7. Para configurar o início de sessão único num **SafetyNet** lado, terá de enviar o **Url de metadados de Federação de aplicação** para [equipa de suporte do SafetyNet](mailto:dev@predictivesolutions.com). Se definir esta definição para que a ligação de SAML SSO definidas corretamente em ambos os lados.
