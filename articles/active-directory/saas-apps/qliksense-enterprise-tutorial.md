...
3. Sobre o **Qlik sentido empresarial domínio e URLs** secção, execute os seguintes passos:
...
6. Prepare o ficheiro XML de metadados de Federação para que pode carregar que para o servidor Qlik Sense.
...
7. Navegue para o Qlik sentido Qlik Management Console (QMC) como um utilizador que pode criar as configurações de virtual proxy.

8. No QMC, clique nas **Virtual Proxies** item de menu.

    ![QlikSense][qs6]

9. Na parte inferior do ecrã, clique nas **criar novo** botão.

    ![QlikSense][qs7]

10. É apresentado o ecrã de edição de proxy Virtual.  No lado direito da tela é um menu para tornar as opções de configuração visível.

    ![QlikSense][qs9]

11. Com a opção de menu de identificação marcada, introduza as informações de identificação para a configuração de proxy virtual do Azure.
...
12. Clique na opção de menu de autenticação para o tornar visível.  É apresentado o ecrã de autenticação.
...
13. Clique nas **BALANCEAMENTO de carga** opção de menu para torná-lo visível.  É apresentado o ecrã de balanceamento de carga.

    ![QlikSense][qs11]

14. Clique nas **adicionar novo nó de servidor** botão, o nó de motor selecione ou nós Qlik Sense enviará sessões para fins de balanceamento de carga e, clique no **Add** botão.

    ![QlikSense][qs12]

15. Clique na opção de menu avançado para torná-lo visível. É apresentado o ecrã avançado.
...
16. Clique nas **aplicar** botão.

    ![QlikSense][qs14]

17. Clique em OK para aceitar a mensagem de aviso que indica a proxies ligados para o proxy virtual serão reiniciados.

    ![QlikSense][qs15]

18. No lado direito da tela, é apresentado o menu de itens associados.  Clique nas **Proxies** opção de menu.

    ![QlikSense][qs16]

19. É apresentado o ecrã de proxy.  Clique nas **Link** na parte inferior para ligar um proxy para o proxy virtual.

    ![QlikSense][qs17]

20. Selecione o nó de proxy que irá suportar esta ligação de virtual proxy e clique nas **Link** botão.  Depois de associar, o proxy será listado sob proxies associados.

    ![QlikSense][qs18]
  
    ![QlikSense][qs19]

21. Após cerca de cinco a dez segundos, será apresentada a mensagem de QMC atualizar.  Clique nas **QMC atualizar** botão.

    ![QlikSense][qs20]

22. Quando atualiza o QMC, clique nas **Virtual proxies** item de menu. A nova entrada de virtual proxy SAML está listada na tabela na tela.  Único clique na entrada virtual proxy.

    ![QlikSense][qs51]

23. Na parte inferior do ecrã, irá ativar o botão de metadados de transferir SP.  Clique nas **SP transferir metadados** botão para guardar os metadados para um ficheiro.

    ![QlikSense][qs52]

24. Abra o ficheiro de metadados de sp.  Observe a **entityID** entrada e o **AssertionConsumerService** entrada.  Estes valores são equivalentes para o **identificador**, **iniciar sessão no URL** e o **URL de resposta** na configuração da aplicação do Azure AD. Cole esses valores no **Qlik sentido empresarial domínio e URLs** secção na configuração da aplicação do Azure AD Se eles não correspondem, em seguida, deve substituir o Assistente de configuração de aplicações do Azure AD.
...
2. Na lista de aplicações, selecione **Qlik sentido empresarial**.
