...
3. **[Criar um utilizador de teste Mobile Xpense](#create-a-mobile-xpense-test-user)**  - para ter um equivalente da Eduarda Almeida no Mobile Xpense que está ligado à representação de utilizador do Azure AD.
...
3. Sobre o **Mobile Xpense domínio e URLs** secção, execute os seguintes passos, se desejar configurar a aplicação no modo de IDP iniciado:
...
7. Para configurar o início de sessão único num **Mobile Xpense** lado, terá de enviar o transferido **XML de metadados** para [equipa de suporte do Mobile Xpense](http://www.mobilexpense.net/contact). Se definir esta definição para que a ligação de SAML SSO definidas corretamente em ambos os lados.
