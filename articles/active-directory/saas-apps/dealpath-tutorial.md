...
3. **[Criar um utilizador de teste Dealpath](#create-a-dealpath-test-user)**  - para ter um equivalente da Eduarda Almeida na Dealpath que está ligado à representação de utilizador do Azure AD.
...
3. Sobre o **Dealpath domínio e URLs** secção, execute os seguintes passos:
...
6. Sobre o **Dealpath configuração** secção, clique em **configurar Dealpath** para abrir **configurar início de sessão** janela. Cópia a **ID de entidade de SAML e SAML único início de sessão no URL do serviço** partir o **secção de referência rápida.**

    ![Configuração de Dealpath](./media/dealpath-tutorial/tutorial_dealpath_configure.png) 

7. Numa janela do browser web diferente, início de sessão para Dealpath como administrador.

8. No canto superior direito, clique em **ferramentas de administração** e navegue até à **integrações**, em seguida, em **SAML 2.0 autenticação** secção clique **definições de atualização**:

    ![Configuração de Dealpath](./media/dealpath-tutorial/tutorial_dealpath_admin.png)

9. Na **configurar a autenticação SAML 2.0** página, execute os seguintes passos:
