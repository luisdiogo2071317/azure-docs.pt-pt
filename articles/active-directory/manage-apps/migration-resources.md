...
| [Ferramenta: Script de preparação de migração de serviços Federação do Active Directory](https://aka.ms/migrateapps/adfsscript) | Este é um script pode executar no seu servidor de serviços de Federação do Active Directory (AD FS) no local para determinar a preparação dos aplicativos para a migração para o Azure AD.|
