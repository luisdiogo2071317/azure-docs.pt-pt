---
title: Resolver problemas de sincronização de hash de palavra-passe com a sincronização do Azure AD Connect | Microsoft Docs
...
Para implementação de ligar do Azure Active Directory (Azure AD) com a versão 1.1.614.0 ou depois, utilize a tarefa de resolução de problemas no Assistente para resolver problemas de sincronização de hash de palavra-passe:
...
7. No sub menu, selecione **sincronização de hash de palavra-passe não funciona em todas as**.
...
* Para cada existente conector no local do Active Directory (que corresponde a uma floresta do Active Directory existente):
   ...
   * Procura palavra-passe hash eventos de heartbeat da sincronização nos registos de eventos de aplicações do Windows.
   ...
   * Para cada domínio do Active Directory no conector do Active Directory no local:
      ...
      * Valida se as contas de serviços de domínio do Active Directory (AD DS) utilizadas pelo conector do Active Directory no local tem o nome de utilizador correto, palavra-passe e as permissões necessárias para a sincronização de hash de palavra-passe.

O diagrama seguinte ilustra os resultados do cmdlet para uma topologia do domínio único, no local do Active Directory:

![Resultados de diagnóstico para a sincronização de hash de palavra-passe](./media/active-directory-aadconnectsync-troubleshoot-password-hash-synchronization/phsglobalgeneral.png)
...
![sincronização de hash de palavra-passe não está ativada](./media/active-directory-aadconnectsync-troubleshoot-password-hash-synchronization/phsglobaldisabled.png)
...
![Servidor do Azure AD Connect está no modo de teste](./media/active-directory-aadconnectsync-troubleshoot-password-hash-synchronization/phsglobalstaging.png)

#### <a name="no-password-hash-synchronization-heartbeat-events"></a>Não há eventos de heartbeat de sincronização de hash de palavra-passe
...
![Não existem heart de sincronização de hash de palavra-passe beat eventos](./media/active-directory-aadconnectsync-troubleshoot-password-hash-synchronization/phsglobalnoheartbeat.png)
...
![Credencial incorreto](./media/active-directory-aadconnectsync-troubleshoot-password-hash-synchronization/phsglobalaccountincorrectpermission.png)
...
![Credencial incorreto](./media/active-directory-aadconnectsync-troubleshoot-password-hash-synchronization/phsglobalaccountincorrectcredential.png)
...
![Resultados de diagnóstico para a sincronização de hash de palavra-passe - único objeto](./media/active-directory-aadconnectsync-troubleshoot-password-hash-synchronization/phssingleobjectgeneral.png)
...
sincronização de hash de palavra-passe para esta conta do Active Directory no local falha porque não existe nenhum objecto correspondente no inquilino do Azure AD. É devolvido o erro seguinte:

![Objeto do AD do Azure está em falta](./media/active-directory-aadconnectsync-troubleshoot-password-hash-synchronization/phssingleobjectnotexported.png)
...
![Palavra-passe temporária não seja exportado](./media/active-directory-aadconnectsync-troubleshoot-password-hash-synchronization/phssingleobjecttemporarypassword.png)
...
Por predefinição, o Azure AD Connect armazena os resultados de tentativas de sincronização de hash de palavra-passe durante sete dias. Se não existirem resultados disponíveis para o objeto selecionado do Active Directory, é devolvido o aviso seguinte:

![Resultados de diagnóstico para um único objeto - não existem históricos de sincronização de palavra-passe](./media/active-directory-aadconnectsync-troubleshoot-password-hash-synchronization/phssingleobjectnohistory.png)
...
2. Execute o script no [obter o estado das definições de sincronização de palavra-passe](#get-the-status-of-password-sync-settings) secção. Proporciona uma descrição geral da configuração da sincronização de palavra-passe.  
    ...
    ![Saída do script do PowerShell a partir das definições de sincronização de palavra-passe](./media/active-directory-aadconnectsync-troubleshoot-password-hash-synchronization/psverifyconfig.png)  
...
4. Aceda a **ligar à floresta do Active Directory**.  
    ...
    ![Conta utilizada pelo conector do Active Directory](./media/active-directory-aadconnectsync-troubleshoot-password-hash-synchronization/connectoraccount.png)  
...
6. Os controladores de domínio está acessível pelo Azure AD Connect? Se o servidor de ligação não é possível ligar a todos os controladores de domínio, configure **utilizar apenas o controlador de domínio preferencial**.  
    ...
    ![Controlador de domínio utilizada pelo conector do Active Directory](./media/active-directory-aadconnectsync-troubleshoot-password-hash-synchronization/preferreddc.png)  
...
Pode facilmente resolver problemas de sincronização de hash de palavra-passe, verificando o estado de um objeto.
...
1. No **computadores e utilizadores do Active Directory**, procure o utilizador e, em seguida, certifique-se de que o **o utilizador deve alterar a palavra-passe no próximo início de sessão** caixa de verificação está desmarcada.  
    ...
    ![Active Directory produtivas palavras-passe](./media/active-directory-aadconnectsync-troubleshoot-password-hash-synchronization/adprodpassword.png)  
...
2. Se a palavra-passe procura correta no Active Directory, siga o utilizador no motor de sincronização. Seguindo o utilizador do Active Directory no local ao Azure AD, pode ver se encontra um erro de descritivo no objeto.
    ...
    ![Pesquisa de utilizador no espaço de conector com DN](./media/active-directory-aadconnectsync-troubleshoot-password-hash-synchronization/searchcs.png)  
    ...
    ![Detalhes de registo do objecto](./media/active-directory-aadconnectsync-troubleshoot-password-hash-synchronization/csobjectlog.png)  
    ...
    ![Informações de linhagem sobre um utilizador](./media/active-directory-aadconnectsync-troubleshoot-password-hash-synchronization/cspasswordsync.png)  

    i. Clique em **as propriedades do objeto de Metaverso** para apresentar uma lista de atributos de utilizador.  

    ![Informações de Metaverso](./media/active-directory-aadconnectsync-troubleshoot-password-hash-synchronization/mvpasswordsync.png)  
    ...
    ![Informações de Metaverso](./media/active-directory-aadconnectsync-troubleshoot-password-hash-synchronization/mvconnectors.png)  
    ...
    ![Caixa de diálogo de propriedades do objeto de espaço de conector](./media/active-directory-aadconnectsync-troubleshoot-password-hash-synchronization/cspasswordsync2.png)  
...
* [Implementar a sincronização de hash de palavra-passe com a sincronização do Azure AD Connect](active-directory-aadconnectsync-implement-password-hash-synchronization.md)
