...
O [agendador](active-directory-aadconnectsync-feature-scheduler.md) é executado a cada 30 minutos por predefinição. Certifique-se de que não está a iniciar enquanto estiver a efetuar alterações e as novas regras de resolução de problemas. Para desativar temporariamente o scheduler, inicie o PowerShell e execute `Set-ADSyncScheduler -SyncCycleEnabled $false`.

![Desativar o scheduler](./media/active-directory-aadconnectsync-change-the-configuration/schedulerdisable.png)  
...
2. Sobre o **Descrição** página, introduza o seguinte:  
   ...
   * **Tipo de objeto Metaverso/sistema ligado**: selecione **usuário** e **pessoa**, respectivamente.
...
**Pré-visualização e uma sincronização completa num único objeto**  
   ...
   ![Confirmar pré-visualização](./media/active-directory-aadconnectsync-change-the-configuration/commitpreview.png)  
...
Para informações gerais sobre o cenário para nesta seção, consulte [controlar o processo de fluxo de atributo](active-directory-aadconnectsync-understanding-declarative-provisioning.md#control-the-attribute-flow-process).

Existem duas formas de fluxo não de um atributo. A primeira é utilizando o Assistente de instalação para [remover atributos selecionados](active-directory-aadconnect-get-started-custom.md#azure-ad-app-and-attribute-filtering). Esta opção funciona se nunca tiver sincronizado o atributo antes. No entanto, se tiver começado a sincronizar este atributo e removê-lo mais tarde com esta funcionalidade, o pára de motor de sincronização gerenciamento atributo e os valores existentes é deixado no Azure AD.
...
![Atributos de extensão incorreto](./media/active-directory-aadconnectsync-change-the-configuration/badextensionattribute.png)
...
4. Certifique-se de que as alterações pretendidas estão prestes a ser exportado ao pesquisar o espaço conector.
  ![Eliminação em etapas](./media/active-directory-aadconnectsync-change-the-configuration/deletetobeexported.png)
...
- Em alternativa, pode derivar o valor para o atributo UserType de outras propriedades. Por exemplo, pretende sincronizar todos os utilizadores como **convidado** se suas instalações atributo userPrincipalName do AD acaba com parte do domínio *@partners.fabrikam123.org*. 
...
### <a name="step-2-add-the-source-attribute-to-the-on-premises-ad-connector-schema"></a>Passo 2: Adicionar o atributo de origem com o local do esquema de conector AD
...
![Adicionar o atributo de origem para o local esquema de conector AD](./media/active-directory-aadconnectsync-change-the-configuration/usertype1.png)
...
![Adicionar o atributo de origem para o esquema do conector do Azure AD](./media/active-directory-aadconnectsync-change-the-configuration/usertype2.png)

### <a name="step-4-create-an-inbound-synchronization-rule-to-flow-the-attribute-value-from-on-premises-active-directory"></a>Passo 4: Criar uma regra de sincronização de entrada para o valor do atributo do Active Directory no local do fluxo
...
6. Vá para o **transformação** separador e implementar a regra de transformação pretendida. Por exemplo, se tiver designado um não utilizado no local atributo de AD (por exemplo, extensionAttribute1) como o atributo de origem para o UserType, pode implementar um fluxo de atributos direto:
    ...
    Noutro exemplo, pretende derivar o valor para o atributo UserType de outras propriedades. Por exemplo, pretende sincronizar todos os utilizadores como convidado se suas instalações atributo userPrincipalName do AD acaba com parte do domínio *@partners.fabrikam123.org*. Pode implementar uma expressão como esta:
...
5. Certifique-se **exportações pendentes** para o Azure AD:
   ...
   2. Na **procurar espaço conector** caixa de diálogo pop-up:
      ...
      - Selecione todas as três caixas de verificação: **Add**, **modificar**, e **eliminar**.
