...
Ambas as opções pedem aos utilizadores para se registar para Azure multi-factor Authentication a primeira vez que iniciar sessão depois de ativassem os requisitos. Ambas as opções também funcionam com o configuráveis [definições de multi-factor Authentication do Azure](howto-mfa-mfasettings.md).
...
Não mova os utilizadores diretamente para o *imposto* estado. Se o fizer, aplicações não baseadas no browser deixam de funcionar porque o utilizador não passou pelo registo do MFA do Azure e obter um [palavra-passe de aplicação](howto-mfa-mfasettings.md#app-passwords). 
...
- Obter sugestões sobre o [melhores práticas para acesso condicional](../active-directory-conditional-access-best-practices.md).

- Gerir definições de multi-factor Authentication do Azure para [os seus utilizadores e os respetivos dispositivos](howto-mfa-userdevicesettings.md).
