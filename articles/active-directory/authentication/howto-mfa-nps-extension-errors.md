...
| **CLIENT_CERT_INSTALL_ERROR** | Poderá existir um problema com a forma como o certificado de cliente foi instalado ou associado ao seu inquilino. Siga as instruções em [a extensão de NPS do MFA de resolução de problemas](howto-mfa-nps-extension.md#troubleshooting) para investigar problemas de certificado de cliente. |
| **ESTS_TOKEN_ERROR** | Siga as instruções em [a extensão de NPS do MFA de resolução de problemas](howto-mfa-nps-extension.md#troubleshooting) para investigar o certificado de cliente e ADAL token problemas. |
...
| **REGISTRY_CONFIG_ERROR** | Uma chave está em falta no registo para a aplicação, o que poderá ser porque a [script do PowerShell](howto-mfa-nps-extension.md#install-the-nps-extension) não foi executado após a instalação. A mensagem de erro deverá incluir a chave em falta. Certifique-se de que a chave em HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\AzureMfa. |
