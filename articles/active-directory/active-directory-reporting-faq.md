...
**R:** pode consultar os registos de até 1 milhões (se não utilizar o operador top, que ordena o registo por mais recentes). Se utilizar o operador "top", pode consultar cópias de segurança para os registos de 500K. Pode encontrar consultas de exemplo sobre como utilizar a API [aqui](active-directory-reporting-api-getting-started.md).
...
**R:** Sim, Identity Protection avalia risco para todos os fluxos de autenticação se interativo ou não interativo. No entanto, todos os relatórios apenas de inícios de sessão mostra apenas os interativos inícios de sessão.
...
**R:** para lhe fornecer uma aprofundadas sobre todos os os risco inícios de sessão no seu ambiente, "início de sessão com o risco adicional detetado" funciona como o marcador de posição para inícios de sessão para deteções que são exclusivos para subscritores do Azure AD Identity Protection.
