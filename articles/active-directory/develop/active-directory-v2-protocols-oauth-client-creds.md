...
Em mais típica *polvo de três OAuth*, um aplicativo cliente é concedido permissão para aceder a um recurso em nome de um utilizador específico. A permissão é delegada do usuário ao aplicativo, normalmente durante a [consentimento](active-directory-v2-scopes.md) processo. No entanto, no fluxo de credenciais de cliente, são concedidas permissões diretamente para o próprio aplicativo. Quando a apresenta aplicação que um token a um recurso, o recurso impõe essa aplicação em si tem autorização para efetuar uma ação e não que o usuário tem autorização.

## <a name="protocol-diagram"></a>Diagrama de protocolo
O fluxo de credenciais de cliente inteira é semelhante ao seguinte diagrama. Nós descrevemos, cada um dos passos neste artigo.
...
| --- | --- | --- |
| inquilino |Necessário |O inquilino do diretório que pretende pedir permissão do. Isso pode ser no formato de nome amigável ou de GUID. Se não sabe qual inquilino pertence o usuário e pretende que os mesmos possam iniciar sessão com qualquer inquilino, utilize `common`. |
...
| Parâmetro | Descrição |
| --- | --- | --- |
| inquilino |O inquilino do diretório concedidas as permissões pedidos, no formato GUID de seu aplicativo. |
...
| Parâmetro | Descrição |
| --- | --- | --- |
| erro |Uma cadeia de caracteres de código de erro que pode usar para classificar tipos de erros, e o que pode utilizar para reagir a erros. |
...
| trace_id |Um identificador exclusivo para o pedido que pode ajudar com o diagnóstico. |
| correlation_id |Um identificador exclusivo para o pedido que pode ajudar com os diagnósticos em componentes. |
