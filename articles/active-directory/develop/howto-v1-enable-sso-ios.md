...
ms.locfileid: "39601977"
---
# <a name="how-to-enable-cross-app-sso-on-ios-using-adal"></a>Como ativar o SSO em vária aplicações em dispositivos iOS através da ADAL

Desde que único início de sessão (SSO) para que os utilizadores só precisam de introduzir as respetivas credenciais de uma vez e ter essas credenciais automaticamente trabalhar em aplicativos agora é esperado pelos clientes. A dificuldade de introduzir o nome de utilizador e a palavra-passe num ecrã pequeno, muitas vezes vezes combinados com um fator adicional (2FA) como uma chamada telefónica ou um código SMS, os resultados no insatisfação rápida, se um utilizador tem de fazê-lo mais do que uma vez para o seu produto.

Além disso, se aplicar uma plataforma de identidade que outros aplicativos podem utilizar como Accounts da Microsoft ou uma conta de trabalho do Office 365, o que os clientes exigem que as credenciais para ser disponibilizada para utilização em todas as suas aplicações, independentemente do fornecedor.

A plataforma do Microsoft Identity, juntamente com os nossos SDKs de identidade da Microsoft, faz todo esse trabalho difícil para e dá-lhe a capacidade de fascinarão seus usuários com o SSO no seu próprio conjunto de aplicativos ou, tal como acontece com a nossa capacidade de Mediador e o autenticador aplicações, através de todo o dispositivo.

Este passo a passo informa como configurar o SDK na sua aplicação para fornecer este benefício aos seus clientes.

Este passo a passo aplica-se a:
...
O documento anterior pressupõe que sabe como [aprovisionar as aplicações no portal do legado para o Azure Active Directory](active-directory-how-to-integrate.md) integrados a sua aplicação com o [SDK iOS do Microsoft Identity](https://github.com/AzureAD/azure-activedirectory-library-for-objc).
...
* Utilizador não é possível experiência de início de sessão único em todas as aplicações que utilizam um Microsoft Identity, apenas entre essas Identities Microsoft que tenha configurado a sua aplicação.
...
Esta é uma representação de como os SDKs de identidade da Microsoft trabalham com o armazenamento partilhado das suas aplicações para ativar o SSO:
...
Inícios de sessão assistida por mediador são experiências de início de sessão que ocorrem dentro da aplicação de Mediador e utilizam o armazenamento e a segurança de Mediador para partilhar as credenciais em todas as aplicações no dispositivo que se aplicam a plataforma do Microsoft Identity. Isso significa que seus aplicativos contam com o Mediador de sessão aos utilizadores. No iOS e Android, estes mediadores são fornecidos por meio de aplicativos que podem ser baixados que os clientes instalar de forma independente ou enviado para o dispositivo por uma empresa que gere o dispositivo para o utilizador. Um exemplo deste tipo de aplicativo é a aplicação Microsoft Authenticator em dispositivos iOS. No Windows, esta funcionalidade é fornecida por um selecionador de conta incorporado no sistema operativo, conhecido em termos técnicos, como o Mediador de autenticação Web.
...
Para não-mediador SSO assistido em todos os aplicativos os SDKs de identidade do Microsoft gerenciar muito da complexidade do SSO para. Isto inclui como localizar o usuário correto no cache e manter uma lista de utilizadores com sessão iniciada para que possa consultar.
...
4. Conte-nos SDKs de identidade do Microsoft keychain partilhado que querem que utilizar.
...
Deve estar imaginando como irá identificar aplicações diferentes para o serviço Microsoft Identity se ele usa a ID da mesma aplicação. A resposta é com o **URIs de redirecionamento**. Cada aplicativo pode ter vários URIs de redirecionamento registados no portal de inclusão. Cada aplicação no seu conjunto terá um URI de redirecionamento diferente. É um exemplo de como isso é exibido abaixo:
...
*Tenha em atenção que o formato destes URIs de redirecionamento é explicado abaixo. Pode usar qualquer URI de redirecionamento, a menos que pretenda suportar o mediador, caso em que eles devem ter um aspeto semelhante a acima*
...
Assim que tiver a elegibilidade de keychain ativada em cada uma das suas aplicações e está pronto para utilizar o SSO, conte-no SDK do Microsoft Identity keychain, utilize a seguinte definição na sua `ADAuthenticationSettings` com a definição seguinte:
...
```
O `AD_CREDENTIALS_AUTO` definição permitirá que o SDK de identidade da Microsoft tentar chamar para o mediador, `AD_CREDENTIALS_EMBEDDED` impedirá que o SDK de identidade da Microsoft chamada para o mediador.

#### <a name="step-2-registering-a-url-scheme"></a>Passo 2: Registar um esquema de URL

A plataforma do Microsoft Identity usa URLs para invocar o agente e, em seguida, retornar o controle para a sua aplicação. Para concluir essa ida e volta tem um esquema de URL registado para a sua aplicação que irá conhecer a plataforma do Microsoft Identity. Isso pode ser, além de outros esquemas de aplicação que pode tiver registado anteriormente com a sua aplicação.
...
Este URI de redirecionamento tem de ser especificado no seu registo de aplicação utilizando o [portal do Azure](https://portal.azure.com/). Para obter mais informações sobre o registo de aplicação do Azure AD, consulte [integração com o Azure Active Directory](active-directory-how-to-integrate.md).
