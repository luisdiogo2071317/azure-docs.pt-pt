...
|errorURL     |string         |`http://MyRegisteredAppError`         |O URL de erros encontrados numa aplicação. |
...
|optionalClaims     |  string       |     `null`    |    O [afirmações opcionais](active-directory-optional-claims.md) devolvido no token pelo serviço de token de segurança para esta aplicação específica.     |
...
|Home page     |  string       |`http://MyRegistererdApp`         |    O URL para a home page da aplicação.     |
...
|logoutUrl     |   string      |     `http://MyRegisteredAppLogout`    |   O URL a fim de sessão da aplicação.      |
...
|publicClient     |  boolean       |      `false`   | Especifica se uma aplicação é um cliente público (por exemplo, uma aplicação instalada em execução num dispositivo móvel). Predefinição é falsa.        |
...
|samlMetadataUrl    |string| `http://MyRegisteredAppSAMLMetadata` |O URL para os metadados SAML da aplicação.| 
