...
| `unique_name` | Cadeia | Apresente apenas nos tokens de v1.0. Fornece um valor de legível humano que identifica o assunto do token. Este valor não é garantido de ser exclusivo dentro de um inquilino e deve ser utilizado apenas para fins de exibição. |
...
As seguintes declarações serão incluídas na v1.0 tokens se aplicável, mas não estão incluídas nos tokens de v2.0 por predefinição. Se estiver a utilizar a versão 2.0 e precisar de uma dessas declarações, o pedido com através de [afirmações opcionais](active-directory-optional-claims.md).
