...
É o âmbito deste tópico para lhe fornecer o código de exemplo para o **API de auditoria**.
...
O script devolve o resultado do relatório de auditoria no formato JSON. Também cria um `audit.json` ficheiro com o mesmo resultado. Pode experimentar ao modificar o script devolver dados a partir de outros relatórios ou comente os formatos de saída que não é necessário.

## <a name="bash-script"></a>Scripts de bash
    #!/bin/bash
...
* Gostaria de personalizar os exemplos neste tópico? Veja o [auditoria do Azure Active Directory referência da API](active-directory-reporting-api-audit-reference.md). 
