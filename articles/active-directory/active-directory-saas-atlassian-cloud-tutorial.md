...
3. No **Atlassian nuvem domínio e os URLs** secção, execute os seguintes passos, se pretender configurar a aplicação no **IDP** iniciada modo:
    ...
    ![Domínio de nuvem Atlassian e URLs único informações de início de sessão](./media/active-directory-saas-atlassian-cloud-tutorial/tutorial_atlassiancloud_url.png)
    
    a. No **identificador** caixa de texto, escreva o URL:`https://auth.atlassian.com/saml/<unique ID>`
    
    b. No **URL de resposta** caixa de texto, escreva o URL:`https://auth.atlassian.com/login/callback?connection=saml-<unique ID>`
...
4. Verifique **Mostrar avançadas definições de URL** e executar o passo seguinte, se pretender configurar a aplicação no **SP** iniciada modo:
    ...
    > [!NOTE] 
    > Estes valores não estiverem reais. Atualize estes valores com o identificador real, URL de resposta e URL de início de sessão. Pode obter estes valores a partir do ecrã de configuração de SAML do Atlassian na nuvem que é explicada em passos posteriores do tutorial.
...
6. A aplicação de nuvem Atlassian espera as asserções de SAML num formato específico, que necessita para adicionar mapeamentos de atributos personalizado à sua configuração de atributos de Token SAML. Por predefinição, o identificador de utilizador está mapeado com user.userprincipalname. Altere esta para mapear com **user.mail**. Também pode escolher qualquer outro valor adequado pela configuração da organização, mas na maioria dos cenários, correio eletrónico deve funcionar.
...
8. No **configuração da nuvem Atlassian** secção, clique em **configurar a nuvem de Atlassian** para abrir **configurar início de sessão** janela. Copiar o **ID de entidade de SAML e único início de sessão no URL do serviço SAML** do **secção de referência rápida.**
...
9. Para obter SSO configurado para a sua aplicação, o início de sessão no portal de Atlassian utilizando os direitos de administrador.

10. Navegue para **administração de sites Atlassian** > **organizações e segurança**. Se ainda não o fez, crie e atribua o nome da organização. Em seguida, no painel de navegação esquerdo, clique em **domínios**.

    ![Configurar o início de sessão único](./media/active-directory-saas-atlassian-cloud-tutorial/tutorial_atlassiancloud_06.png)

11. Selecione a forma como os que pretende verificar o seu domínio - **DNS** ou **HTTPS**.

    ![Configurar o início de sessão único](./media/active-directory-saas-atlassian-cloud-tutorial/tutorial_atlassiancloud_17.png)

12. Verificação de DNS, selecione **DNS** separador no **domínios** e execute os passos de follwing da página:
...
13. Volte à **página domínios** na administração da organização e clique em de **verificar domínio** botão. Introduza o nome de domínio no pop-up e clique o **verificar domínio** botão.
...
14. Verificação de HTTPS, selecione o **HTTPS** separador no **domínios** página e execute os seguintes passos:
...
15. Volte à **domínios** página na administração da organização e clique em de **verificar domínio** botão. Introduza o **nome de domínio** no pop-up e clique o **verificar domínio** botão.

    ![Configurar o início de sessão único](./media/active-directory-saas-atlassian-cloud-tutorial/tutorial_atlassiancloud_22.png)

16. Se o processo de verificação pode localizar o ficheiro que carregou no diretório de raiz, o estado do domínio atualizações **Verified**.

    ![Configurar o início de sessão único](./media/active-directory-saas-atlassian-cloud-tutorial/tutorial_atlassiancloud_23.png)

    > [!NOTE]
    > Para obter mais informações sobre a verificação de domínio, consulte [documentação de verificação de domínio do Atlassian](https://confluence.atlassian.com/cloud/domain-verification-873871234.html)

17. Na barra de navegação esquerdo, clique em **SAML-início de sessão único**. Se ainda não o fez, subscreve do Atlassian Identity Manager.

    ![Configurar o início de sessão único](./media/active-directory-saas-atlassian-cloud-tutorial/tutorial_atlassiancloud_11.png)

18. No **configuração SAML do adicionar** caixa de diálogo, adicione as definições do fornecedor de identidade, da seguinte forma:
    ...
    c. Abra o certificado transferido a partir do portal do Azure num bloco de notas, copie os valores sem as linhas de certificado de começar e certificado do fim e cole-a no **X509 pública certificado** caixa.
    
    d. Clique em **Guardar configuração**.
     
19. Atualize as definições do Azure AD para se certificar de que configurou os URLs corretos.
    ...
    > [!NOTE]
    > Tem de clicar em clientes existentes **Sim, atualize a configuração** depois de atualizar o **SP identidade ID** e **URL do serviço de consumidor do SP asserção** valores no Portal do Azure. Novos clientes não precisam de executar este passo. 
    
20. No portal do Azure, clique em **guardar** botão.
