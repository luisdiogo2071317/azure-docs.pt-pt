...
Os administradores de função com privilégios podem rever acesso privilegiado de uma vez um [revisão de segurança foi iniciada](pim-how-to-start-security-review.md). O Azure AD Privileged Identity Management (PIM) enviará automaticamente um e-mail a pedir aos utilizadores para rever o acesso. Se um utilizador não recebeu uma mensagem de e-mail, pode enviar-lhes as instruções [como realizar uma revisão de segurança](pim-how-to-perform-security-review.md).

Depois do período de avaliação de segurança está acima ou tem concluído a todos os usuários de seus Self-rever, siga os passos neste artigo para gerir a revisão e ver os resultados.
...
Se deseja aplicar manualmente os resultados da revisão de segurança, pode exportar a revisão. O **exportar** botão irá começar a transferir um ficheiro CSV. Pode gerir os resultados no Excel ou outros programas que abrem arquivos CSV.
