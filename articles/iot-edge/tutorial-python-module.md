...
> [!IMPORTANT]
> Atualmente só pode ser executado o módulo de Python nos contentores do Linux amd64; Não é possível executar a contentores do Windows ou contentores baseadas em ARM. 
...
* [PIP](https://pip.pypa.io/en/stable/installing/#installation) para instalar pacotes de Python (normalmente, incluídos com a instalação de Python).
...
2. No terminal integrado, introduza o seguinte comando para instalar (ou Atualize) **cookiecutter** (sugerimos fazê-lo num ambiente virtual ou como uma instalação de utilizador conforme mostrado abaixo):
...
8. Adicionar o `TEMPERATURE_THRESHOLD`, `RECEIVE_CALLBACKS`, e `TWIN_CALLBACKS` nos contadores global. O limiar de temperatura define o valor pode exceder a temperatura medida para que os dados a ser enviadas ao IoT Hub.
