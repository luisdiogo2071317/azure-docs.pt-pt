...
2. No terminal integrado, introduza o seguinte comando para instalar (ou atualizar) **cookiecutter**, que serve para criar o modelo de solução do IoT Edge no VS Code:
    ...
    ```
   >[!Note]
   >Certifique-se de que o diretório onde o cookiecutter será instalado está no `Path` do seu ambiente, para que possa invocá-lo a partir de uma linha de comandos. Normalmente, no Windows, adicione `%APPDATA%\Python\PythonVersion\Scripts`, onde tem a sua versão do Python, em vez de PythonVersion.
...
Se não tiver especificado um registo de contentor ao criar a sua solução, mas aceitou o valor do localhost:5000 predefinido, não terá um ficheiro \.env. 
   ...
   ![Área de trabalho da solução Python](./media/tutorial-python-module/workspace.png)
