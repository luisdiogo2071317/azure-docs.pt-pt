...
1. [Instalar o runtime do Azure IoT Edge](./how-to-install-iot-edge-windows-with-windows.md) num dispositivo Windows que pretende utilizar como o gateway transparente.
...
1. Obtenha OpenSSL para Windows. Existem várias formas, pode instalar o OpenSSL:
   ...
   * Transferir o código-fonte OpenSSL e criar os binários no seu computador, por si próprio ou por meio [vcpkg](https://github.com/Microsoft/vcpkg). As instruções indicadas abaixo utilizam vcpkg para transferir o código-fonte, compilação e instalar o OpenSSL no seu computador Windows com passos simples.
      ...
      1. Depois de vcpkg é instalado, a partir de uma linha de comandos do powershell, execute o seguinte comando para instalar o pacote de OpenSSL para o Windows x64. A instalação normalmente demora cerca de 5 minutos a ser concluída.

         ```PowerShell
         .\vcpkg install openssl:x64-windows
         ```
      1. Adicione `$VCPKGDIR\installed\x64-windows\tools\openssl` para sua `PATH` variável de ambiente para que o `openssl.exe` arquivo está disponível para invocação.
...
1. Crie o certificado de AC de proprietário e um certificado intermédio. Os certificados são colocados em `$WRKDIR`.
...
1. Copie os seguintes ficheiros de $WRKDIR em qualquer lugar no seu dispositivo Edge, faremos referência a ela como $CERTDIR. Se para gerar os certificados no seu dispositivo do Edge, ignore este passo.
...
Um dispositivo de downstream pode ser de qualquer aplicação com o [Azure IoT device SDK](../iot-hub/iot-hub-devguide-sdks.md), como aquele simple descritas [ligar o dispositivo ao seu hub IoT com .NET](../iot-hub/quickstart-send-telemetry-dotnet.md). Um aplicativo de dispositivo downstream deve confiar a **proprietário AC** certificado para validar as ligações de TLS para os dispositivos de gateway. Normalmente, é possível executar este passo de duas formas: ao nível do SO ou (para certos idiomas) ao nível da aplicação.
...
* Windows - este é um exemplo de como instalar um certificado de AC num host do Windows.
  1. No menu Iniciar, escreva em "Gerir certificados de computador". Isso deve abrir um utilitário chamado `certlm`.
  2. Navegue para **certificados de computador Local** > **certificados de raiz fidedigna** > **certificados** > com o botão direito > **Todas as tarefas** > **importar** para iniciar o Assistente de importação de certificado.
  3. Siga os passos conforme indicado e importe o certificado ficheiro $CERTDIR/certs/azure-iot-test-only.root.ca.cert.pem.
  4. Quando concluída, deverá ver uma mensagem "Importada com êxito".
