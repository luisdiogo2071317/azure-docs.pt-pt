...
>[!NOTE]
>Estas definições estão em pré-visualização pública no portal do Azure. Para obter documentação sobre como gerir as definições de multi-factor Authentication do Azure na pré-visualização, consulte [definições de configurar o Azure multi-factor Authentication - pré-visualização pública](multi-factor-authentication-whats-next.md).
...
### <a name="use-an-auth-provider"></a>Utilize um fornecedor de autenticação
...
Se tiver um fornecedor do multi-factor Auth ou uma MFA do Azure, Azure AD Premium, ou o Enterprise Mobility + licença de segurança, utilize este método para aceder à página de definições do serviço de MFA.
...
2. Para reportar fraude, introduza o código de fraude (a predefinição é 0) e, em seguida, a tecla #. Será notificado de que foi submetido um alerta de fraude.
3. Termine a chamada.
...
3. Em Selecionar superior **fornecedores do multi-factor Auth**. Aparece uma lista dos seus fornecedores do multi-factor Auth.
...
7. Clique em **Executar**. Isto apresenta um relatório de alertas de fraude. Clique em **exportar para CSV** se pretender exportar o relatório.
...
3. No Portal do Azure multi-factor Authentication gestão, se vir o nome do seu inquilino ou o fornecedor de MFA do Azure no lado esquerdo com um  **+**  junto ao mesmo, clique em de  **+**  Consulte diferentes grupos de replicação do servidor MFA e o grupo predefinido do Azure. Selecione o grupo adequado.
...
6. Introduza o nome de utilizador, o número de segundos que a omissão existirá e o motivo para a omissão. Clique em **ignorar**.
...
6. Especifique o intervalo de datas que pretende ver o relatório. Também pode especificar nomes de utilizador, números de telefone e o estado do utilizador.
7. Clique em **Executar**. Isto apresenta um relatório de omissões. Clique em **exportar para CSV** se pretender exportar o relatório.
...
10. Assim que estiver concluído, uma mensagem confirma que carregou com êxito o ficheiro.
...
4. Na configurar colocação em cache página, clique em **nova Cache**.
...
3. Na página de definições de serviço, em IPs fidedignos, tem duas opções:
   ...
   * **Para pedidos de utilizadores federados com origem na minha intranet** – a caixa de verificação. Todos os utilizadores federados que estão a iniciar sessão na rede da empresa irão ignorar a verificação de dois passos utilizando uma afirmação emitida pelo AD FS.
