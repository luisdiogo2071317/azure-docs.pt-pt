...
3. Selecione a opção de verificação que pretende utilizar. 
  - Se não tiver acesso para os métodos alternativos ambos, contacte o suporte da sua empresa para obter ajuda o início de sessão na sua conta.
  - Se tiver acesso para os métodos alternativos, prossiga com a verificação em dois passos.
...
Se o seu telefone foi perdido ou roubado, recomendamos também que informe o suporte da empresa. Irá precisar de repor as palavras-passe de aplicação e desmarque qualquer memorizadas dispositivos. 

### <a name="use-an-alternate-phone-number"></a>Utilizar um número de telefone alternativo
Se configurar várias opções de verificação, como um número de telefone secundário ou uma aplicação de autenticação num dispositivo diferente, utilize um para iniciar sessão.
...
Existem vários motivos por que razão pode tentar iniciar sessão, mas não receber o texto ou chamada telefónica. Se tiver recebido com êxito textos ou chamadas telefónicas para o seu telefone no passado, em seguida, este problema é provavelmente um problema com o fornecedor de telefone, não a sua conta. Certifique-se de que tem um sinal de boa célula. Além disso, se estiver a tentar receber uma mensagem de texto, certifique-se de que é capaz de receber mensagens de texto. Peça um friend para chamar o texto ou utilizador, como um teste. 
