...
2. Na janela **Novo Projeto**, no painel **Instalado**, expanda **Visual C++**, clique em **Win32** e clique em **Aplicação de Consola Win32**. Dê o nome hellodocumentdb ao projeto e clique em **OK**. 
    ...
    ![Captura de ecrã do assistente Novo projeto](media/documentdb-cpp-get-started/hello.png)
...
5. No separador **NuGet: hellodocumentdb**, clique em **Procurar** e procure *documentdbcpp*. Nos resultados, selecione DocumentDbCPP, conforme mostrado na captura de ecrã seguinte. Este pacote instala referências ao C++ REST SDK, que é uma dependência do DocumentDbCPP.  
    ...
    ![Captura de ecrã que mostra o pacote DocumentDbCPP realçado](media/documentdb-cpp-get-started/cpp.png)
...
![URI do Azure Cosmos DB e chaves no portal do Azure](media/documentdb-cpp-get-started/nosql-tutorial-keys.png)
...
Os [documentos](documentdb-resources.md#documents) são conteúdos JSON definidos pelo utilizador (arbitrários). Pode agora inserir um documento no Azure Cosmos DB. Pode criar um documento ao copiar o código seguinte para o fim da função principal. 
...
![Tutorial C++ - diagrama que ilustra a relação hierárquica entre a conta, a base de dados, a coleção e os documentos](media/documentdb-cpp-get-started/docs.png)

## <a id="QueryDB"></a>Passo 7: Consultar recursos do Azure Cosmos DB
O Azure Cosmos DB suporta [consultas extensas](documentdb-sql-query.md) de documentos JSON armazenados em cada coleção. O código de exemplo seguinte mostra uma consulta criada com sintaxe SQL da BD do Cosmos do Azure, a qual pode ser executada nos documentos que criámos no passo anterior.
...
![Saída da aplicação C++ do Azure Cosmos DB](media/documentdb-cpp-get-started/console.png)
