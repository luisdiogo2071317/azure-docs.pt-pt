...
O SDK de Java de API do SQL Server suporta operações síncronas. Para obter suporte assíncrono, utilize o [SDK Java do SQL Server API Async](sql-api-sdk-async-java.md). 
...
* Suporte adicionado para o índice exclusivo.
* Suporte adicionado para limitar o tamanho de token de continuação nas opções do feed.
* Corrigido um erro na serialização Json (timestamp).
* Corrigido um erro na serialização Json (enumeração).
* Dependência de com.fasterxml.jackson.core:jackson-databind atualizado para 2.9.5.

### <a name="a-name11601160"></a><a name="1.16.0"/>1.16.0
* Ligação melhorada agrupamento para o modo direto.
* Melhorado melhoramento de obtenção prévia para não orderby cruzada consulta da partição.
* Geração de UUID melhorada.
* Lógica de consistência de sessão melhorada.
* Suporte adicionado para multipolygon.
* Suporte adicionado para estatísticas de intervalo de chave de partição para a coleção.
* Corrigido um erro no suporte de multirregião.
...
* Suporte de política de repetição adicionado para limitação de taxa.  
