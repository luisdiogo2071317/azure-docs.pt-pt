---
title: Visualize os dados na atualização do Explorer | Documentos da Microsoft
...
---
# <a name="visualize-data-in-the-explorer-update"></a>Visualize os dados na atualização do Explorer

Este artigo descreve os recursos e as opções disponíveis dentro do Azure Time Series Insights (pré-visualização) aplicativo web do Explorer.

## <a name="prerequisites"></a>Pré-requisitos

Antes de utilizar o Azure Time Series Insights (pré-visualização) Explorador, faça o seguinte:
...
## <a name="learn-about-the-azure-time-series-insights-preview-explorer"></a>Saiba mais sobre o Azure Time Series Insights (pré-visualização) Explorer

  ![um Explorador][1]

O Azure Time Series Insights (pré-visualização) Explorer é dividido em sete elementos:
...
## <a name="time-series-insights-preview-navigation-menu"></a>O Time Series Insights (pré-visualização) Menu de navegação
...
## <a name="time-series-insights-preview-model-search-panel"></a>O Time Series Insights (pré-visualização), o painel de pesquisa de modelo
...
## <a name="time-series-insights-preview-well"></a>O Time Series Insights (pré-visualização) bem
...
Nota: Se vir o ícone que se segue, a instância não tem quaisquer dados durante o período de tempo selecionado.  Para corrigir o problema, pode aumentar o período de tempo selecionado e/ou confirmar que a instância está a enviar dados.

  ![sete de Explorer][7]

## <a name="time-series-insights-preview-chart"></a>O Time Series Insights (pré-visualização) gráfico
...
1. Intervalo selecionado de data – o intervalo de datas selecionado para o painel de gráfico, este controla quais elementos de dados estarão disponíveis para visualização.

1. Ferramenta de controlo de deslize do intervalo de datas interna - abrangem a controles de ponto final de utilizar os dois, clicando e arrastando-os no tempo desejado.

1. Controlo de recolhimento span de tempo - esse controle reduz e expande o editor de span painel de tempo.

1. Controlo de formato do eixo y – clique esse controle para percorrer as opções de exibição disponíveis do eixo y. As opções de vista do eixo y disponíveis são:

    * Predefinição - a cada linha tem um eixo y individual
    * Empilhadas – Isto permite-lhe empilhar várias linhas no mesmo eixo y, com a alteração de dados de eixo y com base na linha selecionada
    * Compartilhado – todos os dados do eixo y apresentados em conjunto

1. Elemento data atual – o elemento de dados atualmente selecionados e os respetivos detalhes associados.
...
## <a name="time-series-insights-preview-time-editor-panel"></a>O Time Series Insights (pré-visualização) painel do Editor de tempo
...
1. **Intervalo de e para o controle da web de datas** – com esse controle da web pode facilmente clicar e selecione a data pretendida e os intervalos de tempo. Também pode utilizar o controlo de alternar entre diferentes fusos horários. Depois de efetuar as alterações que pretende aplicar ao seu espaço de trabalho atual, clique em salvar botão.
...
Clique no controle de web de ligação dentro de um círculo para gerar um URL para guardar ou partilhar a sua sessão de trabalho atual do Time Series Insights que inclui:
...
### <a name="time-series-insights-preview-terms-panel"></a>O Time Series Insights (pré-visualização) painel de termos
...
O TSI (pré-visualização) painel do Editor de termos precisa dos seguintes parâmetros
...
Saiba mais sobre a nova [modelos de série de tempo](./time-series-insights-update-tsm.md).
