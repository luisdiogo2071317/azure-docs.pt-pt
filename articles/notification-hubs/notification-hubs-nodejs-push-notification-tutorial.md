...
Este guia mostra como enviar notificações push com a ajuda dos Notification Hubs do Azure diretamente a partir de uma aplicação Node.js. 
...
O **NotificationHubService** objeto permite-lhe trabalhar com os notification hubs. O código seguinte cria um **NotificationHubService** objeto para o hub de notificação com o nome **hubname**. Adicione-a junto ao topo do **server.js** ficheiro, após a declaração a importar o módulo do azure:
...
4. Clique em **políticas de acesso**. Consulte ambas as cadeias de ligação de acesso partilhado e completa.
...
* **Etiquetas** -o identificador de etiqueta. Se nenhuma tag for fornecido, a notificação é enviada a todos os clientes.
...
* **Etiquetas** -o identificador de etiqueta. Se nenhuma tag for fornecido, a notificação é enviada a todos os clientes registados.
