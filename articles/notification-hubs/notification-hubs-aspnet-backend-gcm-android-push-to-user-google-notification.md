...
ms.locfileid: "33776292"
---
# <a name="tutorial-push-notification-to-specific-android-application-users-by-using-azure-notification-hubs"></a>Tutorial: Enviar notificações push para utilizadores de aplicações Android específicos com os Hubs de Notificação do Azure
...
1. Abra o ficheiro **res/layout/activity_main.xml** e substitua o conteúdo pelas definições de conteúdo que se seguem:
    ...
    ```
3. Abra o ficheiro **res/values/strings.xml** e substitua a definição `send_button` pelas seguintes linhas que redefinem a cadeia de `send_button` e adicionam cadeias para os outros controlos:
    ...
    O esquema gráfico main_activity.xml deverá ter agora um aspeto semelhante ao da seguinte imagem:
   
    ![][A1]
4. Crie uma nova classe com o nome **RegisterClient** no mesmo pacote que a classe `MainActivity`. Utilize o código abaixo para o ficheiro da nova classe.
...
1. Na sua classe `MainActivity`, no método `onCreate`, remova ou acrescente delimitadores de comentário à inicialização do campo `hub` e à chamada para o método `registerWithNotificationHubs`. Em seguida, adicione código para inicializar uma instância da classe `RegisterClient`. O método deverá conter as seguintes linhas:
    ...
    ```
2. Na sua classe `MainActivity`, elimine ou acrescente delimitadores de comentário ao método `registerWithNotificationHubs` completo. Não será utilizado neste tutorial.
3. Adicione as seguintes instruções `import` ao ficheiro **MainActivity.java**.
    ...
    ```            
4. Substitua o código no método onStart pelo seguinte código: 
    ...
    ```       
1. Em seguida, adicione os seguintes métodos para processar o evento de clique no botão **Iniciar sessão** e o envio de notificações push.
...
5. Adicione o seguinte método `DialogNotify` à classe `MainActivity`. 
    ...
    ```
1. Na sua classe `MainActivity`, atualize o método `sendNotificationButtonOnClick` para chamar o método `sendPush` com os serviços de notificação de plataforma selecionados do utilizador do seguinte modo.
    ...
    ```
7. No ficheiro **build.gradle**, adicione a seguinte linha à secção `android` a seguir à secção `buildTypes`. 
