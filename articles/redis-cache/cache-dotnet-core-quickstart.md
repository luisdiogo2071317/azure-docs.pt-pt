---
title: Início Rápido para aprender a utilizar a Cache de Redis do Azure com as aplicações de .NET Core| Microsoft Docs
description: Neste início rápido, aprenda como aceder à Cache de Redis do Azure nas aplicações .NET Core
...
Este início rápido mostra como começar a utilizar a Cache de Redis do Microsoft Azure com o .NET Core. A Cache de Redis do Microsoft Azure baseia-se na popular Cache de Redis open source. Permite-lhe aceder a uma cache de Redis segura, dedicada e gerida pela Microsoft. Pode aceder a uma cache criada com a Cache de Redis do Azure a partir de qualquer aplicação dentro do Microsoft Azure.
...
Adicione os seguintes membros à classe `Program` no *Program.cs*. Este código inicializa uma configuração para aceder ao segredo do utilizador para a cadeia de ligação da cache de redis.
