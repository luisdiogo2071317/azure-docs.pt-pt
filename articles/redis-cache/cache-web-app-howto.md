---
title: Criar uma aplicação Web ASP.NET com a Cache de Redis do Azure | Microsoft Docs
description: Neste início rápido, vai aprender a criar uma aplicação Web ASP.NET com a Cache de Redis do Azure
...
ms.locfileid: "38674891"
---
# <a name="quickstart-create-an-aspnet-web-app"></a>Início Rápido: Criar uma aplicação Web ASP.NET 

## <a name="introduction"></a>Introdução

Este início rápido mostra como criar e implementar uma aplicação Web ASP.NET num Serviço de Aplicações do Azure com o Visual Studio 2017. O exemplo de aplicação liga-se a uma Cache de Redis do Azure para armazenar e obter dados da cache. Depois de concluir o início rápido, terá uma aplicação Web funcional, alojada no Azure, que lê e escreve numa Cache de Redis do Azure.
...
Nesta secção, vai atualizar a aplicação para suportar uma nova vista que apresenta um teste simples da Cache de Redis do Azure.
...
3. O pacote NuGet transfere e adiciona as referências de assemblagem necessárias para a sua aplicação cliente aceder à Cache de Redis do Azure com o cliente de cache StackExchange.Redis. Se preferir utilizar uma versão com um nome forte da biblioteca de cliente `StackExchange.Redis`, instale o pacote `StackExchange.Redis.StrongName`.
...
2. No browser, selecione **Teste da Cache de Redis do Azure** na barra de navegação.

3. No exemplo seguinte, a chave `Message` tinha anteriormente um valor em cache, o qual foi definido com a consola da Cache de Redis do Azure no portal. A aplicação atualizou esse valor em cache. A aplicação também executou os comandos `PING` e `CLIENT LIST`.
...
Selecione **Teste da Cache de Redis do Azure** na barra de navegação para testar o acesso à cache.
