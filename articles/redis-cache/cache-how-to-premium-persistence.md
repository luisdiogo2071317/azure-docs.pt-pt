...
description: Saiba como configurar e gerir as instâncias de Cache de Redis do Azure de escalão Premium de persistência de dados
...
Persistência é configurada do **nova Cache de Redis** painel durante a criação da cache e, no **menu recursos** Premium existentes coloca em cache.
...
A lista seguinte contém respostas para perguntas freqüentes sobre a persistência da Cache de Redis do Azure.
...
Sim, pode alterar a frequência de cópia de segurança para a persistência de RDB no **persistência de dados de Redis** painel. Para obter instruções, consulte [persistência de Redis configurar](#configure-redis-persistence).
