...
Os enlaces de armazenamento de BLOBs são fornecidos na [Microsoft.Azure.WebJobs](http://www.nuget.org/packages/Microsoft.Azure.WebJobs) pacote NuGet, versão 3.x. Código-fonte para o pacote está no [sdk do webjobs do azure](https://github.com/Azure/azure-webjobs-sdk/tree/master/src/Microsoft.Azure.WebJobs.Storage/Blob) repositório do GitHub.
...
* Alta escalabilidade
* Atraso de arranque a frio

### <a name="blob-only-storage-accounts"></a>Contas de armazenamento apenas de BLOBs

[Contas de armazenamento apenas de BLOBs](../storage/common/storage-create-storage-account.md#blob-storage-accounts) são suportadas para o blob de entrada e saída enlaces, mas não para acionadores de Blobs. Acionadores do armazenamento de BLOBs precisam de uma conta de armazenamento para fins gerais.
...
O exemplo seguinte mostra o blob de entrada e saída de ligações num *Function* de ficheiros e [código de JavaScript] (node.md de referência de funções) que utiliza as ligações. A função faz uma cópia de um blob. A função é acionada por uma mensagem de fila que contém o nome do blob para copiar. O blob novo com o nome *{originalblobname}-cópia*.
...
> [Ir para um início rápido que utiliza um acionador do armazenamento de BLOBs](functions-create-storage-blob-triggered-function.md)
