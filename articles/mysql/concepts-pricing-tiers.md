...
É possível criar uma base de dados do Azure para o servidor de MySQL em um de três camadas preços diferentes - básico, fins gerais e otimização de memória. Os escalões de preços são diferenciados pela quantidade de computação no vCores que podem ser aprovisionadas, memória por vCore e a tecnologia de armazenamento utilizado para armazenar os dados. Todos os recursos aprovisionados ao nível do servidor de MySQL. Um servidor pode ter uma ou várias bases de dados.
