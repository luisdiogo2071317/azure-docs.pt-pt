---
title: Descrição geral de ponto final dos serviços de base de dados do Azure para o servidor de MySQL vnet | Microsoft Docs
description: Descreve como funcionam os pontos finais do serviço de vnet para a base de dados do Azure para o servidor de MySQL.
services: mysql
author: mbolz
ms.author: mbolz
manager: jhubbard
...
ms.locfileid: "34655656"
---
# <a name="use-virtual-network-service-endpoints-and-rules-for-azure-database-for-mysql"></a>Utilizar pontos finais do serviço de rede Virtual e as regras da base de dados do Azure para MySQL

*Regras de rede virtual* são uma funcionalidade de segurança de firewall que controla se a base de dados do Azure para o servidor de MySQL aceita comunicações que são enviadas do sub-redes específicas em redes virtuais. Este artigo explica por que razão a funcionalidade de regra de rede virtual, por vezes, é a melhor opção em segurança permitir comunicações para a base de dados do Azure para o servidor de MySQL.

Para criar uma regra de rede virtual, tem primeiro de existir um [rede virtual] [ vm-virtual-network-overview] (VNet) e um [ponto final do serviço de rede virtual] [ vm-virtual-network-service-endpoints-overview-649d] para o regra para referência. A imagem seguinte ilustra como um ponto final de serviço de rede Virtual funciona com a base de dados do Azure para o MySQL:

![Exemplo de como funciona um ponto final de serviço de VNet](media/concepts-data-access-and-security-vnet/vnet-concept.png)

> [!NOTE]
> Para a base de dados do Azure para MySQL, esta funcionalidade está disponível em pré-visualização pública em todas as regiões de nuvem pública do Azure onde a base de dados do Azure para MySQL é implementado.
...
**Ponto final de serviço de rede virtual:** A [ponto final do serviço de rede Virtual] [ vm-virtual-network-service-endpoints-overview-649d] uma sub-rede cujos valores de propriedade incluem um ou mais nomes de tipo de serviço do Azure formal. Neste artigo está interessados no nome do tipo de **Microsoft.Sql**, que se refere ao serviço do Azure com o nome da base de dados do SQL Server. Esta etiqueta de serviço também se aplica à base de dados do Azure para serviços MySQL e PostgreSQL. É importante ter em conta ao aplicar o **Microsoft.Sql** etiquetas de serviço para um ponto final de serviço de VNet irá configurar o tráfego de ponto final de serviço para todos os SQL Database do Azure, base de dados do Azure para o MySQL e base de dados do Azure para servidores de PostgreSQL na sub-rede. 

**Regra de rede virtual:** uma regra de rede virtual para a base de dados do Azure para o servidor de MySQL é uma sub-rede que está listada na lista de controlo de acesso (ACL) da base de dados do Azure para o servidor de MySQL. Para ser na ACL para a base de dados do Azure para o servidor de MySQL, a sub-rede tem de conter o **Microsoft.Sql** nome de tipo.

Uma regra de rede virtual indica que a base de dados do Azure para o servidor de MySQL para aceitar comunicações de cada nó que está na sub-rede.
...
### <a name="c-cannot-yet-have-azure-database-for-mysql-on-a-subnet-without-defining-a-service-endpoint"></a>C. Não é possível ainda tem base de dados do Azure para MySQL numa sub-rede sem definir um ponto final de serviço

Se o **Microsoft.Sql** servidor era um nó numa sub-rede na sua rede virtual, todos os nós dentro da rede virtual conseguiu comunicar com a base de dados do Azure para o servidor de MySQL. Neste caso, as suas VMs conseguiu comunicar com a base de dados do Azure para MySQL sem necessidade de quaisquer regras de rede virtual ou das regras IP.

No entanto a partir de Maio de 2018, a base de dados do Azure para o serviço de MySQL não é ainda entre os serviços que podem ser atribuídos diretamente a uma sub-rede.
...
Cada regra de rede virtual aplica-se à base de dados do Azure completa para o servidor de MySQL, não apenas para uma determinada base de dados no servidor. Por outras palavras, se aplica a regra de rede virtual ao nível do servidor, não ao nível da base de dados.
...
- **Administrador de base de dados:** &nbsp; atualizar a lista de controlo de acesso (ACL) para adicionar a sub-rede especificada para a base de dados do Azure para o servidor de MySQL.
...
> [!NOTE]
> Em alguns casos, a base de dados do Azure para o MySQL e a sub-rede da VNet estão em subscrições diferentes. Nestes casos, tem de garantir as seguintes configurações:
...
Para a base de dados do Azure para MySQL, a funcionalidade de regras de rede virtual tem as seguintes limitações:

- Na firewall para a base de dados do Azure para MySQL, cada regra de rede virtual faz referência a uma sub-rede. Estas sub-redes referenciadas tem de estar alojadas na mesma região geográfica que aloja a base de dados do Azure para MySQL.

- Cada base de dados do Azure para o servidor de MySQL pode ter até 128 entradas ACL para qualquer determinada rede virtual.
...
- Ativar na rede virtual pontos finais do serviço base de dados do Azure para utilizar o MySQL o **Microsoft.Sql** etiquetas de serviço também permite que os pontos finais para todos os serviços de base de dados do Azure: base de dados do Azure para MySQL, base de dados do Azure para PostgreSQL, Base de dados SQL do Azure e do armazém de dados SQL do Azure.
...
- [Criar e gerir a base de dados do Azure para as regras de MySQL VNet no portal do Azure](howto-manage-vnet-using-portal.md)
- [Criar e gerir a base de dados do Azure para regras de MySQL VNet utilizando a CLI do Azure](howto-manage-vnet-using-cli.md)
