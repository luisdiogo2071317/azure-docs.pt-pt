---
title: Criar e gerir a base de dados do Azure para pontos finais do serviço de MySQL VNet e regras utilizando a CLI do Azure | Microsoft Docs
description: Este artigo descreve como criar e gerir a base de dados do Azure para pontos finais do serviço de MySQL VNet e regras utilizando a linha de comandos da CLI do Azure.
services: mysql
author: mbolz
ms.author: mbolz
manager: jhubbard
...
ms.locfileid: "34736597"
---
# <a name="create-and-manage-azure-database-for-mysql-vnet-service-endpoints-using-azure-cli"></a>Criar e gerir a base de dados do Azure para pontos finais do serviço de MySQL VNet, utilizando a CLI do Azure
Pontos finais de serviços de rede (VNet) virtuais e as regras de expandem o espaço de endereço privado de uma rede Virtual para a base de dados do Azure para o servidor de MySQL. Utilizar comandos de Interface de linha de comandos do Azure (CLI) conveniente, pode criar, atualizar, eliminar, listar e Mostrar pontos finais do serviço de VNet e regras para gerir o seu servidor. Para obter uma descrição geral da base de dados do Azure para pontos finais de serviço MySQL VNet, incluindo as limitações, consulte [base de dados do Azure para pontos finais do serviço de MySQL servidor VNet](concepts-data-access-and-security-vnet.md). Pontos finais do serviço de VNet estão disponíveis na pré-visualização pública em todas as regiões suportadas para a base de dados do Azure para MySQL.
...
- Um [base de dados do Azure para a base de dados e servidor MySQL](quickstart-create-mysql-server-database-using-azure-cli.md).

> [!NOTE]
> Suporte para pontos finais do serviço de VNet é apenas para fins gerais e otimização de memória de servidores.

## <a name="configure-vnet-service-endpoints-for-azure-database-for-mysql"></a>Configurar pontos finais do serviço de Vnet para base de dados do Azure para MySQL
O [az rede vnet](https://docs.microsoft.com/cli/azure/network/vnet?view=azure-cli-latest) comandos são utilizados para configurar redes virtuais.
...
Instale a extensão do CLI para base de dados do Azure para pontos finais do serviço de MySQL VNet, utilizando o `az extension add --name rdbms-vnet` comando. 
```azurecli-interactive
az extension add --name rdbms-vnet
```

Execute o `az extension list` comando para verificar a instalação da extensão CLI.
```azurecli-interactive
az extension list
```
O resultado do comando apresenta uma lista de todas as extensões instaladas. A base de dados do Azure para a extensão de MySQL CLI é:

 {"extensionType": "whl", "name": "rdbms vnet", "versão": "10.0.0"}

> [!NOTE]
> Para desinstalar a extensão CLI executar o `az extension remove -n rdbms-vnet` comando. 
...
As VNets e os recursos de serviço do Azure podem pertencer às mesmas subscrições ou a subscrições diferentes. Se os recursos de serviço de VNet e o Azure estão em subscrições diferentes, os recursos devem estar no mesmo inquilino do Active Directory (AD), no momento desta pré-visualização.
...
### <a name="sample-script-to-create-an-azure-database-for-mysql-database-create-a-vnet-vnet-service-endpoint-and-secure-the-server-to-the-subnet-with-a-vnet-rule"></a>Exemplo de script para criar uma base de dados do Azure para a base de dados MySQL, criar uma VNet, o ponto final do serviço de VNet e proteger o servidor para a sub-rede com uma regra de VNet
Neste script de exemplo, altere as linhas realçadas para personalizar o nome de utilizador administrador e a palavra-passe. Substitua o ID de subscrição utilizada no `az account set --subscription` comando com o seu próprio identificador de subscrição.
