...
Tenha em atenção que a contagem da partição no hub de eventos não pode ser modificada após a configuração. Com que em mente, é importante considerar sobre quantos partições precisa antes de começar a utilizar. 
...
São aplicáveis encargos de ligação apenas quando é utilizado o protocolo AMQP. Não há custos de ligação para o envio de eventos por HTTP, independentemente do número de sistemas ou dispositivos de envio. Se planeia utilizar AMQP (por exemplo, para alcançar mais eficiente do evento de transmissão em fluxo ou para ativar a comunicação bidirecional no comando de IoT e cenários de controlo), consulte o [informações sobre os preços de Hubs de eventos](https://azure.microsoft.com/pricing/details/event-hubs/) página para obter detalhes sobre quantos as ligações são incluídas em cada camada de serviço.
