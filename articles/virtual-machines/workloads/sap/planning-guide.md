...
Este documento técnico descreve as noções básicas do Microsoft Azure Virtual Machine e fornece instruções de considerações de planeamento e implementação para as instalações do SAP NetWeaver no Azure e como tal, deve ser o documento ler antes de iniciar real implementações do SAP NetWeaver no Azure.
...
Opposite to onde não mudou capacidades de armazenamento Standard depende do tamanho do disco (ou VHD), Premium Storage atualmente tem três categorias de outro disco, que são apresentadas neste artigo: <https://azure.microsoft.com/pricing/details/storage/unmanaged-disks/>
...
> ![Windows][Logo_Windows] Windows
>
> Ver mais detalhes aqui: <https://docs.microsoft.com/azure/virtual-machines/windows/upload-generalized-managed> definições do Windows (por exemplo, o SID do Windows e o nome de anfitrião) tem de ser abstracted/generalizado na VM no local através de comandos do sysprep.
>
>
> ![Linux][Logo_Linux] Linux
>
...
* Definir o disco de SO de uma nova configuração VM ao VHD ou disco geridos com *conjunto AzureRmVMOSDisk* -consulte o artigo <https://docs.microsoft.com/powershell/module/azurerm.compute/set-azurermvmosdisk>
* Criar uma nova VM a partir da configuração de VM com *New-AzureRmVM* -consulte o artigo <https://docs.microsoft.com/powershell/module/azurerm.compute/new-azurermvm>
* Adicionar um disco de dados para uma nova VM com *adicionar AzureRmVMDataDisk* -consulte o artigo <https://docs.microsoft.com/powershell/module/azurerm.compute/add-azurermvmdatadisk>
...
* Implemente a VM com um modelo JSON que referencia o VHD, conforme mostrado no [este modelo JSON de exemplo](https://github.com/Azure/azure-quickstart-templates/blob/master/201-vm-specialized-vhd-new-or-existing-vnet/azuredeploy.json) ou ao utilizar discos geridos, conforme mostrado no [este modelo JSON de exemplo](https://github.com/Azure/azure-quickstart-templates/blob/master/sap-2-tier-user-image-md/azuredeploy.json).
...
* Definir o disco de SO de uma nova configuração VM para o
  * O VHD com *Set AzureRmVMOSDisk - SourceImageUri - CreateOption fromImage* -consulte o artigo <https://docs.microsoft.com/powershell/module/azurerm.compute/set-azurermvmosdisk>
  * Gerido a imagem de disco *conjunto AzureRmVMSourceImage* -consulte o artigo <https://docs.microsoft.com/powershell/module/azurerm.compute/set-azurermvmsourceimage>
* Criar uma nova VM a partir da configuração de VM com *New-AzureRmVM* -consulte o artigo <https://docs.microsoft.com/powershell/module/azurerm.compute/new-azurermvm>
...
* Criar um contexto de conta de armazenamento para o **origem** conta de armazenamento com *New-AzureStorageContext* -consulte o artigo <https://msdn.microsoft.com/library/dn806380.aspx>
* Criar um contexto de conta de armazenamento para o **destino** conta de armazenamento com *New-AzureStorageContext* -consulte o artigo <https://msdn.microsoft.com/library/dn806380.aspx>
...
Para obter mais em segundo plano nos detalhes de implementação do SAP no Azure, gostaríamos que experimentasse ler capítulo [implementação conceitos de Cloud-Only de instâncias SAP] [ planning-guide-7] deste documento que explica alguns dos as construções de noções básicas do Azure e como estes devem ser utilizadas com aplicações SAP no Azure.
...
| Gateway |sapgw`<nn`> consulte * |3301 |gratuito |Gateway SAP, utilizada para comunicação CPIC e RFC |
| Router SAP |sapdp99 |3299 |gratuito |Os nomes de serviço de CI (instância central) apenas podem ser reatribuídos em /etc/services para um valor arbitrário após a instalação. |
...
No site para site ligados em vários locais cenários, a latência entre no local e o Azure ainda podem ser significativo. Se, siga a sequência de transporte objetos através de sistemas de desenvolvimento e teste para produção ou considere sobre aplicar transportes ou pacotes de suporte para os sistemas diferentes, tenha em atenção que, dependente na localização do diretório central de transporte, Alguns dos sistemas irão encontrar latência elevada ler ou escrever dados no diretório central do transporte. A situação é semelhante a configurações de horizontal SAP onde os diferentes sistemas são distribuídos através de centros de dados diferentes com substancial distância entre centros de dados.
...
Para ativar a monitorização dos sistemas SAP crítico de missão no Azure SAP ferramentas SAPOSCOL ou SAP o agente do anfitrião de monitorização obter dados desligar o anfitrião do serviço de Máquina Virtual do Azure através de uma extensão de monitorização do Azure para SAP. Uma vez que os pedidos por SAP foram muito específicos para aplicações SAP, Microsoft decidiu não genericamente implementa a funcionalidade necessária para o Azure, mas deixe-o para que os clientes implementem os componentes de monitorização necessários e configurações para os respetivos Virtual Máquinas em execução no Azure. No entanto, o ciclo de vida de implementação e gestão dos componentes de monitorização irá ser principalmente automatizada pelo Azure.
...
> ![Windows][Logo_Windows] Windows
>
> Teoria, as VMs que bases de dados de execução podem ser feitas de forma consistente, bem como se o sistema DBMS suporta o VSS do Windows (serviço de cópia sombra de volumes <https://msdn.microsoft.com/library/windows/desktop/bb968832(v=vs.85).aspx>) como, por exemplo, SQL servidor não.
