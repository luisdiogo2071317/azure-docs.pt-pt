---
title: Utilize a automatização do Azure para aumentar verticalmente máquinas virtuais do Windows | Microsoft Docs
description: Aumentar verticalmente Máquina Virtual do Windows em resposta a alertas com a automatização do Azure de monitorização
...
A primeira coisa que precisa de fazer é criar uma conta de automatização do Azure que irá alojar os runbooks utilizados para dimensionar uma Máquina Virtual. Recentemente, o serviço de automatização introduziu a funcionalidade de "Conta Run As" que faz com que a definição de segurança de Principal de serviço para executar automaticamente os runbooks em nome do utilizador muito fácil. Pode ler mais sobre este artigo abaixo:
