...
Manutenção self-service não é recomendada para implementações que utilizam **conjuntos de disponibilidade** , uma vez que estes são configurações de elevada disponibilidade, onde apenas um domínio de atualização é afetado num determinado momento. 
    ...
    - Se uma perda temporária de algumas das sua capacidade (contagem de domínios de atualização/1) é uma preocupação, ele pode facilmente ser compensado de ao alocar instâncias de adição durante o período de manutenção **não** utilizar manutenção self-service na seguinte cenários: 
...
Também pode utilizar o Azure Powershell para ver quando as VMs são agendadas para manutenção. Informações de manutenção planeada estão disponíveis a partir da [Get-AzureRmVM](/powershell/module/azurerm.compute/get-azurermvm) cmdlet quando utiliza o `-status` parâmetro.
...
Também pode obter o estado de manutenção para todas as VMs num grupo de recursos usando [Get-AzureRmVM](/powershell/module/azurerm.compute/get-azurermvm) e não especificar uma VM.
