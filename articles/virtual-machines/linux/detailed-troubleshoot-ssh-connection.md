...
Se não tiver outra VM na mesma rede virtual, pode facilmente criar um. Para obter mais informações, consulte [criar uma VM do Linux no Azure com a CLI](quick-create-cli.md). Elimine a VM extra quando tiver terminado com os seus testes.
...
Se ainda não fez isso, siga as instruções [para repor uma palavra-passe baseado em Linux as máquinas virtuais](reset-password.md).
...
Para obter mais informações sobre resolução de problemas de acesso da aplicação, consulte [resolver problemas de acesso para uma aplicação em execução numa máquina virtual do Azure](troubleshoot-app-connection.md)
