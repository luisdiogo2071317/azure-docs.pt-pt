...
Se copia e colar o conteúdo do ficheiro de chave pública no portal do Azure ou um modelo do Resource Manager, certifique-se de não copiar qualquer espaço em branco adicional ou introduzir linebreaks adicionais. Por exemplo, se usar o macOS, pode encaminhar o ficheiro de chave pública (por predefinição, `~/.ssh/id_rsa.pub`) para **pbcopy** para copiar o conteúdo (existem outros programas Linux que fazem a mesma coisa, como **xclip**).
...
Agora que tem um par de chaves SSH e um ficheiro de configuração SSH configurado, pode iniciar sessão na VM do Linux de maneira rápida e segura. Ao executar o seguinte comando, o SSH localiza e carrega todas as definições do `Host myvm` bloquear no ficheiro de configuração SSH.

```bash
ssh myvm
```

A primeira vez que iniciar sessão para um servidor com uma chave SSH, os prompts de comando para a frase de acesso para esse ficheiro de chave.
