...
Os recursos do Microsoft Azure são distribuídos em várias regiões geográficas em todo o mundo.  Uma "região" representa vários datacenters numa única área geográfica.  Temos 34 regiões geralmente disponíveis em todo o mundo, com outras quatro regiões anunciadas. Uma vez que estamos continuamente a expandir a nossa cobertura global, mantemos uma lista atualizada das regiões já existentes e das recém-anunciadas.

* [Regiões do Azure](https://azure.microsoft.com/regions/)

## <a name="availability"></a>Disponibilidade
Anunciámos um Contrato de Nível de Serviço líder da indústria de 99,9% para máquinas virtuais de instância única, desde que implemente a VM com armazenamento premium para todos os discos.  Para a sua implementação se qualificar para o nosso Contrato de Nível de Serviço de VM de 99,95% standard, continua a ter de implementar duas ou mais VMs que executem a sua carga de trabalho num conjunto de disponibilidade. Esta configuração garante que as VMs são distribuídas em vários domínios de falha nos nossos datacenters, bem como implementadas em anfitriões com diferentes janelas de manutenção. O [SLA do Azure](https://azure.microsoft.com/support/legal/sla/virtual-machines/v1_0/) completo explica a disponibilidade garantida do Azure em termos globais.

## <a name="managed-disks"></a>Managed Disks

O Managed Disks processa a criação e a gestão da conta de Armazenamento do Azure em segundo plano por si e assegura que não tem de se preocupar com os limites de escalabilidade da conta de armazenamento. Basta especificar o tamanho do disco e o escalão de desempenho (Standard ou Premium), e o Azure cria e gere o disco por si. Mesmo que adicione discos ou aumente e reduza verticalmente a VM, não terá de se preocupar se o armazenamento está a ser utilizado. Se estiver a criar VMs novas, [utilize a CLI 2.0 do Azure](quick-create-cli.md?toc=%2fazure%2fvirtual-machines%2flinux%2ftoc.json) ou o portal do Azure para criar as VMs com discos de dados e de SO geridos. Se tiver VMs com discos não geridos, pode [converter as VMs para a criação de cópias de segurança com o Managed Disk](convert-unmanaged-to-managed-disks.md?toc=%2fazure%2fvirtual-machines%2flinux%2ftoc.json).
...
O Microsoft Azure suporta a execução de diversas distribuições Linux populares, que são disponibilizadas e mantidas por múltiplos parceiros.  Vai encontrar distribuições como Red Hat Enterprise, CentOS, SUSE Linux Enterprise, Debian, Ubuntu, CoreOS, RancherOS, FreeBSD, entre outras, no Azure Marketplace. Trabalhamos ativamente com várias comunidades do Linux para adicionar ainda mais tipos à lista [Azure endorsed Linux Distros](endorsed-distros.md?toc=%2fazure%2fvirtual-machines%2flinux%2ftoc.json)(Distribuições Linux aprovadas pelo Azure).

Se a distribuição que preferir não estiver incluída atualmente na galeria, pode [criar e carregar um VHD do Linux para o Azure](create-upload-generic.md?toc=%2fazure%2fvirtual-machines%2flinux%2ftoc.json) para “Trazer a sua própria VM do Linux”.

As máquinas virtuais do Azure permitem-lhe implementar uma vasta gama de soluções de computação de forma ágil. Pode implementar virtualmente qualquer carga de trabalho e qualquer linguagem em praticamente qualquer sistema operativo - Windows, Linux ou num sistema personalizado criado com base na nossa crescente lista de parceiros. Ainda não encontrou o que procura?  Não se preocupe, também pode incluir as suas próprias imagens do local.

## <a name="vm-sizes"></a>Tamanhos de VMs
Quando implementa uma VM no Azure, vai selecionar um tamanho de VM a partir de uma das nossas séries de tamanhos que são adequadas à sua carga de trabalho. O tamanho também afeta a capacidade de processamento, memória e armazenamento da máquina virtual. É faturado com base no período de tempo em que a VM está em execução e a consumir os recursos que lhe foram atribuídos. Uma lista completa dos [tamanhos das Máquinas Virtuais](sizes.md?toc=%2fazure%2fvirtual-machines%2flinux%2ftoc.json).

Seguem-se algumas orientações básicas para selecionar um tamanho de VM a partir de uma das nossas séries (A, D, DS, G e GS).
* As VMs da série A são as VMs de gama de entrada para cargas de trabalho leves e cenários de Dev/Test. Estão amplamente disponíveis em todas as regiões e podem ligar e utilizar todos os recursos standard disponíveis para as máquinas virtuais.
* Os tamanhos da série A (A8 - A11) são configurações especiais para processos de computação intensiva e adequados para aplicações em cluster de computação de elevado desempenho.
...
* A série Dv2 é a versão mais recente da série D e conta com uma CPU mais poderosa. A CPU da série Dv2 é cerca de 35% mais rápida do que a CPU da série D. Baseia-se no processador de última geração de 2,4 GHz Intel Xeon® E5-2673 v3 (Haskell), podendo chegar aos 3,2 GHz com o Intel Turbo Boost Technology 2.0. A série Dv2 tem as mesmas configurações de memória e disco da série D.
...
Nota: as VMs das séries DS e GS têm acesso ao Armazenamento Premium, o nosso armazenamento de baixa latência, elevado desempenho e baseado em SSD para as cargas de trabalho de E/S. O Armazenamento Premium está disponível em determinadas regiões. Para obter mais detalhes, veja:

* [Armazenamento Premium: High-Performance Storage for Azure Virtual Machine Workloads](../windows/premium-storage.md) (Armazenamento Premium: Armazenamento de elevado desempenho para cargas de trabalho de máquinas virtuais do Azure)
...
O Azure está a implementar o suporte para o [cloud-init](http://cloud-init.io/) em todas as Distribuições Linux que o suportem.  Atualmente, as VMs Ubuntu da Canonical são implementadas com o cloud-init ativado por predefinição.  Red Hat RHEL, CentOS e Fedora suportam o cloud-init; contudo, as imagens do Azure que o Red Hat mantém não o têm instalado.  Para utilizar o cloud-init numa família de SO Red Hat, tem de criar uma imagem personalizada com o cloud-init instalado.

* [Utilizar o cloud-init em VMs do Linux no Azure](using-cloud-init.md?toc=%2fazure%2fvirtual-machines%2flinux%2ftoc.json)
...
A Microsoft trabalha de perto com os nossos parceiros para garantir que as imagens disponíveis são atualizadas e otimizadas para runtimes do Azure.  Para obter mais informações sobre os nossos parceiros, veja as páginas do Marketplace abaixo.

* Linux no Azure - [Endorsed Distributions](endorsed-distros.md?toc=%2fazure%2fvirtual-machines%2flinux%2ftoc.json) (Distribuições Suportadas)
...
* [Criar uma VM do Linux através da CLI](quick-create-cli.md?toc=%2fazure%2fvirtual-machines%2flinux%2ftoc.json)
...
* [Criar uma VM do Linux através do portal](quick-create-portal.md?toc=%2fazure%2fvirtual-machines%2flinux%2ftoc.json)

### <a name="login-using-ssh-without-a-password"></a>Iniciar sessão através de SSH sem utilizar palavra-passe
...
* [Add a disk to a Linux VM using the azure-cli](add-disk.md?toc=%2fazure%2fvirtual-machines%2flinux%2ftoc.json) (Adicionar um disco a uma VM do Linux com a CLI do Azure)
* [How to attach a data disk to a Linux VM in the Azure portal](attach-disk-portal.md?toc=%2fazure%2fvirtual-machines%2flinux%2ftoc.json) (Como anexar um disco de dados a uma VM do Linux no portal do Azure)
...
* [Opening ports to a Linux VM in Azure](nsg-quickstart.md?toc=%2fazure%2fvirtual-machines%2flinux%2ftoc.json) (Abrir portas para uma VM do Linux no Azure)
* [Create a Fully Qualified Domain Name in the Azure portal](portal-create-fqdn.md?toc=%2fazure%2fvirtual-machines%2flinux%2ftoc.json) (Criar um Nome de Domínio Completamente Qualificado no portal do Azure)
...
* [Explore our growing list of Sample Scripts for common tasks via AzureCLI](cli-samples.md) (Explorar a crescente lista de Scripts de Exemplo para tarefas comuns através da CLI do Azure)
