...
ms.locfileid: "37033739"
---
# <a name="create-an-elastic-job-agent-using-powershell"></a>Criar um agente de Tarefa Elástica com o PowerShell

As [tarefas elásticas](elastic-jobs-overview.md) permitem a execução de um ou mais scripts Transact-SQL (T-SQL) em paralelo em muitas bases de dados.
...
Instale a pré-visualização mais recente do módulo **AzureRM.Sql** para obter os cmdlets da Tarefa Elástica. Execute os seguintes comandos numa linha de comandos elevada, (execute como administrador).
...
A criação de um agente de Tarefa Elástica requer uma base de dados (S0 ou superior) para utilização como [base de dados de Tarefa](elastic-jobs-overview.md#job-database). 
...
Para utilizar Tarefas Elásticas, registe a funcionalidade na sua subscrição do Azure ao executar o comando seguinte (isto apenas tem de ser executado uma vez em cada subscrição em que queira utilizar Tarefas Elásticas):
...
O cmdlet **New-AzureRmSqlElasticJobAgent** requer uma base de dados SQL do Azure já existente, pelo que os parâmetros *ResourceGroupName*, *ServerName* e *DatabaseName* têm de apontar todos para recursos existentes.
