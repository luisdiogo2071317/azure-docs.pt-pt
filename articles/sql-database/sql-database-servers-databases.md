---
title: Servidores lógicos de SQL do Azure e bases de dados individuais | Documentos da Microsoft
description: Saiba mais sobre o servidor lógico da SQL Database do Azure e conceitos de base de dados individual e os recursos.
...
ms.locfileid: "39091801"
---
# <a name="azure-sql-database-logical-servers-and-single-databases-and-their-resources"></a>Servidores de lógicos de base de dados SQL do Azure e bases de dados individuais e seus recursos
...
> [!TIP]
> Para nomes de grupo e o servidor de recursos válidos, veja [regras e restrições de nomenclatura](https://docs.microsoft.com/azure/architecture/best-practices/naming-conventions).
>
...
## <a name="logical-servers-and-databases"></a>Servidores lógicos e bases de dados

Num servidor lógico, pode criar:

- Uma base de dados criada dentro de um [grupo de recursos do Azure](../azure-resource-manager/resource-group-overview.md) com um [combinado de conjunto de recursos de computação e armazenamento](sql-database-service-tiers-dtu.md) ou um [escala independente de recursos de computação e armazenamento](sql-database-service-tiers-vcore.md). Uma base de dados SQL do Azure está associada um servidor lógico da SQL Database do Azure, que é criado dentro de uma região do Azure específica.
- Uma base de dados criada como parte de uma [conjunto de bases de dados](sql-database-elastic-pool.md) dentro de um [grupo de recursos do Azure](../azure-resource-manager/resource-group-overview.md) com um [combinado de conjunto de recursos de computação e armazenamento (baseados em DTU)](sql-database-service-tiers-dtu.md) ou um [dimensionamento independente de recursos de computação e armazenamento (baseados em vCore)](sql-database-service-tiers-vcore.md) que são partilhados entre todos os bancos de dados no conjunto. Uma base de dados SQL do Azure está associada um servidor lógico da SQL Database do Azure, que é criado dentro de uma região do Azure específica.

> [!IMPORTANT]
> SQL da base de dados a instância gerida, atualmente em pré-visualização pública, é uma [instância de um servidor SQL](sql-database-managed-instance.md) (uma instância gerida) criados dentro de um [grupo de recursos do Azure](../azure-resource-manager/resource-group-overview.md) com um conjunto definido de computação e armazenamento recursos para todas as bases de dados nessa instância do servidor. Uma instância gerida contém as bases de dados do sistema e do usuário. Instância gerida foi concebida para ativar a base de dados lift-and-shift para um PaaS totalmente gerido, sem ter de redesenhar a aplicação. Instância gerida oferece compatibilidade superior com o modelo de programação do SQL Server no local e oferece suporte a grande maioria das funcionalidades do SQL Server e de ferramentas e serviços que acompanha este artigo. Para obter mais informações, veja [Instância Gerida da Base de Dados SQL](sql-database-managed-instance.md). O restante deste artigo não é aplicável para a instância gerida.

## <a name="tds-and-tcpip-connections"></a>Ligações de rede recebido e TCP/IP

Base de dados de SQL do Microsoft Azure suporta a versão de cliente de protocolo do tabular data stream (TDS) 7.3 ou posterior e permite que apenas as ligações de TCP/IP encriptadas.
...
Para ajudar a proteger os seus dados, um [firewall de base de dados SQL](sql-database-firewall-configure.md) impede todo o acesso ao seu servidor de base de dados ou qualquer um dos seus bancos de dados de fora da sua ligação para o servidor diretamente por meio de sua ligação de subscrição do Azure. Para ativar a conetividade adicionais, terá [criar um ou mais regras de firewall](sql-database-firewall-configure.md#creating-and-managing-firewall-rules). Para criar e gerir conjuntos elásticos, veja [conjuntos elásticos](sql-database-elastic-pool.md).
