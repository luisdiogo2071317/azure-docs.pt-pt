...
Neste tutorial, executar consultas distribuídas entre o conjunto completo de inquilino bases de dados de relatórios. Estas consultas podem extrair insights buried dos dados operacionais diárias de inquilinos Wingtip bilhetes SaaS. Para tal, implementar uma base de dados de relatórios adicional para o servidor de catálogo e utilize elástico consulta para ativar as consultas distribuídas.
...
> * Como vistas globais em cada base de dados podem ativar a consultar eficiente entre inquilinos
...
1. No *ISE do PowerShell*, abra a... \\Learning módulos\\análise operacional\\Adhoc Reporting\\*demonstração AdhocReporting.ps1* script e defina o valor seguinte:
...
1. Expanda **bases de dados**, faça duplo clique _contosoconcerthall_e selecione **nova consulta**.
1. Execute as seguintes consultas para explorar a diferença entre as tabelas de inquilino único e as vistas globais:
...
Neste exercício implementa o _adhocreporting_ base de dados. Esta é a base de dados principal que contém o esquema utilizado para consultas em todas as bases de dados do inquilino. A base de dados é implementado o servidor de catálogo existente, o que é o servidor utilizado para todas as bases de dados relacionadas com a gestão na aplicação de exemplo.

1. no *ISE do PowerShell*, abra... \\Learning módulos\\análise operacional\\Adhoc Reporting\\*demonstração AdhocReporting.ps1*. 

1. Definir **$DemoScenario = 2**, _base de dados de relatórios implementar Ad-hoc_.
...
Neste exercício adiciona esquema (a origem de dados externa e definições de tabela externa) para o _adhocreporting_ base de dados para ativar a consulta em todas as bases de dados do inquilino.
...
2. Abrir ...\Learning Modules\Operational Analytics\Adhoc Reporting\ _inicializar AdhocReportingDB.sql_ no SSMS.
3. Reveja o script SQL e tenha em atenção:

   Consulta elástica utiliza uma credencial com âmbito de base de dados para aceder a cada uma das bases de dados do inquilino. Esta credencial tem de estar disponível em todas as bases de dados e devem normalmente concedidos direitos mínimos necessários para ativar estas consultas.

    ![Criar credencial](media/saas-tenancy-cross-tenant-reporting/create-credential.png)

   Com a base de dados de catálogo como a origem de dados externos, consultas são distribuídas para todas as bases de dados registadas no catálogo no momento que da consulta é executada. Como os nomes dos servidores são diferentes para cada implementação, este script obtém a localização da base de dados do catálogo do servidor atual (@@servername) onde o script é executado.
   ...
   A tabela local _VenueTypes_ que é criado e preenchido. Esta tabela de dados de referência é comum em todas as bases de dados de inquilino, pelo que pode ser representado aqui como uma tabela local e preenchidos com os dados comuns. Para algumas consultas, ter esta tabela definida na base de dados principal pode reduzir a quantidade de dados tem de ser movido para a base de dados principal.
...
4. Realce o *que venues estão atualmente registados?* consulta e prima **F5**.
   ...
   Inspecione o plano e vê que o custo total da consulta remota. Cada base de dados do inquilino executa a consulta remotamente e devolve as informações de venue na base de dados principal.

   ![SELECIONAR * de dbo. Venues](media/saas-tenancy-cross-tenant-reporting/query1-plan.png)
...
5. Selecione o seguinte consulta e prima **F5**.
   ...
   ![Associação de dados remota e local](media/saas-tenancy-cross-tenant-reporting/query2-plan.png)

6. Selecione agora o *no qual o dia foram mais pedidos de suporte de artigos vendidos?* consulta e prima **F5**.

   Esta consulta funciona associar um pouco mais complexas e a agregação. Ocorre a maior parte do processamento remotamente.  Apenas as linhas único, que contém permissão venda contagem diárias cada venue por dia, são devolvidas para a base de dados principal.
