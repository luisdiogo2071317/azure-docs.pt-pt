...
* [Resolução de problemas do lado do cliente](#client-side-troubleshooting) – Esta seção fornece diretrizes sobre a identificar e resolver problemas causados pela aplicação ligar à Cache do Azure para o Redis.
* [Resolução de problemas do lado do servidor](#server-side-troubleshooting) – Esta seção fornece diretrizes sobre a identificar e resolver problemas causados em Cache do Azure para o lado do servidor de Redis.
...
2. Aumente o tamanho da VM (para o cliente e Cache do Azure para o servidor Redis), para obter capacidades de largura de banda superior, reduzindo os tempos de transferência de dados de respostas maior. Obter mais largura de banda no apenas o servidor ou apenas no cliente pode não ser suficiente. Medir a utilização de largura de banda e compará-lo com as capacidades do tamanho da VM já existe.
...
#### <a name="resolution"></a>Resolução
* Reveja quaisquer conselhos e alertas mencionado na [Cache do Azure para o Assistente de Redis](cache-configure.md#azure-cache-for-redis-advisor).
...
1. Certifique-se de que a sua Cache do Azure para Redis e a aplicação de cliente estão na mesma região no Azure. Por exemplo, poderá ser tirando tempos limite quando a cache está na região E.U.A. leste, mas o cliente está na região E.U.A. oeste e o pedido não é concluído dentro do `synctimeout` intervalo ou pode estar prontificando tempos limite quando faz a depuração do seu computador de desenvolvimento local. 
   
    É altamente recomendável que a cache e o cliente na mesma região do Azure. Se tiver um cenário que inclui as chamadas de entre várias regiões, deve definir os `synctimeout` intervalo para um valor maior do que o intervalo de 1000 ms predefinido, incluindo um `synctimeout` propriedade na cadeia de ligação. O exemplo seguinte mostra um Cache de StackExchange.Azure para trecho de cadeia de ligação de Redis com um `synctimeout` de 2000 ms.
