...
ms.locfileid: "37063941"
---
# <a name="what-are-azure-reserved-vm-instances"></a>Quais são reservados instâncias de VM do Azure?
[Instâncias de VM do Azure reservado](https://azure.microsoft.com/pricing/reserved-vm-instances) ajuda a poupar dinheiro por pagar previamente um ano ou três anos de capacidade de cálculo, permitindo-lhe obter um desconto nas máquinas virtuais que utilizar. Reservado de instâncias do Azure podem reduzir significativamente os custos de máquina virtual — até 72 por cento num preços pay as you go – com o compromisso de compromisso de um ano ou três anos. Instâncias reservadas fornecem um desconto de faturação e não afetam o estado do tempo de execução das suas máquinas virtuais.

Pode comprar uma instância reservada (RI) [portal do Azure](https://aka.ms/reservations). Para obter mais informações, consulte [Prepay para máquinas virtuais e a poupar dinheiro com instâncias reservados](https://go.microsoft.com/fwlink/?linkid=861721).

## <a name="why-should-i-buy-a-reserved-instance"></a>Por que motivo deve a comprar uma instância reservada?
Se tiver máquinas virtuais executadas durante longos períodos de tempo, a compra de uma instância reservada dá-lhe a opção mais económica. Por exemplo, se executar continuamente quatro instâncias de uma VM D2 padrão na região EUA oeste, sem uma instância reservada-lhe é cobrados às taxas de pay as you go. Se comprar uma instância reservada os quatro VMs, as VMs obtêm imediatamente o benefício de faturação. Estes são já não é-lhe cobrados às taxas de pay as you go. 

## <a name="what-charges-does-a-reserved-instance-cover"></a>O que cobra é abrange uma instância reservado?
Uma instância reservada abrange apenas os custos de infraestrutura de máquina virtual para as máquinas de virtuais do Windows ou Linux. Uma instância reservada não abrange os encargos de armazenamento, redes ou software adicionais. Para máquinas virtuais do Windows, pode cobrir Windows licenciamento os custos com [Azure híbrida benefício](https://azure.microsoft.com/pricing/hybrid-benefit/).

## <a name="whos-eligible-to-purchase-a-reserved-instance"></a>Quem é elegível para comprar uma instância reservada?
Clientes do Azure com estes tipos de subscrições podem comprar uma instância reservada:
-   Enterprise agreement oferta tipo de subscrição (MS-AZR - 0017P).
-   [Pay as you go](https://azure.microsoft.com/offers/ms-azr-0003p/) o tipo de oferta de subscrição (MS-AZR - 003 P). Tem de ser a função de "Proprietário" na subscrição comprar uma instância reservada. Para a compra de instâncias reservadas de inscrição enterprise, o administrador de empresa tem de ativar compras de instância reservado no portal do EA. Por predefinição, esta definição está ativada.
-   Parceiros de fornecedor de solução (CSP) da nuvem podem utilizar o Portal do Azure ou [Centro de parceiros](https://docs.microsoft.com/partner-center/azure-reservations) comprar instâncias reservadas.
...
## <a name="how-is-the-purchased-reserved-instance-discount-applied"></a>Como é aplicado o desconto adquiridas instância reservado?
O desconto instância reservado aplica-se às máquinas virtuais que correspondam os atributos que selecionar ao adquirir a instância reservada. Os atributos incluem o âmbito em que executam as VMs correspondentes. Por exemplo, se pretender um desconto de instância reservado para quatro máquinas virtuais que D2 padrão na região EUA oeste, selecione a subscrição onde as VMs estão em execução. Se estiver a executar as máquinas virtuais em diferentes subscrições na sua conta/inscrição, em seguida, selecione o âmbito como partilhada. Âmbito partilhado permite que o desconto de instância reservado sejam aplicadas nas subscrições. Pode alterar o âmbito depois de comprar uma instância reservada. Para alterar o âmbito, consulte [gerir instâncias reservadas no Azure](billing-manage-reserved-vm-instance.md).
...
Para compreender melhor como instância reservada afeta a faturação da máquina virtual, consulte [compreender a forma como é aplicado o desconto instância reservado](billing-understand-vm-reservation-charges.md).
...
No final do período da instância reservado, o faturação desconto expira e a infraestrutura de máquina virtual é faturada a pay-como-lhe aceda preço. Reservado de instâncias do Azure não a renovação automática. Para continuar a introdução de desconto de faturação, tem de comprar uma nova instância reservada. 
...
- Quota suficiente – uma instância reservada que está confinada para uma única subscrição tem de ter vCPU quota disponível na subscrição para o novo RI. Por exemplo, se a subscrição de destino tem um limite de quota de 10 vCPUs para a série D, não é possível de comprar uma instância reservada para 11 Standard_D1 instâncias. A verificação de quota para instâncias reservadas inclui as VMs já implementadas na subscrição. Por exemplo, se a subscrição tem uma quota de 10 vCPUs para a série D e tiver duas instâncias de standard_D1 implementadas, em seguida, pode adquirir uma instância reservada para 10 standard_D1 instâncias nesta subscrição. 
- Restrições de capacidade – circunstâncias raros, os limites do Azure, a compra de instâncias novos reservadas para subconjunto de VM tamanhos, devido a baixa capacidade numa região.
...
- [Quais são reservados instâncias de VM do Azure?](../virtual-machines/windows/prepay-reserved-vm-instances.md)
