...
|/ suppressions/escrita|Cria/atualizações suppressions|
|ação de anular o registo /|Anula o registo da subscrição para a Microsoft Advisor|
...
| Operação | Descrição |
|---|---|
|checkAccess/ação|Verifica se o autor da chamada está autorizado a executar uma ação específica|
...
|ação de anular o registo /|Anula o registo da subscrição para o fornecedor de recursos do Batch que impede a criação de contas do Batch|
...
|clusters/escrita|Cria um novo cluster de AI do Batch ou atualiza um cluster existente de AI do Batch|
...
|/ fileservers/escrita|Cria um novo servidor de ficheiros de AI do Batch ou atualiza um servidor de ficheiros existente do Batch AI|
...
| Operação | Descrição |
|---|---|
|checknameavailability/ação|Verifica se um nome está disponível para utilização com uma nova Cache de Redis|
...
|imagens/escrita|Cria uma nova Imagem ou atualiza uma existente|
...
|/ orçamentos/escrita|Criar, atualizar e eliminar os orçamentos por uma subscrição ou um grupo de gestão.|
...
|os registos/escrita|Cria ou atualiza um registo de contentor com os parâmetros especificados.|
...
|ação de anular o registo /|Anula o registo da subscrição para o fornecedor de recursos de informações de cliente|
...
|ação de anular o registo /|Anula o registo da subscrição do fornecedor de recursos de Microsoft.DataCatalog.|
...
|/ servidores/escrita|Cria um servidor com os parâmetros especificados ou atualiza as propriedades ou etiquetas para o servidor especificado.|
...
|domínios/leitura|Obter o domínio|
...
|espaços de nomes/escrita|Crie um recurso de espaço de nomes e Atualize as respetivas propriedades. As etiquetas e a capacidade do espaço de nomes são as propriedades que podem ser atualizadas.|
...
|ação de anular o registo /|Regista o Fornecedor de Recursos do EventHub|
...
|/ Componentes/AnalyticsTables/escrita|Escrever um Application Insights analytics tabela esquema|
...
|/ Componentes/ExtendQueries/leitura|Componente de leitura Application Insights expandido os resultados da consulta|
...
|/ Componentes/FeatureCapabilities/leitura|Capacidades de funcionalidade do componente de leitura Application Insights|
...
|/ EventCategories/leitura|A ler uma categoria de eventos|
...
|/ Métricas/fornecedores/métricas/leitura|Ler métricas|
...
|/ Webtests/métricas/leitura|Ler uma métrica de teste Web|
|/Webtests/Read|A ler uma configuração de webtest|
|/ Webtests/escrita|A escrever para uma configuração de webtest|
...
|ação de anular o registo /|Cancela o registo de uma subscrição|
...
| Operação | Descrição |
|---|---|
|checknameavailability/ação|Verifica se está disponível um nome de conta de Media Services|
...
|dashboards/escrita|Adicionar ou modificar o dashboard para uma subscrição.|
...
|/ cofres replicationFabrics/replicationProtectionContainers/failoverCommit/replicationProtectedItems/ação|Consolidação de ativação pós-falha|
|/ cofres replicationFabrics/replicationProtectionContainers/plannedFailover/replicationProtectedItems/ação|Ativação pós-falha planeada|
|/ cofres replicationFabrics/replicationProtectionContainers/replicationProtectedItems/leitura|Ler os itens protegidos|
...
|/ cofres replicationFabrics/replicationProtectionContainers/repairReplication/replicationProtectedItems/ação|Reparar a replicação|
|/ cofres replicationFabrics/replicationProtectionContainers/Reproteção/replicationProtectedItems/ação|Proteja o Item protegido|
...
|/ cofres replicationFabrics/replicationProtectionContainers/testFailoverCleanup/replicationProtectedItems/ação|Limpeza da ativação pós-falha de teste|
|/ cofres replicationFabrics/replicationProtectionContainers/unplannedFailover/replicationProtectedItems/ação|Ativação pós-falha|
...
|/ cofres/replicationFabrics/replicationProtectionContainers/replicationProtectedItems/escrita|Criar ou atualizar quaisquer itens protegidos|
|/ cofres/replicationFabrics/replicationProtectionContainers/replicationProtectionContainerMappings/eliminar|Eliminar quaisquer mapeamentos de contentor de proteção|
|/ cofres replicationFabrics/replicationProtectionContainers/replicationProtectionContainerMappings/leitura|Ler quaisquer mapeamentos de contentor de proteção|
...
|/ cofres/replicationFabrics/replicationProtectionContainers/replicationProtectionContainerMappings/escrita|Criar ou atualizar quaisquer mapeamentos de contentor de proteção|
...
|/ cofres/replicationFabrics/replicationStorageClassifications/replicationStorageClassificationMappings/eliminar|Eliminar quaisquer mapeamentos de classificação de armazenamento|
|/ cofres replicationFabrics/replicationStorageClassifications/replicationStorageClassificationMappings/leitura|Ler quaisquer mapeamentos de classificação de armazenamento|
...
|ação de anular o registo /|Regista a subscrição do fornecedor de recursos de Reencaminhamento e ativa a criação de recursos de Reencaminhamento|
...
|/ pricings/escrita|Atualiza as definições de preços para o âmbito|
...
|/SKU/Read|Obter a lista de descrições de recursos de Sku|
|/sku/regions/read|Obter a lista de descrições de recurso SkuRegions|
|ação de anular o registo /|Regista a subscrição do fornecedor de recursos do ServiceBus e ativa a criação de recursos do ServiceBus|
...
|clusters/escrita|Criar ou Atualizar qualquer Cluster|
...
|aplicações/escrita|Cria uma aplicação.|
...
|/servers/virtualNetworkRules/write|Cria uma regra de rede virtual com os parâmetros especificados ou atualiza as propriedades ou etiquetas para a regra de rede virtual especificado.|
|/ servidores/escrita|Cria um servidor com os parâmetros especificados ou atualiza as propriedades ou etiquetas para o servidor especificado.|
|ação de anular o registo /|Anula o registo da subscrição para o fornecedor de recursos de base de dados do Microsoft SQL Server e permite a criação de bases de dados do Microsoft SQL Server.|
...
|customApis/escrita|Cria ou atualiza uma API personalizada.|
...
|/hostingEnvironments/multiRolePools/Providers/Microsoft.Insights/ metricDefinitions/leitura|Obtem as métricas disponíveis para MultiRole de ambiente de serviço de aplicações|
...
|cargas de trabalho/escrita|Escreve um recurso de carga de trabalho|

## <a name="next-steps"></a>Passos Seguintes

- Saiba como [criar uma função personalizada](custom-roles.md).
- Reveja o [RBAC funções incorporadas](built-in-roles.md).
- Saiba como gerir atribuições de acesso [por utilizador](role-assignments-users.md) ou [por recurso](role-assignments-portal.md) 
