---
title: Pontos finais de serviço de rede virtuais e as regras para o Service Bus do Azure | Microsoft Docs
...
ms.locfileid: "37036717"
---
# <a name="use-virtual-network-service-endpoints-with-azure-service-bus"></a>Utilizar pontos finais do serviço de rede Virtual com o Service Bus do Azure

A integração do Service Bus com [pontos finais do serviço de rede Virtual (VNet)] [ vnet-sep] permite um acesso seguro às capacidades de mensagens de cargas de trabalho como máquinas virtuais que estão vinculadas às redes virtuais , com o caminho de tráfego de rede que está a ser protegido em ambas as extremidades. 

Depois de ter configurado para ser vinculado a pelo menos um ponto final do serviço de sub-rede virtual de rede, o respetivo espaço de nomes do Service Bus já não irá aceitar o tráfego a partir de qualquer local, mas autorizado a rede virtual. Da perspetiva de rede virtual, um espaço de nomes de barramento de serviço de enlace para um ponto final de serviço configura um túnel de rede isolado de sub-rede da rede virtual para o serviço de mensagens.

O resultado é uma relação isolada e privada entre as cargas de trabalho vinculado à sub-rede e respetivo Service Bus espaço de nomes, mantenha apesar do endereço de rede observable do serviço de mensagens serviço ponto final que está a ser um intervalo de IP público.
...
## <a name="binding-service-bus-to-virtual-networks"></a>Barramento de serviço de enlace para redes virtuais

*Regras de rede virtual* são a funcionalidade de segurança de firewall que controla se o servidor do Service Bus do Azure aceita ligações a partir de uma sub-rede de rede virtual específico.

Um espaço de nomes de barramento de serviço de enlace a uma rede virtual é um processo de dois passos. Terá primeiro de criar um **ponto final do serviço de rede Virtual** numa sub-rede da rede Virtual e ative-o para "Microsoft.ServiceBus" como explicado no [descrição geral do ponto final de serviço] [ vnet-sep]. Depois de adicionar o ponto final de serviço, vincular o espaço de nomes do Service Bus ao mesmo com um *regra de rede virtual*.

A regra de rede virtual é uma associação com o nome do espaço de nomes de barramento de serviço com uma sub-rede de rede virtual. Enquanto a regra existe, todas as cargas de trabalho vinculadas à sub-rede concedidas acesso ao espaço de nomes do Service Bus. Barramento de serviço próprio nunca estabelece ligações de saída, não precisa de obter acesso e, por conseguinte, nunca terá acesso à sua sub-rede Ativando a esta regra.
