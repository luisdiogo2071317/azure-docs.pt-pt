...
1. Abra o ficheiro **MapTruckRoute.html** criado na secção anterior e adicione o seguinte código JavaScript ao bloco de *script*, para obter a rota para um camião utilizando o Route Service.
    ...
    Este fragmento de código também envia a consulta para o Route Service, para obter a rota para o ponto de início e de fim especificado, para a sua chave de conta. Os parâmetros opcionais seguintes são utilizados para indicar a rota de um camião:
   - O parâmetro `travelMode=truck` especifica o meio de transporte como *camião*. Outros meios de transporte suportados são *táxi*, *autocarro*, *carrinha*, *motociclo* e a predefinição *carro*.
   ...
   - O parâmetro `vehicleLoadType` classifica a carga como perigosa e restrita em algumas estradas. Atualmente, esta classificação também só é considerada para o meio *camião*.
