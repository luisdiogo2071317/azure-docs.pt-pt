...
Temos um fórum de MSDN que pode postar perguntas. A equipe do produto o fórum é monitorizado ativamente. O Fórum do URL é [ https://aka.ms/aml-forum-service ](https://aka.ms/aml-forum-service). 
...
- Tamanho da pasta de projeto máximo permitido: 25 MB.
    >[!NOTE]
    >Este limite não se aplica a `.git`, `docs` e `outputs` pastas. Estes nomes de pastas diferenciam maiúsculas de minúsculas. Se estiver a trabalhar com arquivos grandes, consulte [persistir alterações e lidar com arquivos grandes](how-to-read-write-files.md).
...
- Tamanho máx. de ficheiros registados no `outputs` pasta após uma execução: 512 MB
  - Isso significa que se o seu script produz um ficheiro mais de 512 MB na pasta saídas, ele não é coletado lá. Se estiver a trabalhar com arquivos grandes, consulte [persistir alterações e lidar com arquivos grandes](how-to-read-write-files.md).
