...
ms.locfileid: "48242968"
---
# <a name="troubleshoot-your-azure-machine-learning-service-deployments"></a>Resolver problemas relacionados com as implementações de serviços do Azure Machine Learning
...
Seguem-se as funções para utilizar quando interromper a implementação os passos:
...
Se o sistema não consegue criar a imagem do Docker, o `image.wait_for_creation` chamada falha com algumas mensagens de erro. Também pode encontrar mais detalhes sobre o erro de log da compilação de imagem. O uri de log é um URL de SAS que aponta para um ficheiro de registo armazenado no armazenamento de Blobs do Azure. Basta copiar e colar o uri numa janela do browser e podem transferir e ver o registo.
...
Depois da imagem é criada com êxito, o sistema tenta iniciar um contentor no ACI ou AKS consoante a configuração de implementação. É sempre recomendado para tentar uma implementação do ACI em primeiro lugar, uma vez que é uma implementação de contentor único simples. Dessa forma pode, então, excluir qualquer problema específico do AKS.
...
Se a imagem é criada com êxito, mas encontrar erros quando implantar a imagem como um contêiner, pode encontrar mensagem de erro no registo do Docker.
...
Agora, para sua janela da linha de comandos e o tipo de comandos para iniciar sessão no ACR (Azure Container Registry) associado à área de trabalho onde está armazenada a imagem a seguir.
...
```
Se não tiver a cli do azure instalada, também pode utilizar `docker login` comando para iniciar sessão para o ACR. Terá de obter o nome de utilizador e palavra-passe do ACR a partir do portal do Azure.

Agora pode obter a imagem do Docker e iniciar um contentor localmente e, em seguida, inicie uma sessão do bash para depuração.
...
Muitas vezes, o `init()` função no script de classificação, `Model.get_model_path()` função é chamada para localizar um ficheiro de modelo ou de uma pasta de ficheiros no contentor. Geralmente essa é uma origem da falha se não é possível localizar o ficheiro de modelo ou pasta. A maneira mais fácil para depurar este erro é executar o abaixo o código de Python no shell do contentor:
...
Isso seria imprimir o caminho local que o seu script de classificação está esperando localizar o ficheiro de modelo ou a pasta. Em seguida, pode verificar se o ficheiro ou pasta está, de fato, onde é esperado que seja.


## <a name="function-fails-runinputdata"></a>Falha de função: run(input_data)
Se o serviço é implementado com êxito, e ele falha quando postar dados para o ponto final de classificação, pode adicionar erro capturando instrução em seu `run(input_data)` função gerar a mensagem de erro. Por exemplo:
