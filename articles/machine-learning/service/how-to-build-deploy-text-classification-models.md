---
title: Criar e implementar um modelo de classificação de texto utilizando o pacote do Azure Machine Learning para análise de texto.
description: Saiba como criar, dar formação, testar e implementar um modelo de classificação de texto utilizando o pacote do Azure Machine Learning para análise de texto.
...
Neste artigo, pode saber como preparar e implementar um modelo de classificação de texto com **Azure Machine Learning pacote para a análise de texto** (AMLPTA). O objetivo da classificação de texto é atribuir um fragmento de texto para uma ou mais predefinidas classes ou categorias. Este texto, por exemplo, pode um documento, artigo de notícias, consulta de pesquisa, e-mail, tweet, pedidos de suporte.

Existem aplicações abrangentes de classificação de texto, tal como: 
+ Categorizar os artigos de newspaper e conteúdo de transmissão de notícias de última hora para tópicos
+ Organizar páginas web em categorias hierárquicas 
+ Filtragem de spam de e-mail
+ Análise de sentimentos
+ Prever intenção do utilizador das consultas de pesquisa
+ Encaminhamento de pedidos de suporte
+ Analisar a comentários de clientes 

O texto classificação modelo edifício e implementação do fluxo de trabalho para um modelo com AMLPTA é o seguinte:
...
Consulte o [documentação de referência do pacote](https://aka.ms/aml-packages/text) para a referência de detalhado para cada módulo e uma classe.
...
1. Deve ser instalado o pacote do Azure Machine Learning para análise de texto. Saiba como [instalar este pacote aqui](https://aka.ms/aml-packages/text).
...
Experimente por si. Transfira o bloco de notas e executá-lo por si.
...
O exemplo seguinte utiliza o [20 newsgroups dataset](http://qwone.com/~jason/20Newsgroups/) que está disponível através de scikit-saiba biblioteca para demonstrar como criar um classificador de texto com o pacote do Azure Machine Learning para análise de texto. 

O conjunto de dados de 20 newsgroups tem cerca 18,000 newsgroups publicações no 20 diferentes tópicos divididos em duas subconjuntos: um para formação e a outra uma avaliação de desempenho. A divisão para formação e teste baseia-se após a data de publicação cada mensagem se antes ou após uma data específica.
...
Defina a localização onde transferiu os dados no parâmetro dir dados. Também pode utilizar os seus próprios dados, o conjunto de dados de entrada tem de ser um formato de ficheiro *.tsv.
...
```
 ...
 Os dados constituem em etiqueta e texto
...
<table border="1" class="dataframe">
  ...
  <tbody>
    ...
    <tr>
      <th>1</th>
      <td>5</td>
      <td>De: Rick Santos &lt; rick@ee.uwm.edu &gt;\nSubject:...</td>
    </tr>
    <tr>
      <th>2</th>
      <td>0</td>
...
Obter o correspondance entre categorias e o respetivo nome.
...
Agora, pode criar um histograma de desenho de exploração preliminar da frequência de classe na formação e testar conjuntos de dados. 
...
Ao executar o bloco de notas Jupypter, rastreia é apresentada depois do bloco de código anterior é executado.


## <a name="train-the-model"></a>Dar formação sobre o modelo

### <a name="specify-scikit-learn-algorithm-and-define-the-text-classifier"></a>Especifique scikit-saiba algoritmo e definir o classificador de texto

Este passo envolve a preparação um scikit-saber o modelo de classificação de texto utilizando um versus Rest algoritmo de regressão logística da aprendizagem.

Para obter uma lista completa de learnings, consulte o [Scikit Learners](http://scikit-learn.org/stable/supervised_learning) documentação.
...
### <a name="examine-and-set-the-parameters-of-the-different-pipeline-steps"></a>Examine e definir os parâmetros dos passos pipeline diferente

Embora o ajuste de um scikit-saiba modelo pré-processamentos está a ser efetuada antes do ajuste através de um pipeline no pré-processador e passos de featurizer (transformação). Por conseguinte, a referência a um "pipeline" de formação. Durante a avaliação, o pipeline completo, incluindo pré-processamentos e scikit-saber de predição de modelos, é aplicada a um conjunto de dados de teste.

***Exemplo apresentado com text_word_ngrams*** 

Normalmente, defina os parâmetros antes de ajustar um modelo. 
...
Para ver os parâmetros estão incluídos para "text_word_ngrams", utilize [get_step_param_names_by_name](https://docs.microsoft.com/python/api/tatk.core.base_text_model.basetextmodel). Esta função devolve os parâmetros, tais como em minúsculas, input_col, output_col e assim sucessivamente. 
...
Se necessário, pode alterar os parâmetros de predefinição.  Com o código seguinte, pode alterar o intervalo de carácter extraídos n-grams de (4,4) para (3,4) para extrair os carateres tri-grams e 4 grams:
...
Se for necessário, pode otimizar o desempenho de modelo executando novamente o modelo passo com parâmetros revistos de ajuste:
...
Aplicar o classificador de texto treinado o conjunto de dados de teste para gerar predições de classe:
...
O [módulo avaliação](https://docs.microsoft.com/python/api/tatk.evaluation) avalia a precisão do classificador de texto treinado no conjunto de dados de teste. A função de evaluate gera uma matriz de confusão e fornece uma pontuação de macro F1.
...
Desenhar a confusão sem matriz de normalização para visualização.
...
Ao executar o bloco de notas a matriz de confusão será apresentada



Desenhar a matriz de confusão normalizado para visualização.
...
Guarde o pipeline de classificação para um ficheiro zip. Além disso, guarde os n-grams word ngrams e o caráter como ficheiros de texto.
...
As classificação pipeline e a palavra-ngrams e o caráter n-grams para inferencing de carga:
...
Avaliar um conjunto de dados de teste, aplique o pipeline de classificação de texto carregar:
...
Nesta secção, implementar o pipeline de classificação de texto como um através de serviço web do Azure Machine Learning [CLI do Azure Machine Learning](https://docs.microsoft.com/azure/machine-learning/desktop-workbench/cli-for-azure-machine-learning). Em seguida, consumir o serviço web para formação e classificação.
...
1. Certifique-se de que o seu ambiente do Azure Machine Learning, a conta de gestão de modelo e o grupo de recursos estão localizados na mesma região.

1. Transfira o ficheiro de configuração de implementação do armazenamento de BLOBs e guardá-lo localmente:
...
1. Atualize o ficheiro de configuração de implementação que transferiu para refletir os recursos:
...
1. Uma vez que o modelo treinado é implementado com êxito, invocar o serviço web de classificação no novo conjunto de dados:
...
1. O serviço web de teste com o corpo de mensagens de correio dois eletrónico obtidas a partir do conjunto de dados 20 newsgrpoups:
...
+ Leia o [descrição geral do pacote e aprender a instalá-lo](https://aka.ms/aml-packages/text).

+ Explorar o [documentação de referência](https://aka.ms/aml-packages/text) para este pacote.
