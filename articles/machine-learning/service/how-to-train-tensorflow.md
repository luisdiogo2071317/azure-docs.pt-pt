...
ms.locfileid: "46974226"
---
# <a name="how-to-train-tensorflow-models"></a>Como utilizar modelos do TensorFlow

Para aprender de rede neurais profundas (DNN) com o TensorFlow, Azure Machine Learning fornece uma classe personalizada do TensorFlow do avaliador. O Azure SDK TensorFlow Estimator (para não ser conflated com o [ `tf.estimator.Estimator` ](https://www.tensorflow.org/api_docs/python/tf/estimator/Estimator) classe) permite-lhe submeter facilmente tarefas de formação de TensorFlow para execuções de nó único e distribuídas na computação do Azure.

## <a name="single-node-training"></a>Treinamento de nó único
Treinamento com o avaliador de TensorFlow é semelhante à utilização a [estimador de base](how-to-train-ml-models.md), leia o artigo que mostra como primeiro e certifique-se de que compreende os conceitos apresentados aqui.
...
* `conda_packages`: A lista de pacotes de Python a serem instalados por meio de conda necessário ao seu script de treinamento. Nesse caso, o script de treinamento usa `sklearn` para carregar os dados, pelo que deve especificar esse pacote a ser instalado.  
...
1. Com base em MPI distribuídas de treinamento com o [Horovod](https://github.com/uber/horovod) framework
2. nativo [distribuídas do TensorFlow](https://www.tensorflow.org/deploy/distributed) por meio do método de servidor de parâmetro
...
* `process_count_per_node`: O número de processos (ou "funcionários") para executar em cada nó. Este argumento assume a predefinição `1`
...
* `worker_count`: O número de funções de trabalho. Este argumento assume a predefinição `1`
* `parameter_server_count`: O número de servidores de parâmetro. Este argumento assume a predefinição `1`
* `distributed_backend`: O back-end a utilizar para formação distribuída. Este argumento é predefinido para `None`. Para fazer o treinamento distribuído por meio do servidor de parâmetros, precisará definir `distributed_backend='ps'`
