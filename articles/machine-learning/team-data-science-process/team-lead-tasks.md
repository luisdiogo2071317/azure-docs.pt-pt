...
>[AZURE.NOTE] As tarefas em blocos de 1 e 2 da figura são necessários se estiver a utilizar o Visual Studio Team Services (VSTS) como o código de plataforma de hospedagem e queira ter um projeto de equipe separadas para a sua própria equipa. Assim que estas tarefas são concluídas, todos os repositórios de sua equipe podem ser criados sob este projeto de equipe. 
...
1. Criar uma **projeto de equipe** no servidor do VSTS do grupo do grupo e dois repositórios de equipe no projeto:
...
- **R1**: A **GroupProjectTemplate** repositório de Git que o seu Gestor de Grupo configurar no seu servidor de grupo do VSTS.
...
Os nomes especificados para os repositórios e diretórios neste tutorial foram fornecidos no pressuposto de que o objetivo é estabelecer um projeto de equipe separadas para a sua própria equipa dentro de um grupo de ciência de dados maior. Mas existem outras opções abertas a como líder de equipe:

- O grupo inteiro pode optar por criar um projeto de equipe único. Em seguida, todos os projetos de todas as equipas de ciência de dados seria sob este projeto de equipa única. Para conseguir isso, pode designar um administrador de git de seguir estas instruções para criar um projeto de equipe único. Este cenário poderá ser válido, por exemplo, para:
...
- As equipes podem optar por utilitários de específicas para a equipe sob o projeto de equipe único para o grupo inteiro ou de modelos de projeto de equipe específico. Neste caso, os líderes de equipe devem criar repositórios de modelo de projeto de equipe e/ou de repositórios de utilitários de equipe sob o mesmo projeto de equipe. Dê um nome estes repositórios *< TeamName\>ProjectTemplate* e *< TeamName\>utilitários*, por exemplo, *TeamJohnProjectTemplate*e *TeamJohnUtilities*. 

De qualquer forma, os líderes de equipe precisam permitir que os membros da sua equipa saber quais repositórios de modelo e os utilitários a adotar quando eles são a configuração e a clonagem de repositórios do projeto e utilitários. Oportunidades potenciais do projeto devem seguir a [tarefas líder de projeto para uma equipa de ciência de dados](project-lead-tasks.md) para criar repositórios de projeto, se em projetos de equipe separado ou num projeto de equipe único. 
...
- Sua **servidor do grupo VSTS** (ou conta de grupo em algum outro código de plataforma de alojamento) tiver sido configurado pelo seu Gestor de grupo.
...
- Se estiver a utilizar **DSVM do Linux**, crie uma chave pública SSH na sua DSVM e adicioná-lo ao seu servidor do VSTS de grupo. Para obter mais informações sobre o SSH, consulte a **criar chave pública SSH** secção a [apêndice de ferramentas e plataformas](platforms-and-tools.md#appendix). 
    
## <a name="1-create-a-team-project-and-repositories"></a>1. Criar um projeto de equipe e repositórios

Conclua este passo se estiver a utilizar o VSTS como seu código de plataforma para colaboração e controlo de versões de alojamento. Esta secção tem de criar três artefactos no servidor do VSTS, do seu grupo de:
...
- Vá para a página inicial do servidor VSTS do seu grupo no URL `https://<VSTS Server Name\>.visualstudio.com`. 
- Clique em **New** para criar um projeto de equipe. 

    ![2](./media/team-lead-tasks/team-leads-2-create-new-team.png)

- Uma janela de projeto de equipe criar pede-lhe para introduzir o nome do projeto (**MyTeam** neste exemplo). Certifique-se de que seleciona **Agile** como o **modelo de processo** e **Git** como o **controle de versão**. 

    ![3](./media/team-lead-tasks/team-leads-3-create-new-team-2.png)

- Clique em **criar projeto**. Seu projeto de equipe **MyTeam** é criado em menos de 1 minuto. 

- Depois do projeto de equipe **MyTeam** é criado e clique em **Navigate ao projeto** botão, para ser direcionado para a home page de seu projeto de equipe. 

    ![4](./media/team-lead-tasks/team-leads-4-create-new-team-3.png)

- Se vir um **Parabéns!** janela de pop-up, clique nas **adicione código** (botão na caixa vermelha). Caso contrário, clique em **código** (na caixa amarela). Isto direciona-o para a página de repositório de Git de seu projeto de equipe. 

    ![5](./media/team-lead-tasks/team-leads-5-team-project-home.png)

### <a name="create-the-myprojecttemplate-repository-r3-on-git"></a>Criar o repositório de MyProjectTemplate (R3) no Git

- Na página de repositório de Git de seu projeto de equipe, clique na seta para baixo junto ao nome do repositório **MyTeam**e selecione **gerir repositórios...** .

    ![6](./media/team-lead-tasks/team-leads-6-rename-team-project-repo.png)

- Sobre o **controle de versão** separador do painel de controlo do seu projeto de equipe, clique em **MyTeam**, em seguida, selecione **repositório de mudança de nome...** . 
...
- Para criar um novo repositório *< nome da sua equipe\>utilitários* em seu projeto de equipe, clique em **novo repositório de...**  sobre o **controle de versão** separador do painel de controlo do seu projeto de equipe.  
...
- Confirme que vê dois repositórios Git novo criados em seu projeto de equipe **MyTeam**. Neste exemplo: 
...
## <a name="2-seed-your-team-projecttemplate-and-teamutilities-repositories"></a>2. Efetuar o seeding seus repositórios ProjectTemplate e TeamUtilities da Equipe
...
Neste passo, inicializa seu repositório de modelos de projeto de equipe do repositório de modelos de projeto do grupo:
...
Estes comandos clonagem seu **GroupProjectTemplate** repositório (R1) no seu servidor do VSTS de grupo para um diretório local no **GitRepos\GroupCommon** no seu computador local. Após a clonagem, diretório **GroupProjectTemplate** (D1) é criado no diretório **GitRepos\GroupCommon**. Aqui, partimos do princípio de que o seu Gestor de grupo criado um projeto de equipe **GroupCommon**e o **GroupProjectTemplate** repositório está sob este projeto de equipe. 


### <a name="clone-your-team-repositories-into-local-directories"></a>Clone os repositórios de equipe para diretórios locais

Estes comandos clonagem seu **MyTeamProjectTemplate** (R3) e **MyTeamUtilities** repositórios (R4) em seu projeto de equipe **MyTeam** no seu servidor do VSTS de grupo para o  **MyTeamProjectTemplate** (D3) e **MyTeamUtilities** diretórios (D4) numa **GitRepos\MyTeam** no seu computador local. 
...
Após a clonagem, dois diretórios **MyTeamProjectTemplate** (D3) e **MyTeamUtilities** (D4) são criados no diretório **GitRepos\MyTeam**. Pressupomos aqui denominado a seu projeto de equipe repositórios de modelo e utilitários **MyTeamProjectTemplate** e **MyTeamUtilities**. 

### <a name="copy-the-group-project-template-content-to-the-local-team-project-template-directory"></a>Copie o conteúdo de modelo de projeto de grupo para o diretório de modelos de projeto de equipe local
...
### <a name="customize-your-team-project-template-or-team-utilities-optional"></a>Personalizar o modelo de projeto de equipe ou utilitários de equipa (opcionais)
...
Os ficheiros no repositório MyTeamProjectTemplate do servidor VSTS do seu grupo são sincronizados quase instantaneamente quando este script é executado.
...
É uma boa prática para verificar neste ficheiro de texto no seu repositório de ProjectTemplate de equipe. Recomendamos que coloque no diretório **Docs\DataDictionaries**. Por conseguinte, este recurso de dados pode ser acedido por todos os projetos da sua equipa. 
...
A partir da home page do seu servidor VSTS de grupo, clique nas **ícone de engrenagem** junto ao seu nome de utilizador no canto superior direito, em seguida, selecione a **segurança** separador. Pode adicionar membros à sua equipe com várias permissões.
