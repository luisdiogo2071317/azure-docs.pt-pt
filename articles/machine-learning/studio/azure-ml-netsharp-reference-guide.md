---
title: 'NET # neuronal guia de idioma de especificação - de redes do Azure Machine Learning | Microsoft Docs'
description: 'Sintaxe para o Net # neuronal redes idioma especificação, juntamente com os exemplos de como criar um modelo de rede neuronal personalizada utilizando Net #'
...
NET # é um idioma desenvolvido pela Microsoft que é utilizado para definir arquiteturas de rede neuronal. Com o Net # para definir a estrutura de uma rede neuronal possibilita definir estruturas complexas, tais como as redes neurais profundo ou convolutions de dimensões arbitrários, o que se sabe para melhorar a aprendizagem dados, tais como as vídeo, áudio ou imagem.

Pode utilizar uma especificação de arquitetura do Net # estes contextos:

+ Todos os módulos de rede neuronal no Microsoft Azure Machine Learning Studio: [várias classes a rede neuronal](https://docs.microsoft.com/azure/machine-learning/studio-module-reference/multiclass-neural-network), [duas classes a rede neuronal](https://docs.microsoft.com/azure/machine-learning/studio-module-reference/two-class-neural-network), e [regressão de rede neuronal](https://docs.microsoft.com/azure/machine-learning/studio-module-reference/neural-network-regression)
+ Funções de rede neuronal no MicrosoftML: [NeuralNet](https://docs.microsoft.com/machine-learning-server/r-reference/microsoftml/neuralnet) e [rxNeuralNet](https://docs.microsoft.com/machine-learning-server/r-reference/microsoftml/rxneuralnet)para o idioma de R e [rx_neural_network](https://docs.microsoft.com/machine-learning-server/python-reference/microsoftml/rx-neural-network) para Python.


Este artigo descreve os conceitos básicos e a sintaxe necessária para desenvolver uma rede neuronal personalizada com Net #: 
...
Uma estrutura de rede neuronal consiste em nós que se encontram organizados em camadas e ligações ponderadas (ou margens) entre os nós. As ligações são direcional e cada ligação tem um nó de origem e de um nó de destino.  

Cada camada trainable (um oculta ou uma camada de saída) tem um ou mais **pacotes ligação**. Um pacote de ligação é constituída por uma camada de origem e uma especificação de ligações de camada dessa origem. Todas as ligações num determinado pacote partilham a mesma camada de origem e a mesma camada de destino. Em Net #, um pacote de ligação é considerado como pertencendo à camada de destino do pacote.
...
Para especificar que o número de nós na camada de uma determinado automaticamente, utilize o `auto` palavra-chave. O `auto` palavra-chave tem efeitos diferentes, consoante a camada:
...
Imediatamente depois de definir a camada trainable, tem de declarar as ligações entre as camadas definidas. A declaração de pacote de ligação começa com a palavra-chave `from`, seguido do nome de camada de origem do pacote e o tipo de pacote de ligação para criar.

Atualmente, são suportados os cinco tipos de pacotes de ligação:

+ **Completo** pacotes, indicados pela palavra-chave `all`
+ **Filtrados** pacotes, indicados pela palavra-chave `where`, seguido de uma expressão de predicado
...
+ **Preenchimento**: (opcional) determina se a entrada deve ser preenchido até ao utilizar um **esquema de preenchimento predefinido**. O valor pode ser um valor booleano único ou pode ser uma cadeia de identificação dos valores booleanos com um comprimento aridade do pacote. 

    Um valor booleano único é expandido para ser uma cadeia de identificação do comprimento correto com todos os componentes igual ao valor especificado. 
    ...
    Se o valor para uma dimensão é False, os kernels são definidas para que o número de nós em cada lado que restam saída é o mesmo (até uma diferença de 1). O valor predefinido deste atributo é uma cadeia de identificação com todos os componentes igual como False.

+ **UpperPad** e **LowerPad**: (opcional) forneça maior controlo sobre a quantidade de preenchimento para utilizar. **Importante:** estes atributos podem ser definido se e apenas se o **preenchimento** propriedade acima é ***não*** definido. Os valores devem ser cadeias de identificação valor de número inteiro com comprimentos são aridade do pacote. Quando estes atributos são especificados, são adicionados nós "fictícias" para as extremidades superiores e inferiores de cada dimensão da camada de entrada. O número de nós adicionados a terminar o inferior e superior em cada dimensão é determinado pelo **LowerPad**[i] e **UpperPad**[i], respetivamente. 

    Para garantir que kernels correspondem apenas a nós "verdadeiro" e não a nós "fictícias", as condições seguintes têm de ser cumpridas:
      - Cada componente do **LowerPad** tem de ser estritamente menor `KernelShape[d]/2`. 
      - Cada componente do **UpperPad** tem de ser maior do que `KernelShape[d]/2`. 
      - O valor predefinido destes atributos é uma cadeia de identificação com todos os componentes iguais a 0. 

    A definição **preenchimento** = true permite a maior quantidade preenchimento conforme necessário para manter o "Centro" de kernel dentro "real" de entrada. Isto altera as contas um pouco para calcular o tamanho de saída. Geralmente, o tamanho de saída *D* é calculada como `D = (I - K) / S + 1`, onde `I` é o tamanho de entrada, `K` é o tamanho de kernel, `S` é stride, e `/` é (arredondar na direção de 0 a divisão de número inteiro ). Se definir UpperPad = [1, 1], o tamanho de entrada `I` é, efetivamente, 29 e, consequentemente, `D = (29 - 5) / 2 + 1 = 13`. No entanto, quando **preenchimento** = true, essencialmente `I` obtém bumped cópias de segurança por `K - 1`; por conseguinte, `D = ((28 + 4) - 5) / 2 + 1 = 27 / 2 + 1 = 13 + 1 = 14`. Ao especificar valores para **UpperPad** e **LowerPad** obter muito mais controlo sobre o preenchimento que o se basta definir **preenchimento** = true.
...
+ A aridade do pacote é 3: ou seja, o comprimento das cadeias de identificação `InputShape`, `KernelShape`, e `Stride`. 
...
**Normalização de resposta** é um esquema de normalização local que foi introduzida pela primeira vez ao Geoffrey Hinton, usados de ter, no documento [ImageNet Classiﬁcation com as redes Neurais Convolutional profundas](http://www.cs.toronto.edu/~hinton/absps/imagenet.pdf). 
...
![fórmula para a estrutura convolutional](./media/azure-ml-netsharp-reference-guide/formula_large.png)
...
> [!NOTE]
> Os nós na camada de destino correspondem a neurons que são nós centrais dos kernels. Por exemplo, se `KernelShape[d]` for ímpar, em seguida, `KernelShape[d]/2` corresponde ao nó de central kernel. Se `KernelShape[d]` é mesmo, o nó central em `KernelShape[d]/2 - 1`. Por conseguinte, se `Padding[d]` é False, o primeiro e último `KernelShape[d]/2` nós não tem nós correspondentes na camada de destino. Para evitar esta situação, definir **preenchimento** como [true, true,..., true].
...
+ A primeira linha define a camada de entrada (com o nome `Data`). Quando utiliza o `auto` palavra-chave, a rede neuronal inclui automaticamente todas as colunas de funcionalidade nos exemplos a entrada. 
...
+ A terceira linha define a camada de saída (com o nome `O`), que inclui 10 nós de saída. Se a rede neuronal é utilizada para classificação, é um nó de saída por classe. A palavra-chave **sigmoid** indica que a função de saída é aplicada à camada de saída.
...
+ A estrutura tem uma camada única de entrada, `Image`.
...
+ A camada de saída, `Digit`, está ligado apenas à terceira camada oculta, `Hid3`. A palavra-chave `all` indica que a camada de saída está completamente ligada à `Hid3`.
+ A aridade do convolution é três: o comprimento das cadeias de identificação `InputShape`, `KernelShape`, `Stride, and `partilha '. 
...
+ Porque `Sharing[d]` for False apenas para `d == 0`, é o número de kernels `MapCount * NodeCount\[0] = 10 * 5 = 50`. 
