...
+ A terceira linha define a camada de saída (com o nome `O`), que contém 10 nós de saída. Se a rede neural é utilizada para classificação, existe um nó de saída por classe. A palavra-chave **sigmoide** indica que a função de saída é aplicada para a camada de saída.
