...
1. Iniciar o aprovisionamento de um cluster, utilizando os passos em [clusters do HDInsight baseado em Linux aprovisionar](hdinsight-hadoop-create-linux-clusters-portal.md). Certifique-se de criar o cluster com o **personalizado** fluxo de criação do cluster. O cluster tem de cumprir os seguintes requisitos.
    ...
    * Terá de utilizar armazenamento do Azure que o arquivo de dados. Utilizar o Presto no cluster que utilize o armazenamento do Azure Data Lake como a opção de armazenamento ainda não é uma opção.
...
2. Sobre o **definições avançadas** área, selecione **ações de Script**e forneça as informações abaixo. Também pode escolher a opção "Instalar Presto" para o tipo de script.
...
6. Depois das alterações são aplicadas à configuração do cluster, pode acessar o Airpal interface da web utilizando os seguintes passos no [portal do Azure](https://portal.azure.com):
    ...
    1. Partir do **aplicações instaladas** página, local a entrada de tabela para **airpal**e selecione **Portal**.
    ...
    1. Quando lhe for pedido, introduza as credenciais de administrador que especificou ao criar o cluster de Hadoop do HDInsight.
...
* [Instalar e utilizar Hue em clusters do HDInsight](hdinsight-hadoop-hue-linux.md). Hue é uma interface de Usuário que torna mais fácil criar, executar e guardar Apache Pig e tarefas do Hive do web.
