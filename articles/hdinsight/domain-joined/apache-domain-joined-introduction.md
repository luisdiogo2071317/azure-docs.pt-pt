---
title: Uma introdução à segurança do Hadoop com clusters do Azure HDInsight associado a um domínio
description: Saiba como associados a um domínio o Azure HDInsight clusters suportam os quatro pilares da segurança corporativa.
services: hdinsight
author: omidm1
manager: jhubbard
editor: cgronlun
...
ms.locfileid: "39112541"
---
# <a name="an-introduction-to-hadoop-security-with-domain-joined-hdinsight-clusters"></a>Uma introdução à segurança do Hadoop com clusters do HDInsight associados a um domínio

No passado, o Azure HDInsight suportado apenas um único utilizador: administrador local. Isto resultava muito bem para equipas ou departamentos de aplicações mais pequenos. Como cargas de trabalho baseadas no Hadoop ganhavam mais popularidade no setor empresarial, a necessidade para suportam capacidades de nível empresarial, como a autenticação baseada no Active Directory, multiutilizador e controlo de acesso baseado em funções se tornou cada vez mais importante. 

Pode criar um cluster do HDInsight associado a um domínio do Active Directory. Em seguida, pode configurar uma lista de funcionários da empresa que podem autenticar através do Azure Active Directory para iniciar sessão no cluster do HDInsight. Qualquer pessoa fora da empresa não pode iniciar sessão nem aceder ao cluster do HDInsight. 
...
> [!NOTE]
> As novas funcionalidades descritas neste artigo estão disponíveis em pré-visualização apenas nos seguintes tipos de cluster: Hadoop, Spark e a consulta interativa. Oozie está agora ativado em clusters associados a um domínio. Para aceder à IU da web de Oozie, devem ativar os utilizadores [túnel](../hdinsight-linux-ambari-ssh-tunnel.md).

Segurança empresarial contém quatro pilares principais: segurança de perímetro, autenticação, autorização e encriptação.

![Benefícios dos clusters do HDInsight associados a um domínio nos quatro pilares da segurança empresarial](./media/apache-domain-joined-introduction/hdinsight-domain-joined-four-pillars.png).
...
Um administrador empresarial pode criar um cluster do HDInsight associados a um domínio num [rede virtual](https://azure.microsoft.com/services/virtual-network/). Todos os nós do cluster do HDInsight são associados ao domínio gerido pela empresa. Isto é conseguido através da utilização de [do Azure Active Directory Domain Services](../../active-directory-domain-services/active-directory-ds-overview.md). 
...
Um cluster do HDInsight associados a um domínio utiliza a IU familiar do Ranger Apache para procurar registos de auditoria. No back-end, o Ranger utiliza [Apache Solr](http://hortonworks.com/apache/solr/) para armazenar e procurar os registos.
...
* [Planear clusters do HDInsight associados a um domínio](apache-domain-joined-architecture.md)
