---
title: Cliente de autoteste previamente validar uma aplicação SaaS do Azure | Documentos da Microsoft
description: Como criar um cliente autoteste para pré-validar uma aplicação SaaS do Azure.
...
---
# <a name="create-a-self-test-client-to-pre-validate-an-azure-saas-application"></a>Criar um cliente de autoteste de mensagens em fila para pré-validar uma aplicação SaaS do Azure
...
A API tem a seguinte estrutura.
...
|          Nome de DNS          |         O nome de DNS da VM que pretende testar.          |
|          Utilizador          |          Nome de utilizador para estabelecer ligação e o início de sessão numa máquina virtual.         |
|           Palavra-passe         |         A palavra-passe para ligar a e o início de sessão VM.          |
|          SO          |         O sistema operativo da VM, "Linux" ou "Windows".          |
|         PortNo           |         O número de porta aberta para ligar à VM. O número de porta é, normalmente, "22" para Linux e "5986" para o Windows.          |
...
Captura de ecrã seguinte mostra um exemplo dos resultados de teste JSON visualizados num Visualizador JSON online.
...
A chamada da API com o cURL, siga estes passos:
...
20. Criar uma chave secreta ao selecionar a chave **Descrição** caixa de texto. Configure os seguintes campos:
    ...
    - Sob **valor**, copie a chave.
     ...
     >[!Important]
     >Não será capaz de ver o valor da chave depois de sair do formulário de chaves.
...
Depois de o testar a aplicação SaaS do Azure com êxito, pode [publicar a oferta](./cpp-publish-offer.md)