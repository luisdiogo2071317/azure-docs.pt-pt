...
O Azure Marketplace e AppSource são pontos de iniciar conjunta mercado aceda atividades com a Microsoft e um flywheel crescimento de negócio. Ao utilizar a promoção de iniciação, geração a pedido e conjunta venda e marketing, pode efetuar as listagens de marketplace centerpiece do motor de negócio a nuvem. Não existem não existem taxas para participar no marketplace. O nosso objetivo é ligar os clientes da Microsoft com as melhores soluções e serviços que oferece a nossa ecossistema de parceiro.
...
## <a name="azure-marketplace-and-appsource-storefronts"></a>Azure Marketplace e AppSource storefronts
...
Os storefronts marketplace são alinhados com audiências e produtos em nuvem da Microsoft para ajudar os clientes localizar que precisam. Cada storefront oferece opções de publicação especializadas para o ajudar a maximizar o investimento de publicação. A tabela seguinte resume estas opções:
...
## <a name="using-the-storefronts"></a>Utilizar os storefronts
...
Interagir com os profissionais de TI e programadores de nuvem através do Azure Marketplace para detetar, tente e comprar soluções de IaaS, SaaS e PaaS:
...
Interagir com os utilizadores empresariais no AppSource para localizar, tente e obter serviços de implementação e aplicações de SaaS de linha de negócio para ajudar a resultados de negócios de unidade e reduzir o tempo de valor: 
...
Escolher um storefront começa com identificar o público-alvo para a sua oferta. O Azure Marketplace está alinhado às necessidades dos programadores e profissionais de TI. AppSource está alinhada às necessidades dos utilizadores empresariais. Se a sua solução destina-se ambos os público, tem de publicar a lista em ambos os storefronts apenas uma vez.
...
Todas as opções de avaliação são implementadas para o seu ambiente de avaliação e/ou a subscrição do Azure, em vez de no ambiente ou na subscrição do Azure do cliente. Avaliações devem ser de cliente mínimo e guiado sem qualquer compras adicionais se existirem, a configuração adicional para concluir uma simples utilizar maiúsculas e minúsculas. Avaliações tem de incluir suporte gratuito, pelo menos, durante o período de avaliação. Utilizadores de avaliação, devem ser nurtured e monitorizados ao longo de um caminho de avaliação deliberate para obter os melhores resultados. Os publicadores são encorajados a utilizar clientes potenciais clientes potenciais do marketplace e intelligence de na aplicação do publicador para monitorizar e gerir os utilizadores de avaliação.

Existem três cenários típicos de avaliação:


|**Opção de avaliação**  |**Principais vantagens**  |**Escolha esta opção se...**  |
|---------|---------|---------|
|**Avaliação gratuita**    |     Permite que um cliente experimentar o produto antes de poderem comprar com um método para converter para utilização paga automático. Também permite proofs de conceito para o cliente e o engagement conjunta com equipas de vendas da Microsoft. |     A sua solução é uma máquina virtual ou modelo de solução.<br><br> A solução é uma oferta de SaaS e oferecem um produto de SaaS multi-inquilino. <br><br>Tem uma experiência de primeira execução para obter um cliente de cópias de segurança e executar rapidamente. <br><br>Ter um único inquilino, mas estiver a adicionar clientes como os utilizadores convidados.|
**Unidade de teste**     |     Permite que um cliente experimentar o produto antes de poderem comprar. Também fornece uma experiência assistida da sua solução sobre uma configuração de pré-configurada. |   A solução é uma máquina virtual, um modelo de solução ou uma aplicação SaaS com um inquilino único ou é complexa para aprovisionar. <br><br>Não tem um método para converter a versão de avaliação para uma oferta paga. |
|**Demonstração interativa**    |  Permite aos clientes ver o seu produto em ação, sem a complexidade da configuração.       |    A solução requer configuração complexa que seria difíceis de alcançar no período de avaliação.     |


#### <a name="free-trial"></a>Avaliação gratuita

Utilize uma versão de avaliação gratuita quando a aplicação ou solução oferece uma versão de avaliação gratuita-para-tente, baseados em SaaS. Esta opção unidades de clientes potenciais clientes potenciais de alta qualidade de clientes interessados, para o ajudar a começar a sua flywheel de negócio. Gratuitas podem ser apresentadas como contas de avaliação de utilização limitada ou duração limitada. Deve incluem uma chamada para a acção para acelerar a conversão para a utilização paga do seu software.

#### <a name="test-drive"></a>Versão de teste

Utilize uma unidade de teste quando a solução é implementada através de um ou mais máquinas virtuais através de aplicações de IaaS ou SaaS. A vantagem desta abordagem é que o aprovisionamento automatizado de uma aplicação virtual ou ambiente de solução toda couched num parceiro alojadas "guiado apresentação da" da solução para avaliação de cliente sem custos adicionais para o cliente. O cliente não precisa de ser um cliente do Azure existente, para ajudar a gerar a clientes de potenciais clientes potenciais superior qualidade.

Existem vantagens adicionais para uma unidade de teste:
...
- % de 36 do novo aquisições de cliente no marketplace provenientes de clientes que demorou uma unidade de teste. 
- Unidades de teste ativar sellers de campo da Microsoft compreender melhor o seu produto para os esforços vende conjunta.

#### <a name="interactive-demo"></a>Demo interativa

Colocar os seus clientes através de uma experiência orientada do seu produto utilizando uma demonstração interativa. A vantagem desta opção é que pode fornecer uma experiência de avaliação sem aprovisionamento complicada para soluções complexas. Esta opção fornece aos clientes um aspeto em torno da solução. E permite editores começar a receber de clientes potenciais clientes potenciais que podem ser nurtured para fundamentais sobre oportunidades para iniciar o flywheel de negócio. 

### <a name="transaction"></a>Transação

No Azure Marketplace, utilize um *máquina virtual* quando a solução é implementada como uma aplicação virtual para a subscrição do cliente. Máquinas virtuais estão completamente comércio ativado através de modelos de licenciamento pay as you go ou BYOL-ativado. Aloja a transação de comércio da Microsoft e bills o cliente em nome do publicador. O publicador obtém a vantagem de tirar partido da relação de pagamento preferencial do cliente com a Microsoft, incluindo o contrato Enterprise. 

>[!NOTE]
>Neste momento, compromissos monetários um Enterprise Agreement podem ser utilizados contra a utilização do Azure a aplicação virtual, mas não em relação a taxas de licença de software do publicador.

Utilize um *modelo de solução do Azure* quando uma solução requer a automatização de implementação e configuração adicional para além da aplicação virtual. Modelos de solução podem automatizar o aprovisionamento de um ou mais recursos de máquina virtual e podem aprovisionar os recursos de rede e armazenamento. Modelos de solução podem fornecer as vantagens de automatização em máquinas de virtuais único e em ambientes de solução com base em IaaS completa. Saiba mais sobre como criar modelos de solução [GitHub](https://github.com/MicrosoftDocs/azure-docs).

Utilize um *aplicação gerida por Azure* quando estiver a implementar uma máquina virtual ou uma solução de IaaS com base em toda a subscrição de um cliente – e o cliente ou do publicador pretender que a solução para ser gerido por terceiros (por exemplo, uma TAMA ou \\<ServerName>\SMS_<SITECODE>\HOTFIX\<KB). Saiba mais sobre como criar aplicações geridas [Azure geridos descrição geral de aplicações](https://docs.microsoft.com/azure/managed-applications/overview). Para obter uma lista de perguntas mais comuns, consulte [perguntas frequentes do Marketplace](https://azure.microsoft.com/marketplace/faq/).

>[!NOTE]
> Aplicações geridas tem de ser implementáveis através do Marketplace. Se a comunicação de cliente for uma preocupação, tenha em atenção que pode chegar aos clientes interessados se tiver ativado de partilha de oportunidades potenciais.

### <a name="azure-certified-program"></a>Programa de certificado do Azure

Todas as máquinas virtuais publicadas no Azure Marketplace são testadas para o programa de certificados do Azure. O programa:

- Garante aos clientes que a máquina virtual é compatível com a plataforma do Azure e o Marketplace vender modelo.
- Testes de conformidade de segurança da imagem online, incluindo vírus e software maligno.
- Permite badging ao nível da oferta para melhorar a promoção para os clientes do Microsoft enterprise como uma solução validada.
...
#### <a name="pay-as-you-go-and-bring-your-own-license-billing-options"></a>Pay as you go e colocar a sua própria licença opções de faturação
...
### <a name="single-billing-and-payment-methods"></a>Métodos único faturação e pagamento
...
### <a name="prerequisites-for-all-marketplace-publishing-options"></a>Pré-requisitos para todas as opções de publicação do Marketplace
...
### <a name="certify-your-azure-ad-integration-for-the-marketplace-multitenant-applications"></a>Certificar a integração do Azure AD para o mercado: aplicações multi-inquilino
...
Para aplicações de AppSource que expandem o Office, Dynamics e o Power BI, pode saber mais sobre os requisitos específicos na documentação do produto específico nesta secção. 
...
## <a name="cloud-partner-portal-pre-publishing-checklist-for-the-azure-marketplace"></a>Nuvem Portal de parceiros previamente publicação lista de verificação para o Azure Marketplace
...
|Terá de artefacto esta publicação  |Para este tipo de oferta  |
|---------|---------|
|**Título (máximo de 50 carateres)**    |  Transação: máquinas virtuais, as aplicações do Azure (modelos de solução e aplicações geridas)       |
|**Resumo (máximo de 200 carateres)**    |  Transação: máquinas virtuais, as aplicações do Azure (modelos de solução e aplicações geridas)       |
|**Resumo longo (máximo, 256 carateres)**     |   Transação: máquinas virtuais, as aplicações do Azure (modelos de solução e aplicações geridas)      |
|**Descrição baseado em HTML (máximos 3,000 carateres)**    |  Transação: máquinas virtuais, as aplicações do Azure (modelos de solução e aplicações geridas)       |
|**Logótipos da empresa (40, 40, 90, 90, 115 x 115, 255 x 115, 815 x 290)**    |  Transação: máquinas virtuais, as aplicações do Azure (modelos de solução e aplicações geridas)       |
...
Nesta secção, vamos explicar os passos para:

- Tornar-se de um publicador no Azure Marketplace e AppSource.
- Obtenha acesso ao Portal de parceiros de nuvem. Irá utilizar este portal para criar, publicar e manter a sua oferta. 
...
|Inicie sessão no Portal de parceiros de nuvem     |  15 minutos       |   Depois do parceiro de recebe a aprovação da equipa do Marketplace, que os respetivos nomination for aprovado, parceiros acesso para o [Portal de parceiros de nuvem](https://cloudpartner.azure.com/) está ativada. O parceiro deve utilizar o ID do Microsoft do formulário nomination para iniciar sessão no respetivo perfil de fabricante no Portal de parceiros de nuvem. Após o registo com o Centro de programadores, o parceiro tem de associar a conta de centro de programadores com o respetivo perfil de publicador do Azure Marketplace para publicar.      |

#### <a name="create-a-microsoft-id"></a>Criar uma ID da Microsoft

Durante o processo de publicação de mercado completo, utilize um endereço de e-mail que identifica a conta do Marketplace. Este endereço de e-mail tem de ser registado como um ID da Microsoft e é utilizado para o [Microsoft Developer Center](https://developer.microsoft.com/) e [Portal de parceiros de nuvem](https://cloudpartner.azure.com/). 

Deve ter apenas uma conta de ID da Microsoft para os Azure Marketplace e AppSource ofertas. Recomendamos vivamente que não a partilhá-las com outros serviços ou ofertas.
...
- [Formulário de nomination do Azure Marketplace](http://aka.ms/listonazuremarketplace)   
- [Formulário de nomination AppSource](http://aka.ms/listonappsource)

Depois de submeter o formulário, a equipa de Marketplace irá rever a aplicação e validar o pedido. Depois do pedido é revisto, serão notificados por e-mail com os passos para se tornar um parceiro aprovado no Portal de parceiros de nuvem.
...
Durante o processo, iremos irá recolher informações de endereço de empresa, informações de conta do banco e impostos informações. Normalmente, estas são obtidas por meio de contactos financeiros ou profissionais. Além disso, tem de concluir os seguintes componentes de perfil do publicador para concluir as fases de criação de oferta e a implementação:
...
Para obter uma explicação passo a passo deste processo, consulte [instruções sobre como registar no Centro de programadores](#instructions-on-how-to-register-in-the-developer-center). 
