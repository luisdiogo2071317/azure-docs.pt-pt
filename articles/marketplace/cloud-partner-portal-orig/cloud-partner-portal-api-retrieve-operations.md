---
title: Obter operações de API | Documentos da Microsoft
...
|  status                      | Estado da operação, qualquer um dos "não foi iniciada | em execução | falhou | concluído`. Only one operation can have status `em execução "cada vez. |
