...
O Azure Data Lake Tools para Visual Studio fornece o tipo de projeto U-SQL que o ajuda a orgnize U-SQL scripts. Usando o projeto de U-SQL para gerir o seu código de U-SQL torna os cenários de CI/CD mais facilmente.
...
1.  Adicionar a tarefa de restauro do NuGet para obter a solução referenciado incluindo o pacote NuGet `Azure.DataLake.USQL.SDK`, para que o MSBuild, pode encontrar os destinos de linguagem U-SQL. 
...
1.  Adicionar a tarefa de restauro do NuGet para obter a solução referenciado incluindo o pacote NuGet `Azure.DataLake.USQL.SDK`, para que o MSBuild, pode encontrar os destinos de linguagem U-SQL. 
    ...
    ![Tarefa do MSBuild de CD de CI do conjunto de data Lake para o projeto de banco de dados U-SQL](./media/data-lake-analytics-cicd-overview/data-lake-analytics-set-vsts-msbuild-task.png) 
...
2. Adicionar uma **tarefa da linha de comandos** além de criar ou versão pipeline e preencha a chamada de script `PackageDeploymentTool.exe`. O script de exemplo é o seguinte: 
