...
Aceda a **ficheiro > Definições > criar > execução > implementação**. Selecione **ferramentas de compilação > Maven > importar**. Em seguida, verifique **automaticamente projetos do Maven importação**.

Abra `Main.java` e substitua o bloco de código existente pelo seguinte fragmento de código:
...
Substitua os marcadores de posição para:
* `CLIENT-ID`,
* `CLIENT-SECRET`,
* `TENANT-ID`
* `SUBSCRIPTION-ID`
...
### <a name="wait-for-input"></a>Aguarde pela entrada
...
* Para saber mais sobre U-SQL, consulte [Introdução à linguagem U-SQL de Análise do Azure Data Lake](data-lake-analytics-u-sql-get-started.md) e [Referência de linguagem U-SQL](https://go.microsoft.com/fwlink/?LinkId=691348).
