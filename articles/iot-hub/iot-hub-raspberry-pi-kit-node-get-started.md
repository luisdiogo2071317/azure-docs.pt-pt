...
* Um quadro Raspberry Pi 2 ou 3 do Raspberry Pi.
...
* Um monitor, um teclado USB e um rato que se liga a Pi.
* Um Mac ou um PC com Windows ou Linux.
...
1. Transferir Raspbian.
   1. [Transferir Raspbian Stretch](http://downloads.raspberrypi.org/raspbian/images/raspbian-2017-07-05/) (o ficheiro. zip).

   > [!WARNING]
   > Utilize acima hiperligação para transferir `raspbian-2017-07-5` imagem zip. A versão mais recente das imagens Raspbian tem alguns problemas conhecidos com Pi preparar o nó, o que poderá provocar a falha na sua próximos passos.
...
1. Instale Raspbian para o cartão microSD.
   ...
   1. Selecione a unidade de cartão microSD. Etcher a poderá já ter selecionado a correta da unidade.
...
1. Ligar o instalador de plataforma para o monitor, teclado e rato. 
1. Iniciar o instalador de plataforma e, em seguida, inicie sessão no Raspbian utilizando `pi` como o nome de utilizador e `raspberry` como a palavra-passe.
1. Clique no ícone de Raspberry > **preferências** > **Raspberry Pi configuração**.

   ![O menu de Raspbian preferências](media/iot-hub-raspberry-pi-kit-node-get-started/1_raspbian-preferences-menu.png)
...
> [!NOTE] 
Para ativar SSH e I2C, pode encontrar mais de referência de documentos no [raspberrypi.org](https://www.raspberrypi.org/documentation/remote-access/ssh/) e [Adafruit.com](https://learn.adafruit.com/adafruits-raspberry-pi-lesson-4-gpio-setup/configuring-i2c).
...
O sensor BME280 pode recolher dados relativos à temperatura e humidade. O LED fica intermitente quando o dispositivo envia uma mensagem para a nuvem. 
...
1. Ligar ao seu Raspberry Pi com um dos seguintes clientes SSH do seu computador anfitrião:
...
1. Instale Node.js e NPM a Pi.
   
   Em primeiro lugar, verifique a sua versão do Node.js. 
   
   ```bash
   node -v
   ```

   Se a versão é inferior à 4. x, ou se não houver nenhuma Node.js no seu Pi, instale a versão mais recente.
...
1. Clone a aplicação de exemplo.

   ```bash
   git clone https://github.com/Azure-Samples/iot-hub-node-raspberrypi-client-app
   ```

1. Instale todos os pacotes para o exemplo. A instalação inclui dispositivos IoT do Azure SDK, biblioteca BME280 Sensor e biblioteca de preparar o Pi.
   ...
   ```
   > [!NOTE] 
   Poderá demorar alguns minutos a concluir este processo de instalação, dependendo da sua ligação de rede.
...
1. Abra o ficheiro de configuração executando os seguintes comandos:
   ...
   Existem dois itens neste ficheiro, que pode configurar. A primeira é `interval`, que define o intervalo de tempo (em milissegundos) entre as mensagens enviadas para a nuvem. Segunda é `simulatedData`, que é um valor booleano para se deve utilizar dados de sensores simulados ou não.
