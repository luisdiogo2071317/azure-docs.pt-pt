...
## <a name="using-the-mqtt-protocol-directly"></a>Através do protocolo MQTT diretamente
...
* Para o **nome de utilizador** campo, utilize `{iothubhostname}/{device_id}/api-version=2016-11-14`, onde `{iothubhostname}` é o CName completo do IoT hub.
...
* Para o **palavra-passe** campo, utilize um token SAS. O formato do SAS token é igual de protocolos de HTTPS e AMQP:
  ...
  Ao testar, também pode utilizar para várias plataformas [extensão IoT Toolkit do Azure para Visual Studio Code](https://marketplace.visualstudio.com/items?itemName=vsciot-vscode.azure-iot-toolkit) ou o [Device Explorer] [ lnk-device-explorer] ferramenta rapidamente gerar uma SAS de token que Pode copiar e colar no seu próprio código:

Para o Toolkit de IoT do Azure:
...
Como para atualizações de propriedade, `null` valores significa que o membro de objeto do JSON está a ser eliminado.
