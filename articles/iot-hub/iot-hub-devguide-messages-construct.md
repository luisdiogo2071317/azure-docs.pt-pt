...
| Para |Um destino especificado na [Cloud-para-dispositivo] [ lnk-c2d] mensagens. | Não para mensagens de C2D; Sim, caso contrário. |
| ExpiryTimeUtc |Data e hora de expiração de mensagem. | Sim |
| EnqueuedTime |Data e hora a [Cloud-para-dispositivo] [ lnk-c2d] mensagem foi recebida pelo IoT Hub. | Não para mensagens de C2D; Sim, caso contrário. |
...
| ConnectionDeviceGenerationId |Um ID definido pelo IoT Hub nas mensagens do dispositivo para a cloud. Contém a **generationId** (como por [das propriedades de identidade do dispositivo][lnk-device-properties]) do dispositivo que enviou a mensagem. | Não para mensagens D2C; Sim, caso contrário. |
