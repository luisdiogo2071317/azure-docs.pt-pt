---
title: A configuração de cluster de Hadoop, Spark, Kafka, HBase ou servidor R - Azure HDInsight
...
ms.locfileid: "37062084"
---
# <a name="quickstart-set-up-clusters-in-hdinsight"></a>Início rápido: Configurar clusters no HDInsight

Este guia de introdução, saber preparar e configurar clusters no HDInsight com o Hadoop, Spark, Kafka, consulta interativa, HBase, servidor R ou Storm. Também aprender personalizar clusters, associe-las a um domínio e anexar a [pré-visualização do Azure Data Lake armazenamento Gen2](introduction.md).
...
Armazenamento do Data Lake é utilizado como a camada de dados neste guia de introdução. Com o serviço de espaços de nomes hierárquica e [Hadoop controlador](abfs-driver.md), armazenamento do Data Lake está otimizado para análise e de processamento distribuído. Mantém os dados armazenados no armazenamento do Data Lake, mesmo depois de um cluster do HDInsight é eliminado.
...
| [CLI do Azure (ver 1.0)](../../hdinsight/hdinsight-hadoop-create-linux-clusters-azure-cli.md) |&nbsp; |✔ |&nbsp; |&nbsp; |
...
![hdinsight criar as opções de criação rápida personalizada](media/quickstart-create-connect-hdi-cluster/hdinsight-creation-options.png)
...
> [!IMPORTANT]
> O Linux é o único sistema operativo utilizado na versão 3.4 ou superior do HDInsight. Para obter mais informações, consulte [HDInsight 3.3 extinção](../../hdinsight/hdinsight-component-versioning.md#hdinsight-windows-retirement).
...
| --- | --- |
| [Hadoop](../../hdinsight/hadoop/apache-hadoop-introduction.md) |Análise dos dados armazenados e consulta de batch |
| [HBase](../../hdinsight/hbase/apache-hbase-overview.md) |Processamento de grandes quantidades de dados de NoSQL sem esquemas, |
| [Consulta interativa](../../hdinsight/interactive-query/apache-interactive-query-get-started.md) |Dentro da memória colocação em cache para mais rápidas e interativas consultas do Hive |
| [Kafka](../../hdinsight/kafka/apache-kafka-introduction.md) | Uma plataforma de transmissão em fluxo distribuída que pode ser utilizada para criar pipelines de dados de transmissão em fluxo em tempo real e aplicações |
| [R Server](../../hdinsight/r-server/r-server-overview.md) |Vários estatísticas de macrodados, modelação preditiva e capacidades de aprendizagem |
| [Spark](../../hdinsight/spark/apache-spark-overview.md) |Processamento de memória, consultas interativas, processamento de fluxo micro batch |
...
Escolha a versão do HDInsight para este cluster. Para obter mais informações, consulte [as versões do HDInsight suportado](../../hdinsight/hdinsight-component-versioning.md#supported-hdinsight-versions).

### <a name="enterprise-security-package"></a>Pacote de segurança da empresa

Para tipos de cluster do Hadoop, Spark e interativa consulta, pode optar por ativar o **pacote de segurança empresarial**. Este pacote fornece a opção para ter uma configuração em cluster mais segura ao utilizar o Apache Ranger e integrar com o Azure Active Directory. Para obter mais informações, consulte [Enterprise o pacote de segurança no Azure HDInsight](../../hdinsight/domain-joined/apache-domain-joined-introduction.md).

![hdinsight criar opções escolher o pacote de segurança da empresa](./media/quickstart-create-connect-hdi-cluster/hdinsight-creation-enterprise-security-package.png)

Para obter mais informações sobre como criar HDInsight associados a um domínio do cluster, consulte [criar ambiente de sandbox de HDInsight associados a um domínio](../../hdinsight/domain-joined/apache-domain-joined-configure.md).
...
* Utilizador do SSH (clusters do Linux): utilizada para ligar ao cluster através de SSH. Para obter mais informações, veja [Utilizar SSH com o HDInsight](../../hdinsight/hdinsight-hadoop-linux-use-ssh-unix.md).

O pacote de segurança da empresa permite-lhe integrar HDInsight com o Active Directory e o Apache Ranger. Vários utilizadores podem ser criados utilizando o pacote de segurança da empresa.
...
Embora uma instalação local do Hadoop utiliza o distribuídas ficheiro sistema Hadoop (HDFS) para o armazenamento de cluster, a nuvem utiliza pontos finais de armazenamento ligados ao cluster. A utilização de clusters do HDInsight uma [Gen2 de armazenamento do Data Lake](abfs-driver.md) ou [Blobs no armazenamento do Azure](../../hdinsight/hdinsight-hadoop-use-blob-storage.md). A utilização do Storage do Azure ou o armazenamento do Data Lake significa que pode eliminar em segurança os clusters do HDInsight utilizados para o cálculo mantendo os dados.

> [!WARNING]
> Não é suportada através de uma conta de armazenamento adicional numa localização diferente do HDInsight cluster.

Durante a configuração, o ponto final de armazenamento de predefinido, especificar armazenamento do Data Lake. O armazenamento de predefinido contém aplicações e do sistema registos. Opcionalmente, pode especificar contas adicionais de armazenamento do Azure Data Lake ligadas que o cluster possa aceder. O cluster do HDInsight e as contas do storage dependente tem de ser na mesma localização do Azure.

![As definições de armazenamento de cluster: pontos finais de armazenamento compatível com HDFS](media/quickstart-create-connect-hdi-cluster/hdinsight-cluster-creation-storage-TEMP.png)

> [!IMPORTANT]
> Certifique-se para **desativar o acesso do Data Lake Store**. Este definições refere-se para o antigo *Data Lake Store* funcionalidade e tem de ser desativado na ordem de *armazenamento do Data Lake* funcionalidades funcione corretamente.
...
Para obter mais informações, consulte [utilizar arquivos de metadados externo no Azure HDInsight](../../hdinsight/hdinsight-use-external-metadata-stores.md).
...
| Tipo | Nós | Diagrama |
| --- | --- | --- |
| Hadoop |Nó principal (2), o nó de dados (1 +) |![Nós de cluster do HDInsight Hadoop](media/quickstart-create-connect-hdi-cluster/hdinsight-hadoop-cluster-type-nodes.png) |
| HBase |Servidor HEAD (2), servidor de região (1 +), nó mestre/ZooKeeper (3) |![Nós de cluster HBase do HDInsight](media/quickstart-create-connect-hdi-cluster/hdinsight-hbase-cluster-type-setup.png) |
| Storm |Nó nimbus (2), servidor supervisor (1 +), o nó de ZooKeeper (3) |![Nós de cluster do HDInsight Storm](media/quickstart-create-connect-hdi-cluster/hdinsight-storm-cluster-type-setup.png) |
| Spark |Nó principal (2), o nó de trabalho (1 +), nós de ZooKeeper (3) (gratuita para o tamanho de VM de ZooKeeper A1) |![Nós de cluster do Spark do HDInsight](media/quickstart-create-connect-hdi-cluster/hdinsight-spark-cluster-type-setup.png) |

Para obter mais informações, consulte [predefinido tamanhos de configuração e a máquina virtual do nó para clusters](../../hdinsight/hdinsight-component-versioning.md#default-node-configuration-and-virtual-machine-sizes-for-clusters) no "Quais são os componentes do Hadoop e versões no HDInsight?"
...
![HDInsight nó dos tamanhos da VM](media/quickstart-create-connect-hdi-cluster/hdinsight-node-sizes.png)
...
>
> Cascata não é suportada pelo HDInsight e não é elegível para Support da Microsoft. Para apresenta uma lista dos componentes suportados, consulte [Novidades nas versões do cluster fornecidas pelo HDInsight](../../hdinsight/hdinsight-component-versioning.md).
>
>
...
Para obter mais informações, consulte [HDInsight personalizar clusters com o arranque](../../hdinsight/hdinsight-hadoop-customize-cluster-bootstrap.md).
...
Se a sua solução requer tecnologias que são distribuídas por vários tipos de cluster do HDInsight, uma [rede virtual do Azure](../../hdinsight/https://docs.microsoft.com/azure/virtual-network) pode ligar os tipos de cluster necessários. Esta configuração permite que os clusters e qualquer código que implementar aos mesmos, para comunicar diretamente entre si.

Para obter mais informações sobre como utilizar uma rede virtual do Azure com o HDInsight, consulte [expandir HDInsight com redes virtuais do Azure](../../hdinsight/hdinsight-extend-hadoop-virtual-network.md).

Para obter um exemplo de utilização de dois tipos de cluster dentro de uma rede virtual do Azure, consulte [utilize Spark estruturados transmissão em fluxo com Kafka](../../hdinsight/hdinsight-apache-kafka-spark-structured-streaming.md). Para obter mais informações sobre como utilizar o HDInsight com uma rede virtual, incluindo requisitos de configuração específicos para a rede virtual, consulte [capacidades de expandir HDInsight utilizando a Azure Virtual Network](../../hdinsight/hdinsight-extend-hadoop-virtual-network.md).
...
- [Controlador de sistema de ficheiros do ABFS Hadoop para Gen2 de armazenamento do Azure Data Lake](abfs-driver.md)
- [Tutorial: Extrair, transformar e carregar dados utilizando o Apache Hive no Azure HDInsight](tutorial-extract-transform-load-hive.md)
- [Quais são o HDInsight, ecossistema do Hadoop, e clusters do Hadoop?](../../hdinsight/hadoop/apache-hadoop-introduction.md)
- [Introdução à utilização do Hadoop no HDInsight](../../hdinsight/hadoop/apache-hadoop-linux-tutorial-get-started.md)
- [Hadoop no HDInsight a partir do Windows PC de trabalho](../../hdinsight/hdinsight-hadoop-windows-tools.md)
