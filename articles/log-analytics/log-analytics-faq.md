...
O agente Log Analytics foi concebido para garantir que ele ocupa pouco espaço. O volume de dados varia consoante as soluções que ativa. Pode encontrar informações detalhadas sobre o volume de dados e ver a divisão pela solução no [utilização](log-analytics-usage.md) página.
...
O escalão de preço gratuito é uma boa forma de carregar vários servidores e o volume de dados típico do medidor. Geral de utilização é apresentada no [utilização](log-analytics-usage.md) página.
