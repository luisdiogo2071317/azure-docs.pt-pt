...
Não há um compromisso entre fiabilidade de alertas e a capacidade de resposta de alertas. Pode optar por configurar os parâmetros de alerta para minimizar falsas alertas e alertas em falta ou pode escolher os parâmetros de alerta para responder rapidamente às condições que estão a ser monitorizadas, mas ocasionalmente gera alertas falsos ou em falta.

Regras de alerta são definidas pelos seguintes detalhes:

- **Pesquisa de registo**. A consulta que é executada sempre que a regra de alerta é acionado. Os registos devolvidos por esta consulta são utilizados para determinar se é criado um alerta.
- **Janela de tempo**. Especifica o intervalo de tempo para a consulta. A consulta devolve apenas os registos que foram criados neste intervalo da hora atual. Isto pode ser qualquer valor entre cinco minutos e 24 horas. O intervalo deve ser suficientemente grande para acomodar razoáveis atrasos na ingestão. A janela de tempo tem de ser duas vezes o comprimento do atraso mais longo, que quer ser capaz de processar.<br> Por exemplo, se pretender que os alertas para ser fiável para atrasos de 30 minutos, em seguida, o intervalo tem de ser uma hora.
    ...
    - **Falta alertas**. Assuma o atraso de ingestão é 60 minutos, por vezes, mas a maioria das vezes é quinze minutos. Se a janela de tempo é definida como 30 minutos, em seguida,-falhas de acertos num alerta quando o atraso é de 60 minutos, porque os dados não estão disponíveis para a pesquisa quando é executada a consulta de alerta. 
...
- **Número de resultados** regra de alerta criar sempre um tempo de alerta único **medida métrica** regra de alerta cria um alerta para cada objeto que é superior ao limiar.
...
O limiar para um **número de resultados** regra de alerta é maior ou menor do que um valor específico. Se o número de registos devolvidos pela pesquisa de registo corresponde este critério, é criado um alerta.
...
| Tipo |*Alerta* |
| SourceSystem |*OMS* |
| *objeto*  | [Alertas de medida métrica](#metric-measurement-alert-rules) tem uma propriedade para o campo de grupo. Por exemplo, se a pesquisa de registo de grupos no computador, o registo de alerta com tem um campo de computador com o nome do computador como o valor.
