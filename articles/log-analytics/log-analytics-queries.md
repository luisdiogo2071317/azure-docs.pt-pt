...
- **Regras de alerta.** [Regras de alerta](log-analytics-alerts.md) identificar proativamente os problemas dos dados na sua área de trabalho.  Cada regra de alerta baseia-se uma pesquisa de registos que está a ser executada automaticamente em intervalos regulares.  Os resultados são inspecionados para determinar se um alerta deve ser criado.
...
- **API do log Analytics.**  O [a API de pesquisa de registos do Log Analytics](log-analytics-log-search-api.md) permite que qualquer cliente de REST API recuperar dados de registo de área de trabalho.  O pedido de API inclui uma consulta que é executada no Log Analytics para determinar os dados a obter.
