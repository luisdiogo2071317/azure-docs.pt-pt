...
>
> - Não são suportadas atualizações entre versões principais de uma distribuição Linux num servidor físico ou máquina virtual VMware protegido para recuperação de sites. Ao atualizar o sistema operativo em versões principais (por exemplo, CentOS 6.* para CentOS 7.*), desative a replicação da máquina, atualize o sistema operativo na máquina e, em seguida, ativar a replicação novamente.
>
...
**Contagem de discos de dados** | Contagem é 64 ou menos se está a replicar *VMs de VMware para Azure*. Contagem é 16 ou menos se está a replicar *VMs de Hyper-V para o Azure*. | A verificação de pré-requisitos falha se não suportado.
**Tamanho do VHD do disco de dados** | 4,095 GB | A verificação de pré-requisitos falha se não suportado.
...
**Tipo de VM** | Geração 1.<br/><br/> Geração 2 – Windows. | Geração 2 VMs com um tipo de disco de SO de basic (que inclui um ou dois volumes de dados formatados como VHDX) e inferior a 300 GB de espaço em disco são suportadas.<br></br>As VMs do Linux geração 2 não são suportadas. Para obter mais informações, consulte [recuperação após desastre para o Azure avançada](https://azure.microsoft.com/blog/2015/04/28/disaster-recovery-to-azure-enhanced-and-were-listening/).|
