---
title: "Como efetuar a reativação a partir do Azure para VMware com o Azure Site Recovery | Microsoft Docs"
...
> [!WARNING]
> Não é possível reativação pós-falha depois de ter um [concluído migração](migrate-overview.md#what-do-we-mean-by-migration), mover uma máquina virtual para outro grupo de recursos ou eliminar a máquina virtual do Azure. Se desativar a proteção da máquina virtual, não é possível reativação pós-falha.
...
1. [Proteja](vmware-azure-reprotect.md) as máquinas virtuais do Azure para que possam iniciar replicar máquinas virtuais de VMware no seu site no local. Como parte deste processo, terá também de:
    1. Configurar um destino principal no local: destino principal do Windows para máquinas virtuais Windows e [destino principal do Linux](vmware-azure-install-linux-master-target.md) para máquinas virtuais do Linux.
    2. Configurar um [servidor de processos](vmware-azure-set-up-process-server-azure.md).
    3. Iniciar [Proteja](vmware-azure-reprotect.md). Isto irá desativar a máquina virtual no local e sincronizar os dados da máquina virtual do Azure com os discos no local.
...
> [!IMPORTANT]
> Antes de iniciar a reativação pós-falha, certifique-se de que concluiu que as máquinas virtuais. As máquinas virtuais devem estar num estado protegido e o respetivo estado de funcionamento deve ser **OK**. Para voltar a proteger as máquinas virtuais, leia [como voltar a proteger](vmware-azure-reprotect.md).
