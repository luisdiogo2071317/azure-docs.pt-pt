...
ms.locfileid: "55823386"
---
# <a name="move-azure-vms-to-another-region"></a>Mover VMs do Azure para outra região

Existem vários cenários em que iria querer mover as máquinas de virtuais de IaaS do Azure existentes de uma região para outra - para melhorar a fiabilidade e disponibilidade das suas VMs existentes ou para melhorar a capacidade de gestão ou por razões de governação etc., conforme detalhado. 
...
> * [Eliminar o recurso na região de origem ](#discard-the-resource-in-the-source-region)

> [!IMPORTANT]
> Este documento orienta-o para mover as VMs do Azure de uma região para outra, conforme é, se o requisito melhorar a disponibilidade da sua infraestrutura ao mover VMs para as zonas de disponibilidade, consulte o tutorial aqui.

## <a name="verify-prerequisites"></a>Certifique-se de pré-requisitos

- Certifique-se de que ter as VMs do Azure na região do Azure da qual pretende mover origem.
...
3. Se a VM está a tentar mover não tem acesso à internet e estiver a utilizar um proxy de firewall para controlar o acesso de saída, verifique os requisitos [aqui](azure-to-azure-tutorial-enable-replication.md#configure-outbound-network-connectivity).
4. Identificar e lista pendente o esquema de rede de origem e de todos os recursos que está a utilizar atualmente - incluindo, mas não se limitando a balanceadores de carga, NSGs, etc IP público para a verificação.
...
5. Pode optar por vão, com os recursos de destino predefinido ou utilizar aqueles que criou previamente.
...
> [!IMPORTANT]
> É importante executar o passo acima para evitar ser cobrado a mudança para o post de replicação do Site Recovery.
