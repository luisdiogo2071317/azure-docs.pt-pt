...
* [Preparar o VMware no local](tutorial-prepare-on-premises-vmware.md)

Antes de começar, é útil [rever a arquitetura](concepts-vmware-to-azure-architecture.md) dos cenários de recuperação após desastre.
...
6. Em **Configurar servidor vCenter Server/vSphere ESXi**, introduza o FQDN ou endereço IP do servidor vCenter ou anfitrião vSphere onde estão localizadas as VMs que pretende replicar. Introduza a porta em que o servidor está a ouvir. Introduza um nome amigável a utilizar para o servidor VMware no cofre.
7. Introduza as credenciais a utilizar pelo servidor de configuração para ligar ao servidor VMware. O Site Recovery utiliza estas credenciais para detetar automaticamente as VMs VMware que estão disponíveis para replicação. Selecione **Adicionar** e, em seguida, selecione **Continuar**.
...
3. A Recuperação de Sites verifica que tem uma ou mais contas de armazenamento e redes do Azure compatíveis.
   ...
   ![Separador Destino](./media/tutorial-vmware-to-azure/storage-network.png)
...
6. Em **Retenção do ponto de recuperação**, utilize a predefinição de 24 horas como a duração da janela de retenção para cada ponto de recuperação. Neste tutorial, utilize 72 horas. As VMs replicadas podem ser recuperadas para qualquer ponto numa janela.
...
6. Em **Destino**, selecione a subscrição e o grupo de recursos em que pretende criar as VMs de ativação pós-falha. Selecione o modelo de implementação que pretende utilizar no Azure (clássico ou Resource Manager) para as VMs de ativação pós-falha.
...
11. Em **Propriedades** > **Configurar propriedades**, selecione a conta a utilizar pelo servidor de processos para instalar automaticamente o Serviço de Mobilidade na máquina.
...
13. Selecione **Ativar a Replicação**.
