...
* VM de VMware, onde os seguintes controladores não são controladores de arranque:
    ...
    * intelide
    * ATAPI
* VM do VMware que não têm o DHCP ativado, rrespective de se estão a utilizar DHCP ou endereços IP estáticos.
...
Se pretender ligar a VMs do Azure com RDP após a ativação pós-falha, siga os requisitos resumidos na tabela.
