---
title: Automatização de aplicações de Web de três camadas RU oficial - descrição geral
description: Automatização de aplicações de Web de três camadas RU oficial - descrição geral
services: security
...
> [!NOTE]
> Serviços do Azure estão implementados regionally e os clientes podem configurar a determinados serviços do Azure para armazenar dados de cliente apenas numa única região. Microsoft Azure fornece uma lista dos centros de dados globalmente disponíveis para fornecer disponibilidade e fiabilidade numa escala global. Todos os centros de dados do Azure foi certificou contra a norma ISO/IEC 27001: 2013. A Georreplicação RU consiste em 2 regiões: sul do RU e RU oeste.

|||
|---|---|
| **Cliente** | Este blueprint solicita o administrador para cada região implementar os recursos do Azure. As regiões recomendadas para implementação são sul do RU ou RU oeste. |
| **Provider&nbsp;(Microsoft&nbsp;Azure)** | Não Aplicável |


 ## <a name="ncsc-cloud-security-principle-22"></a>Princípio de segurança de nuvem NCSC 2.2
### <a name="datacenter-security"></a>Segurança de datacenters
Localizações utilizadas para fornecer serviços de nuvem tem física proteção contra acesso não autorizado, adulteração, roubo ou a reconfiguração dos sistemas. Proteções inadequadas poderão resultar no divulgação, alteração ou perda de dados.
...
|||
|---|---|
| **Cliente** | Os clientes não têm acesso físico a quaisquer recursos de sistema nos centros de dados do Azure; medidas de proteção de segurança do Centro de dados são implementadas e geridas pelo Microsoft Azure. Este princípio é herdado a partir do Microsoft Azure. |
| **Provider&nbsp;(Microsoft&nbsp;Azure)** | Microsoft Azure implementa este princípio em nome dos clientes. Microsoft Azure é executado em instalações do Microsoft distribuídas geograficamente, espaço e utilitários de partilha com outros serviços online da Microsoft. Cada função é concebida para ser executado 24 x 7 x 365 e emprega várias medidas de norma da indústria para ajudar a proteger as operações de falha de energia, intrusões físico e falhas de rede. Nestes centros de dados está em conformidade com as normas da indústria (por exemplo, ISO 27001) para segurança física e disponibilidade. Estes são geridos, monitorizados e administrados pela equipa de operações da Microsoft. <br /> <br /> Clientes do Azure podem ter a certeza de que os controlos de segurança física são cumpridos Azure em todos os centros de dados devido a Azure que contém os certificados em todos os centros de dados para a norma do ISO/IEC 27001: 2013. A Georreplicação RU consiste em duas regiões: sul do RU e RU oeste. |
...
Para garantir que os dados não estão disponíveis para entidades não autorizadas com o acesso físico à infraestrutura, contidos no âmbito do serviço de dados de utilizador devem ser protegidos, independentemente do suporte de dados de armazenamento que é mantido. Sem medidas adequadas no local, dados podem ser divulgados inadvertidamente no suporte de dados rejeitados, perdido ou roubado.
...
> [!NOTE]
> Se o cliente configura o Microsoft Azure adequadamente ao ativar o Azure Site Recovery e alternativo armazenamento de dados no Centro de dados de georreplicação graficamente localizado noutro, Microsoft Azure pode suportar a operação contínua de recursos implementados no cliente.
...
| **Provider&nbsp;(Microsoft&nbsp;Azure)** | Devido a servidores em nuvem do cliente que está a ser virtual, o paradigma separação física já não se aplica. Microsoft Azure foi concebido para ajudar a identificar e contador riscos inerentes num ambiente multi-inquilino. Armazenamento de dados e processamento logicamente é segregado entre utilizadores do Azure utilizando o Active Directory e a funcionalidade desenvolvida especificamente para os serviços de multi-inquilino, que visa para garantir que os dados de utilizador armazenados nos centros de dados partilhados do Azure não está acessível por outro organização. <br /> <br /> Fundamentais para qualquer arquitetura da nuvem partilhado é o isolamento fornecido para cada utilizador impedir que um utilizador mal intencionado ou comprometido que afetam o serviço ou os dados de outro. <br /> <br /> Para obter mais informações sobre a separação de inquilino do Microsoft, consulte a descrição completa no [segurança do Azure e matriz de responsabilidades do cliente de conformidade Blueprint - princípios de segurança de nuvem NCSC -](https://aka.ms/blueprintuk-gcrm). |
...
- Configuração e gestão de alterações - deve garantir que as alterações no sistema foram corretamente testadas e autorizadas. As alterações não devem alterar inesperadamente propriedades de segurança
- Gestão de vulnerabilidade - deve identificar e mitigar problemas de segurança nos componentes constituintes
- Monitorização protective - deve colocar medidas no local para detetar ataques e a atividade não autorizada no serviço
...
As alterações que podem afetar a segurança do serviço devem ser identificadas e geridas. As alterações não autorizadas devem ser detetadas.
...
| **Provider&nbsp;(Microsoft&nbsp;Azure)** | Gestão de atualizações de segurança ajuda a proteger os sistemas de vulnerabilidades conhecidas. Sistemas de implementação integrada do Azure utiliza para gerir a distribuição e a instalação das atualizações de segurança para Microsoft software. Azure também é possível desenhar nos recursos do Microsoft Security Response Center (MSRC), que identifica, monitoriza, responde a e resolve incidentes de segurança e na nuvem vulnerabilidades à volta o relógio, cada dia do ano. |
...
| **Provider&nbsp;(Microsoft&nbsp;Azure)** | Microsoft Azure Security definiu os requisitos para a monitorização do Active Directory. Serviço equipas configurar ferramentas de monitorização de Active Directory, de acordo com estes requisitos. Ferramentas de monitorização de Active Directory incluem o agente de monitorização (MA) e o System Center Operations Manager (SCOM), que estão configurados para alertas de tempo de fornecer ao pessoal de segurança do Microsoft Azure em situações que requerem ação imediata. |
...
Estes processos não precisa de ser complexo ou necessitam de grandes quantidades de descrição, mas a gestão de incidentes boa irá minimizar o impacto para os utilizadores de segurança, fiabilidade e problemas de ambiente com um serviço.
...
| **Provider&nbsp;(Microsoft&nbsp;Azure)** | O grupo do Microsoft Cloud fornecer cadeia (MCSC) consiste em seis equipas exclusivas cada contribuir para a proteção do Azure de ameaças para a cadeia de fornecer.  <br />  <br /> -Aprovisionamento <br /> -Operações de cliente <br /> -Qualidade implementação <br /> -Gestão de relação fornecedor <br /> -Spares <br />  <br /> Para obter mais informações sobre o grupo MCSC da Microsoft, consulte a descrição completa no [segurança do Azure e matriz de responsabilidades do cliente de conformidade Blueprint - princípios de segurança de nuvem NCSC -](https://aka.ms/blueprintuk-gcrm). |
...
O fornecedor deve disponibilizar as ferramentas para gerir em segurança a utilização do seu serviço. Os procedimentos e interfaces de gestão são uma parte vital a barreira de segurança, a impedir o acesso não autorizado e a alteração dos seus recursos, aplicações e dados.
...
Todos os acessos a interfaces de serviço devem ser restrita para utilizadores autenticados e autorizados.
Fraca autenticação para estas interfaces pode ativar o acesso não autorizado aos seus sistemas, resultando no roubo ou modificação dos seus dados, as alterações ao seu serviço, ou um denial of service.
...
A estrutura, implementação e gestão dos sistemas de administração devem seguir boa prática enterprise, enfrenta reconhecer o respetivo valor elevado aos atacantes.
...
> [!NOTE]
> O cliente pode utilizar o Centro de segurança do Azure para o ajudar a evitar, detetar e responder a ameaças com uma maior visibilidade e controlo sobre a segurança dos recursos do Azure. O Centro de segurança do Azure fornece gestão de políticas e monitorização de segurança integrada nas subscrições do Azure, ajuda a detetar ameaças que caso contrário podem passar despercebidas e funciona com um ecossistema abrangente de soluções de segurança.

|||
|---|---|
| **Cliente** | Os modelos Azure Resource Manager e o associada recursos que compõem este blueprint siga uma abordagem de defence-na profundidade para segurança. Para poder estar em conformidade com este princípio, é necessária configuração adicional pelo cliente para utilização em produção (por exemplo, software de gestão de base de dados, implementação de aplicação web). |
| **Provider&nbsp;(Microsoft&nbsp;Azure)** | Não Aplicável |
