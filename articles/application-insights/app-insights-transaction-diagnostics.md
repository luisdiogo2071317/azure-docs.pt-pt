...
*Esta experiência está atualmente em pré-visualização e substitui os painéis de diagnóstico do lado do servidor existente.*
...
* Selecione qualquer linha para ver os respectivos detalhes à direita. Quando clica no ícone de gerador de perfis de uma linha de pedido ou um ícone de instantâneo de depuração na linha de uma exceção é aberto o painel de detalhes correspondente.

> [!NOTE]
As chamadas de outros componentes têm duas linhas: uma linha representa a chamada de saída (dependência) componente do autor da chamada do e a outra linha corresponde do pedido de entrada, o componente chamado. A última opção é denominado localhost para ajudar a diferenciar entre eles. Utilize o canal de comentários do canto superior direito para nos informar como sentir sobre a apresentação atualizada.

### <a name="time-sequenced-telemetry-of-the-selected-component-operation"></a>Telemetria sequenciado de tempo da operação de componentes selecionado
...
* Se estes componentes são recursos do Application Insights separados, tem acesso necessário à-las?
...
Neste momento, estamos são a mostrar a chamada de dependência de saída separados do pedido de entrada. Normalmente, as chamadas de dois parecem idênticas com apenas o valor de duração existente que está a ser diferente devido a rede percurso de ida e volta. Para ajudar a distinguir entre eles, estamos a ligar para o componente de receber o pedido de "localhost" com um ícone de servidor. Esta linha seguirá imediatamente a linha de dependência. Esta apresentação dos dados é confuso? Dê-nos seus comentários!
