...
![Abra pesquisa de diagnóstico](./media/app-insights-diagnostic-search/001.png)

![Captura de ecrã de gráficos de pesquisa de diagnóstico](./media/app-insights-diagnostic-search/002.png)
...
![Abra o Visual Studio pesquisa do Application Insights](./media/app-insights-diagnostic-search/32.png)
...
![Captura de ecrã de um pedido de dependência individuais](./media/app-insights-diagnostic-search/003.png)

Isso iniciará o modo de exibição de detalhes de transação de ponta a ponta:

![Captura de ecrã da vista de detalhes de transação de ponta a ponta.](./media/app-insights-diagnostic-search/004.png)
...
* **Rastreio** - [registos de diagnóstico](app-insights-asp-net-trace-logs.md) incluindo chamadas TrackTrace, log4Net, NLog e System.Diagnostic.Trace.
...
* **Vista de página** - [telemetria enviada pelo cliente web](app-insights-javascript.md), utilizado para criar relatórios de vista de página. 
* **Evento personalizado** – se tiver inserido chamadas de trackevent () para [monitorizar a utilização](app-insights-api-custom-events-metrics.md), pode procurá-los aqui.
* **Exceção** - não identificadas [exceções no servidor](app-insights-asp-net-exceptions.md)e os que inicie sessão com trackexception ().
* **Dependência** - [chamadas a partir da sua aplicação de servidor](app-insights-asp-net-dependencies.md) a outros serviços, como REST APIs ou bases de dados e AJAX chama a partir de seu [código de cliente](app-insights-javascript.md).
* **Disponibilidade** -os resultados da [testes de disponibilidade](app-insights-monitor-web-app-availability.md).
...
| `apple AND banana` |Encontre eventos que contêm as duas palavras. Utilizar o capital "e", não "e". |
| `apple OR banana`<br/>`apple banana` |Encontre eventos que contêm o word. Utilize "Ou", não "ou".<br/>Formulário curto. |
...
* Capturar rastreios de registos da sua arquitetura de registo favorita na [.NET](app-insights-asp-net-trace-logs.md) ou [Java](app-insights-java-trace-logs.md). Isso significa que pode pesquisar por meio de seus rastreios de registos e correlacioná-los com outros eventos, exceções e vistas de página. 
* [Escrever código](app-insights-api-custom-events-metrics.md) para enviar eventos personalizados e vistas de página e exceções. 

[Saiba como enviar registos e telemetria personalizada para o Application Insights](app-insights-asp-net-trace-logs.md).

## <a name="questions"></a>AS PERGUNTAS E RESPOSTAS
### <a name="limits"></a>A quantidade de dados é retido?

Consulte a [resumo de limites](app-insights-pricing.md#limits-summary).

### <a name="how-can-i-see-post-data-in-my-server-requests"></a>Como posso ver dados de POSTAGEM nos meus pedidos de servidor?
Nós não iniciar sessão automaticamente os dados de POSTAGEM, mas pode usar [chamadas TrackTrace ou registo](app-insights-asp-net-trace-logs.md). Coloca os dados POST no parâmetro de mensagem. É possível filtrar a mensagem da mesma forma, que pode filtrar nas propriedades, mas o limite de tamanho é maior.
