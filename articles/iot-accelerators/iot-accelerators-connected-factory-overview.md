...
> [!NOTE]
> Se detetar algum problema ao implementar o acelerador da solução, consulte [Permissões no site azureiotsuite.com][lnk-permissions] e as [FAQ sobre Fábrica Ligada](iot-accelerators-faq-cf.md). Se os problemas persistirem, crie um pedido de serviço no [portal][lnk-portal].
...
* Um painel **Localizações de fábricas** que mostra o estado, a localização e a configuração de produção atual da solução. Quando executar a solução pela primeira vez, existem vários dispositivos simulados. A simulação de linha de produção é composta por três servidores OPC UA reais por linha de produção que realizam tarefas simuladas e partilham dados. Para obter mais informações sobre OPC UA, veja as [FAQ sobre a Fábrica Ligada](iot-accelerators-faq-cf.md).
