---
title: Autenticar e autorizar utilizadores ponto a ponto no Serviço de Aplicações do Azure no Linux | Microsoft Docs
...
* [Instale o .NET Core 2.0](https://www.microsoft.com/net/core/).
...
Este passo não está relacionado com a autenticação e autorização. No entanto, vai precisar dele mais tarde para [chamar a API de back-end a partir do código de front-end do browser](#call-api-securely-from-browser-code), para que o browser permita as chamadas de API entre domínios a partir da aplicação Angular.js. O Serviço de Aplicações no Linux não tem uma funcionalidade CORS incorporada, como [o respetivo equivalente no Windows](../app-service-web-tutorial-rest-api.md#add-cors-functionality), pelo que tem de adicioná-la manualmente para a aplicação de back-end.

No repositório local, abra o ficheiro _Startup.cs_. No método `ConfigureServices(IServiceCollection services)`, adicione a seguinte linha de código:

```csharp
services.AddCors();
```

No método `Configure(IApplicationBuilder app)`, adicione a seguinte linha de código ao início (substitua *\<front_end_app_name>*):
...
Guarde as alterações. Novamente na _janela de terminal local_, execute os seguintes comandos para consolidar as alterações no repositório Git.
...
> [!NOTE]
> Não se preocupe em partilhar este código entre as aplicações de front-end e back-end. Não tem qualquer efeito CORS na aplicação de front-end.
> 

### <a name="push-to-azure-from-git"></a>Enviar para o Azure a partir do Git

Na janela de terminal local, execute os seguintes comandos do Git para implementar na aplicação de back-end. Substitua _&lt;deploymentLocalGitUrl-of-back-end-app>_ pelo URL da instância remota do Git que guardou em [Criar recursos do Azure](#create-azure-resources). Quando lhe forem pedidas credenciais pelo Gestor de Credenciais do Git, confirme que introduz as [credenciais de implementação](../app-service-deployment-credentials.md?toc=%2fazure%2fapp-service%2fcontainers%2ftoc.json) e não as que utilizou para iniciar sessão no portal do Azure.
...
Vai utilizar o Azure Active Directory como o fornecedor de identidade. Para obter mais informações, veja [Configure Azure Active Directory authentication for your App Services application](../app-service-mobile-how-to-configure-active-directory-authentication.md?toc=%2fazure%2fapp-service%2fcontainers%2ftoc.json) (Configurar a autenticação do Azure Active Directory na sua aplicação dos Serviços de Aplicação).
...
No [portal do Azure](https://portal.azure.com), clique no menu da esquerda para abrir a página de gestão da aplicação de back-end: **Grupos de recursos** > **myAuthResourceGroup** > _\<back\_end\_app\_name>_.
...
Se preferir, navegue para `http://<front_end_app_name>.azurewebsites.net`. Deve ser agora direcionado para uma página de início de sessão. Depois de iniciar sessão, continua sem poder aceder aos dados da aplicação de back-end, porque ainda tem de fazer três coisas:
...
Para obter informações sobre como configurar esta opção para outros fornecedores, veja [Refresh access tokens](../app-service-authentication-how-to.md?toc=%2fazure%2fapp-service%2fcontainers%2ftoc.json#refresh-access-tokens) (Atualizar os tokens de acesso).
...
Volte a iniciar sessão em `http://<front_end_app_name>.azurewebsites.net`. Na página do contrato de utilização de dados do utilizador, clique em **Aceitar**.
...
Na Linha 51, defina a variável `apiEndpoint` como o URL da aplicação de back-end (`http://<back_end_app_name>.azurewebsites.net`). Substitua _\<back\_end\_app\_name>_ pelo nome da sua aplicação no Serviço de Aplicações.
...
Navegue novamente até `http://<front_end_app_name>.azurewebsites.net`. Deverá ser agora capaz de criar, ler, atualizar e eliminar os dados da aplicação de back-end, diretamente na aplicação Angular.js.
