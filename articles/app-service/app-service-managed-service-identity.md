...
ms.locfileid: "37061442"
---
# <a name="how-to-use-azure-managed-service-identity-in-app-service-and-azure-functions"></a>Como utilizar o Azure identidade de serviço geridas no serviço de aplicações e funções do Azure

> [!NOTE] 
> No Linux e a aplicação Web para contentores do serviço de aplicações não suportam atualmente identidade de serviço geridas.

> [!Important] 
> Identidade de serviço geridas para o serviço de aplicações e funções do Azure não irão comportar-se conforme esperado se a aplicação é migrada através de subscrições/inquilinos. A aplicação precisa de obter uma nova identidade, o que pode ser efetuada, desativando e voltando a reativar a funcionalidade. Consulte [remover uma identidade](#remove) abaixo. Recursos a jusante também tem de ter atualizadas para utilizar a nova identidade de políticas de acesso.
...
Os passos seguintes irão guiá-lo através da criação de uma aplicação web e atribuição do mesmo uma identidade com o Azure PowerShell:
...
2. Crie uma aplicação web com o Azure PowerShell. Para obter mais exemplos de como utilizar o Azure PowerShell com o App Service, consulte [exemplos do PowerShell do serviço de aplicações](../app-service/app-service-powershell-samples.md):
...
Onde `<TENANTID>` e `<PRINCIPALID>` são substituídos por GUIDs. A propriedade tenantId identifica que pertence a identidade de inquilino do AAD. O principalId é um identificador exclusivo para a identidade da aplicação de novo. Dentro do AAD, o serviço principal tem o mesmo nome que deu à sua instância do serviço de aplicações ou funções do Azure.
...
Para saber mais sobre Microsoft.Azure.Services.AppAuthentication e as operações de expõe, consulte o [Referência de Microsoft.Azure.Services.AppAuthentication] e [do serviço de aplicações e KeyVault com .NET de MSI exemplo](https://github.com/Azure-Samples/app-service-msi-keyvault-dotnet).
...
<a name="token-csharp"></a>Para efetuar este pedido em c#:
```csharp
...
Uma identidade pode ser removida ao desativar a funcionalidade com o portal, o PowerShell ou o CLI da mesma forma que tenha sido criado. O protocolo de modelo REST/ARM, isto é feito ao definir o tipo "None":
...
Remover a identidade desta forma, eliminará também o principal do AAD. Identidades atribuídas de sistema são automaticamente removidas do AAD ao eliminar o recurso de aplicação.

> [!NOTE] 
> Também é uma definição da aplicação que pode ser definida, WEBSITE_DISABLE_MSI, que apenas desativa o serviço de token local. No entanto, sai de identidade no local e ferramentas ainda aparecer MSI como "ativado" ou "ativado". Como resultado, a utilização desta definição não é recommmended.
