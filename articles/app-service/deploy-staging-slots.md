...
Não existe conteúdo após a criação de ranhura de implementação. Pode implementar para o bloco de um ramo do repositório diferente ou um repositório completamente diferente. Também pode alterar a configuração da ranhura. Utilize as credenciais de implementação ou perfil de publicar associadas com o bloco de implementação para atualizações de conteúdo.  Por exemplo, pode [publicar esta ranhura com o git](deploy-local-git.md).
