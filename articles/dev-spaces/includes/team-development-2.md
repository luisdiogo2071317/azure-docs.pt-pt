...
1. Pode confirmar que o serviço está a ser executado no seu próprio espaço executando `azds space list` novamente. Em primeiro lugar, verá uma instância de `mywebapi` está agora em execução `scott` espaço (a versão em execução no `default` está ainda em execução, mas não estiver listado). Lugar, o acesso ao ponto de URL para `webfrontend` é o prefixo com o texto "scott.s.". Este URL é exclusivo para o `scott` espaço. O URL especial significa que os pedidos enviados para o URL"blogue" irão tentar efetuar a primeira rotas a serviços no `scott` espaço, mas se não conseguir, será revertem para serviços no `default` espaço.
