...
2.  Para ver outros exemplos de configuração personalizada, ligue para o contentor de pré-visualização pública com o Explorador de armazenamento do Azure.
    ...
    d. Faça duplo clique o `UserScenarios` pasta. Nesta pasta são os seguintes itens:
       ...
       1. A `.NET FRAMEWORK 3.5` pasta, que contém uma configuração personalizada para instalar uma versão anterior do .NET Framework que poderão ser necessárias para componentes personalizados em cada nó do seu IR. SSIS do Azure

       2. A `BCP` pasta, que contém uma configuração personalizada para instalar os utilitários da linha de comandos do SQL Server (`MsSqlCmdLnUtils.msi`), incluindo o programa de cópia em massa (`bcp`), em cada nó do seu IR. SSIS do Azure
