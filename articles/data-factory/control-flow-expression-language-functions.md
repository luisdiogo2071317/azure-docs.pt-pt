...
|não|Devolve VERDADEIRO se o parâmetro for `false`. Ambos os argumentos precisam ser booleanos. A seguinte devolve `true`:  `not(contains('200 Success','Fail'))`<br /><br /> **Número de parâmetro**: 1<br /><br /> **Nome**: booleano<br /><br /> **Descrição**: devolve true se o parâmetro é `false`. Ambos os argumentos precisam ser booleanos. A seguinte devolve `true`:  `not(contains('200 Success','Fail'))`|  
