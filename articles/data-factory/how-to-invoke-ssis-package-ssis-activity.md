...
As instruções neste artigo utilizam uma base de dados SQL do Azure que aloja o catálogo do SSIS. Também pode utilizar uma instância de gerida de SQL no Azure (pré-visualização).
...
2. Na **atividades** caixa de ferramentas, expanda **gerais**e arraste e largue o **executar pacotes do SSIS** atividade para a superfície de desenho do pipeline. 
   ...
   ![Arraste a atividade do SSIS para a superfície de desenho](media/how-to-invoke-ssis-package-ssis-activity/ssis-activity-designer.png) 
...
Opcionalmente, atribuir valores, expressões ou funções, o que podem referenciar variáveis de sistema do Data Factory, para os parâmetros do projeto ou o pacote no formato JSON utilizando o **vista de código-fonte** botão na parte inferior do SSIS a executar Caixa de atividade do pacote, ou o **código** botão no canto superior direito da área do pipeline. Por exemplo, pode atribuir parâmetros do pipeline de fábrica de dados ao seu projeto do SSIS ou parâmetros de pacote, conforme mostrado nas capturas de ecrã seguintes:
...
4. Clique na ligação **Ver Execuções de Atividades**, na coluna **Ações**. Pode ver apenas uma atividade executar conforme o pipeline tem apenas uma atividade (atividade a executar o pacote do SSIS).

    ![Execuções de atividade](./media/how-to-invoke-ssis-package-ssis-activity/ssis-activity-runs.png)

5. Pode executar o seguinte procedimento **consulta** contra o SSISDB de base de dados no seu servidor SQL do Azure para verificar se o pacote executado. 
...
6. Também pode obter o ID de execução do SSISDB a partir da saída da execução de atividade do pipeline e utilize o ID para verificar os registos de execução mais abrangentes e mensagens de erro no SSMS.
...
Nesta secção, utilizar o Azure PowerShell para criar um pipeline do Data Factory com uma atividade SSIS que executa um pacote do SSIS. 
...
### <a name="create-a-data-factory"></a>Criar uma fábrica de dados
Pode utilizar a fábrica de dados mesmo que tenha o IR Azure-SSIS ou criar uma fábrica de dados separado. O procedimento seguinte fornece passos para criar uma fábrica de dados. Criar um pipeline com uma atividade SSIS nesta fábrica de dados. A atividade SSIS é executado o pacote do SSIS. 
...
Neste passo, vai criar um pipeline com uma atividade do SSIS. A atividade executa seu pacote do SSIS. 
